...
export default function ApiPlayground() {
  ...
  const [shouldRenderResponse, setShouldRenderResponse] = useState(false);
  ...
  // Animate selected endpoint panel on change
  useGSAP(() => {
    if (!selectedPanelRef.current || !selectedEndpoint) return;
    gsap.from(selectedPanelRef.current, {
      opacity: 0,
      scale: 0.98,
      duration: 0.25,
      ease: "power2.out",
  ...
  // Manage response panel render + animation
  useEffect(() => {
    if (response) {
      setShouldRenderResponse(true);
    }
  }, [response]);

  useEffect(() => {
    if (!responsePanelRef.current || !shouldRenderResponse) return;
    if (response) {
      gsap.fromTo(
      ...
      gsap.to(responsePanelRef.current, {
        opacity: 0,
        y: 10,
        duration: 0.2,
        onComplete: () => setShouldRenderResponse(false),
      });
    }
  }, [response, shouldRenderResponse]);
        ...
        {selectedEndpoint ? (
          ...
          >
            ...
            {shouldRenderResponse && (
              <div ref={responsePanelRef} className="space-y-4">
                <div className="flex items-center justify-between">
