...
export function RoadmapKanbanBoard({
  ...
  const itemsByStatus = useMemo(() => {
    ...
    items.forEach((item) => {
      const status = (item as any).status || "planned";
      if (status in groups) {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    const activeStatus = (activeItem as any).status || "planned";

    // If over a column or an item in a different column
    let overStatus = overId;
    if (typeof overId === "number") {
      const overItem = items.find((i) => i.id === overId);
      overStatus = (overItem as any).status || "planned";
    }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (dragActiveId !== overId) {
      ...
      if (activeItem && overItem && (activeItem as any).status === (overItem as any).status) {
        const status = (activeItem as any).status || "planned";
