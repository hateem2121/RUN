...
interface ProductData {
  ...
  selectedFiberComposition?: string;
  customWeight?: string;
  customFit?: string;
  customizationOptions?: string[];
  certificates?: Array<{ name?: string } | string>;
  fabric?: FabricData;
...
export const createTechnologySpecifications = (productData?: ProductData): SpecificationSection => {
  const technicalSpecs = productData?.technicalSpecs || {};
  const defaultItems = [
...
export const createSustainabilitySpecifications = (
  ...
  if (certificates.length > 0) {
    items.push({
      ...
      value: certificates
        .map((cert) => (typeof cert === "object" && cert !== null ? cert.name || "" : cert))
        .join(", "),
    });
  }
...
export const createCareSpecifications = (productData?: ProductData): SpecificationSection => {
