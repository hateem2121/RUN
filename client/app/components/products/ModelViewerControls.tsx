...
export function ModelViewerControls({
  ...
  const setCameraPosition = (position: (typeof positions)[0]) => {
    if (modelViewerRef.current) {
      ...
      if ("target" in position && typeof position.target === "string") {
        modelViewerRef.current.cameraTarget = position.target;
      }
      if ("fieldOfView" in position && typeof position.fieldOfView === "string") {
  ...
  const toggleAnnotations = () => {
    if (modelViewerRef.current) {
      ...
      annotations.forEach((annotation) => {
        (annotation as HTMLElement).style.display = showAnnotations ? "none" : "block";
      });
