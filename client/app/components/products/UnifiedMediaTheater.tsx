...
export function UnifiedMediaTheater({
  ...
  // GSAP slide-in animation when selected media changes
  useGSAP(() => {
    if (!slideRef.current) return;
    const dur = prefersReducedMotion ? 0.1 : 0.3;
    gsap.fromTo(
      slideRef.current,
      { opacity: 0, x: direction * 100 },
      { opacity: 1, x: 0, duration: dur, ease: "power2.inOut" },
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {currentMedia && (
            <div ref={slideRef} key={currentMedia.id} className="absolute inset-0">
              {isVideo ? (
                <div className="relative h-full w-full">
                  ...
                  >
                    <button
                      ...
                      className="pointer-events-auto rounded-full bg-black/50 p-4 text-white transition-all hover:bg-black/70 active:scale-95"
                ...
                <div className="relative h-full w-full">
                  ...
                  <LazyUnifiedModelViewer
                    asset={modelAsset as MediaAsset}
                    config={modelConfig as ModelViewerConfig}
      ...
      {!isFullscreen && (
        <div className="mt-3 space-y-1 text-center text-xs text-(--product-muted)">
