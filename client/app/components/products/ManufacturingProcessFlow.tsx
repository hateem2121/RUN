...
export function ManufacturingProcessFlow() {
  ...
  useGSAP(
    () => {
      ...
      // Animate process node entrance (replaces motion.div scale: 0 → 1)
      ...
      // Animate detail cards entrance (replaces motion.div opacity/y)
      gsap.from(".process-detail-card", {
        opacity: 0,
        y: 20,
        duration: 0.4,
        stagger: 0.1,
    ...
    { scope: containerRef, dependencies: [activeProcesses] },
  );
        ...
        <div ref={containerRef} className="relative">
          ...
          <div className="relative flex items-center justify-between">
            {activeProcesses.map((process, index) => (
              <div key={process.id} className="relative flex-1">
                ...
                {/* Process Node */}
                <div className="process-node relative z-default flex flex-col items-center">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            {activeProcesses.map((process, index) => (
              <div
                key={process.id}
                className="process-detail-card rounded-lg border border-blue-200 bg-linear-to-br from-background to-blue-50 p-3"
              >
