...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  const callIfFn = useCallback(<T extends object, K extends keyof T>(obj: T | null, key: K) => {
  ...
  if (!webglSupport?.supported) {
    return (
      ...
      >
        ...
        <div
          className="mix-blend-[var(--gb-blend-mode)] absolute inset-0 block bg-[image:linear-gradient(var(--gb-angle),var(--gb-color1),var(--gb-color2))]"
          style={cssCustomProps as React.CSSProperties}
