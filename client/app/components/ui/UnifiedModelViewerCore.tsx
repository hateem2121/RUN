...
export function UnifiedModelViewer({
  ...
  const setupModelViewerEvents = useCallback(() => {
    ...
    const handleModelLoaded = () => {
      ...
      // Track performance
      if (loadingState.startTime > 0) {
        const duration = Date.now() - loadingState.startTime;
        reportCustomMetric("3D_LOAD_TIME", duration, asset.id.toString());
      }
    };
  ...
  useEffect(() => {
    if (finalConfig.loading === "lazy" && containerRef.current) {
      ...
      observerRef.current = new IntersectionObserver(
        ...
        {
          threshold: 0.1,
          rootMargin: "200px", // Start loading 200px before element comes into view for smoother experience
        },
      );
      ...
      const isInViewport = rect.top < globalThis.innerHeight + 200 && rect.bottom > 0;
    ...
    >
      ...
      >
        ...
        {(!shouldLoadModel || !isVisible) && (
          <div className="center-flex from-surface-subtle to-surface-muted dark:from-muted dark:to-background absolute inset-0 bg-linear-to-br overflow-hidden">
            ...
            <div className="space-y-2 text-center z-10">
              ...
              {!shouldLoadModel ? (
                <p className="text-text-muted text-xs font-medium">Scroll to load 3D</p>
              ) : (
                <p className="text-text-muted text-xs font-medium">Model hidden</p>
              )}
            </div>
          </div>
        )}
