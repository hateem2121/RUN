...
function InitiativeRow({
  ...
  return (
    ...
    >
      {/* Image side */}
      <div className="initiative-image relative h-custom-space-231 w-full overflow-hidden rounded-custom-space-232 lg:w-1/2 group shadow-xl hover:shadow-2xl transition-all duration-500">
        {imageUrl ? (
          <img
            src={imageUrl}
          ...
          <div className="h-full w-full bg-gradient-to-br from-custom-misc-299/10 to-custom-misc-300/5 flex items-center justify-center">
            <div className="text-custom-misc-301/30 text-6xl">🌿</div>
          </div>
        )}
      ...
      <div className={cn("initiative-content flex flex-col gap-6 lg:w-1/2 lg:p-16")}>
        {initiative.impact && (
          <span className="text-sm font-bold uppercase tracking-widest text-custom-misc-302">
            {initiative.impact}
          </span>
        )}
        <h3 className="text-4xl lg:text-5xl font-light text-custom-misc-303">
          {titleMain} <span className="font-bold text-custom-misc-304">{titleAccent}</span>
        </h3>
...
export function InitiativesSection({
  ...
  return (
    <section
      ref={sectionRef}
      className="relative bg-custom-misc-308 py-24"
      aria-label="Sustainability initiatives"
    >
      <div className="container mx-auto px-6 lg:px-10">
        <div className="initiatives-header mb-20 max-w-2xl">
          <h2 className="font-neue-stance text-3xl font-bold text-custom-misc-309 md:text-5xl mb-4">
            {title}
          </h2>
          <p className="text-lg text-custom-misc-310">{description}</p>
        </div>
