            ...
            {roadmap.map((item, index) => (
              <div key={item.id} className={`flex flex-col md:flex-row gap-8 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                 ...
                 <div className="md:w-1/2 pl-12 md:pl-0 md:pr-12 md:text-right relative">
                    {/* Dot on line */}
                    <div className={`absolute left-0 top-1 w-3 h-3 rounded-full bg-primary border-4 border-black box-content 
                    ...
                    {index % 2 !== 0 && (
                       <Card className="bg-black/50 border-white/10 p-6 backdrop-blur-sm hover:border-primary/30 transition-colors inline-block text-left w-full">
                          <span className="text-primary font-mono text-sm mb-2 block">{item.timeline}</span>
