...
(() => {
  ...
  function testFocusManagement() {
    ...
    const hasTabindex = Array.from(focusable).some(
      (el) => el.hasAttribute("tabindex") || el.tagName.toLowerCase() === "button",
    );
  ...
  function testZIndexSystem() {
    ...
    const zIndices = ["--z-modal-backdrop", "--z-modal", "--z-modal-nested", "--z-modal-overlay-1"];
  ...
  async function testApiIntegration() {
    try {
      ...
      if (mediaResponse.ok) {
        ...
        const isValid =
          mediaData &&
          (Array.isArray(mediaData) || (mediaData.data && Array.isArray(mediaData.data)));
