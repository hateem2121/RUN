...
const pristineFetch = async (url: string, options: RequestInit = {}): Promise<Response> => {
  ...
  const timeoutId = setTimeout(() => {
    controller.abort();
    console.warn(`⏰ [Worker Timeout] Request to ${url} timed out after 5 minutes`);
  }, 5 * 60 * 1000); // 5 minutes to match apiRequest
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      console.error(`💥 [Worker Abort] Upload to ${url} cancelled or timed out`);
...
const initializeUpload = async (_fileId: string, filename: string, size: number, mimeType: string): Promise<{uploadId: string, chunkSize: number, totalChunks: number}> => {
...
const uploadChunk = async (
  ...
  // ENHANCED LOGGING: Confirm exact URL and headers being sent
  console.log(`[Worker Chunk Upload] Chunk ${chunkIndex}: POST ${targetUrl}`);
  console.log(`[Worker Chunk Upload] Headers:`, headers);
  console.log(`[Worker Chunk Upload] Body size: ${chunkData.byteLength} bytes`);
  ...
  console.log(`[Worker Chunk Upload] Response: ${response.status} ${response.statusText}`);
  console.log(`[Worker Chunk Upload] Response headers:`, Object.fromEntries(response.headers.entries()));
  ...
  if (!response.ok) {
    ...
    console.error(`[Worker Chunk Upload] Error response body:`, responseBody);
    throw new Error(`Chunk ${chunkIndex} failed: ${response.status} ${response.statusText} - ${responseBody}`);
  }
  ...
  try {
    switch (type) {
      case 'start': {
        ...
        const { uploadId, chunkSize, totalChunks } = await initializeUpload(fileId, name, size, mimeType);
