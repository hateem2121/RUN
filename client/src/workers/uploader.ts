...
const pristineFetch = async (
	...
	try {
		...
		if (error instanceof Error && error.name === "AbortError") {
			throw new Error(
				`Worker upload timeout: Request took longer than 5 minutes`,
			);
		}
		throw error;
	}
...
const finalizeUpload = async (uploadId: string): Promise<any> => {
	...
	if (!response.ok) {
		throw new Error(
			`Finalize failed: ${response.status} ${response.statusText}`,
		);
	}
...
const processFileUpload = async (session: UploadSession): Promise<void> => {
	const { fileId, uploadId, file, chunkSize, totalChunks, abortController } =
		session;
	let uploadedBytes = 0;
