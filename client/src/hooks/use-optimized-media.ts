...
export function useOptimizedMedia(
	...
	const urls = mediaData?.variants
		? generateOptimizedUrls(mediaData.variants)
		: null;
	const srcSet = mediaData?.variants
		? generateSrcSet(mediaData.variants)
		: undefined;
...
export function useMediaPerformanceMonitor() {
	...
	const recordLoadTime = (
		...
		setStats((prev) => ({
			...
			averageLoadTime:
				(prev.averageLoadTime * prev.totalRequests + loadTime) /
