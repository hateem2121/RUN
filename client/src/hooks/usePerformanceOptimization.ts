...
export function usePerformanceOptimization({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (renderTime > (thresholds.renderTime || 16)) {
        console.warn(`[Performance] ${componentName} render time: ${renderTime.toFixed(2)}ms (threshold: ${thresholds.renderTime}ms)`);
      }
    };
  });
  ...
  const trackMemoryUsage = useCallback(() => {
    ...
    if (usedMB > (thresholds.memoryUsage || 50)) {
      console.warn(`[Performance] ${componentName} memory usage: ${usedMB.toFixed(2)}MB (threshold: ${thresholds.memoryUsage}MB)`);
    }
  }, [componentName, enableMemoryTracking, thresholds.memoryUsage]);

  // API performance tracking
  const trackApiCall = useCallback((apiKey: string) => {
    ...
    return () => {
      ...
      if (responseTime > (thresholds.apiResponseTime || 200)) {
        console.warn(`[Performance] ${componentName} API ${apiKey} response time: ${responseTime.toFixed(2)}ms (threshold: ${thresholds.apiResponseTime}ms)`);
      }
  ...
  const optimizeComponentCount = useCallback((count: number) => {
    metrics.current.componentCount = count;

    if (count > 100) {
      console.warn(`[Performance] ${componentName} has ${count} components. Consider virtualization.`);
    }
  }, [componentName]);
