...
export function useAnalytics(options: UseAnalyticsOptions = {}) {
  ...
  useEffect(() => {
    if (location !== prevLocationRef.current) {
      ...
      if (prevLocationRef.current && timeOnPreviousPage > 1000) { // Only if spent more than 1 second
...
export function useMediaAnalytics() {
  const trackMediaLoad = (assetId: number, startTime: number, cacheStatus?: string, fileSize?: number) => {
    const loadTime = Date.now() - startTime;
    analytics.trackMediaPerformance(
      assetId, 
      loadTime, 
      cacheStatus || 'unknown', 
      fileSize || 0
