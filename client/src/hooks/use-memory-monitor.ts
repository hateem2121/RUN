...
export function useMemoryMonitor() {
  ...
  const forceCleanup = useCallback(() => {
    ...
    if ('gc' in window && typeof window.gc === 'function') {
      try {
        window.gc();
        if (process.env.NODE_ENV === 'development') console.log('[Memory Monitor] Forced garbage collection');
      } catch (e) {
        if (process.env.NODE_ENV === 'development') console.warn('[Memory Monitor] GC not available');
      }
    }
    ...
    modelViewers.forEach((viewer, index) => {
      if (index > 1 && viewer.src !== undefined) { // Keep only first model loaded
        viewer.src = '';
      }
    });
    ...
    if (process.env.NODE_ENV === 'development') console.log('[Memory Monitor] Forced cleanup completed');
  }, []);
    ...
    const interval = setInterval(() => {
      ...
      if (memoryInfo) {
        if (memoryInfo.critical) {
          if (process.env.NODE_ENV === 'development') console.warn(`[Memory Monitor] CRITICAL: ${memoryInfo.usedMB}MB/${memoryInfo.limitMB}MB (${memoryInfo.percentage}%)`);
          forceCleanup();
          ...
          if (process.env.NODE_ENV === 'development') console.log(`[Memory Monitor] WARNING: ${memoryInfo.usedMB}MB/${memoryInfo.limitMB}MB (${memoryInfo.percentage}%)`);
        }
      }
