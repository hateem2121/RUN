...
export function useAnalyticsTracker() {
	...
	const trackPageView = useCallback((page: string) => {
		const analytics = JSON.parse(
			localStorage.getItem("productAnalytics") || "{}",
		);
	...
	const trackProductView = useCallback((productName: string) => {
		...
		analytics.productViews[productName] =
			(analytics.productViews[productName] || 0) + 1;
		localStorage.setItem("productAnalytics", JSON.stringify(analytics));
	}, []);
	...
	const trackSearch = useCallback((searchTerm: string) => {
		...
		const existing = analytics.searchTerms.find(
			(item: SearchTerm) => item.term === searchTerm,
		);
		if (existing) {
			existing.count++;
		} else {
		...
		analytics.searchTerms.sort(
			(a: SearchTerm, b: SearchTerm) => b.count - a.count,
		);
	...
	const trackFilterUsage = useCallback((filterType: string) => {
		...
		const existing = analytics.filterUsage.find(
			(item: FilterUsage) => item.filter === filterType,
		);
		if (existing) {
			existing.count++;
		} else {
	...
	const trackViewMode = useCallback((mode: string) => {
		...
		const total =
			analytics.viewModeUsage.reduce(
				(sum: number, item: ViewModeUsage) => sum + item.count,
				0,
			) + 1;

		const existing = analytics.viewModeUsage.find(
			(item: ViewModeUsage) => item.mode === mode,
		);
		if (existing) {
			existing.count++;
		} else {
	...
	const trackFunnelStage = useCallback((stage: string) => {
		...
		if (stageIndex !== -1) {
			...
			analytics.conversionFunnel.forEach((item: ConversionFunnelStage) => {
				item.percentage =
					baseCount > 0 ? Math.round((item.count / baseCount) * 100) : 0;
			});
		}
