...
export function useAnalyticsTracker() {
  ...
  const trackViewMode = useCallback((mode: string) => {
    ...
    const total = analytics.viewModeUsage.reduce((sum: number, item: ViewModeUsage) => sum + item.count, 0) + 1;
  ...
  const trackFunnelStage = useCallback((stage: string) => {
    ...
    const stageIndex = analytics.conversionFunnel.findIndex((item: ConversionFunnelStage) => item.stage === stage);
    if (stageIndex !== -1) {
