...
export function useResourceData(type: ResourceType | 'all') {
  ...
  const { data: sizeCharts = [], isLoading: sizeChartsLoading } = useQuery<SizeChart[]>({
  ...
  const isLoading = type === 'all' 
    ? certificatesLoading || accessoriesLoading || sizeChartsLoading || fabricsLoading || fibersLoading
