...
export function useViewportAwarePositioning() {
  const [viewport, setViewport] = useState<ViewportDimensions>(() => ({
    width: typeof window !== 'undefined' ? window.outerWidth : 1024,
    height: typeof window !== 'undefined' ? window.outerHeight : 768,
   ...
   */
  const calculateModalBounds = useCallback((constraints: PositionConstraints): ModalBounds => {
  ...
  const calculateOptimalPosition = useCallback((
    ...
    const suggestedSize = {
      width: contentWidth > maxWidth ?
        `min(${maxWidth}px, 95vw)` :
        `min(${contentWidth}px, 95vw)`,
      height: contentHeight > maxHeight ?
        `min(${maxHeight}px, 95vh)` :
        `min(${contentHeight}px, 95vh)`,
    };
   ...
   */
  const calculatePosition = useCallback((constraints: PositionConstraints): PositionResult => {
    const bounds = calculateModalBounds(constraints);
