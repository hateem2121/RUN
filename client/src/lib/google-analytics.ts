...
export class GoogleAnalytics {
   ...
   */
  public trackPageView(path?: string, title?: string, customParams?: GAPageView['custom_parameters']): void {
  ...
  public trackPagePerformance(): void {
    ...
    new PerformanceObserver((entryList) => {
      for (const entry of entryList.getEntries()) {
        const fidEntry = entry as PerformanceEntry & { processingStart: number; id?: string };
    ...
    new PerformanceObserver((entryList) => {
      for (const entry of entryList.getEntries()) {
        const clsEntry = entry as PerformanceEntry & { hadRecentInput?: boolean; value: number; id?: string };
   ...
   */
  public trackProductInteraction(action: 'view' | 'click' | 'inquiry', productName: string, categoryName?: string): void {
    this.trackEvent({
   ...
   */
  public trackMediaPerformance(assetId: number, loadTime: number, cacheStatus: string, fileSize: number): void {
    this.trackEvent({
      action: 'media_load',
      category: 'Performance',
      label: `Asset ${assetId}`,
  ...
  public getConfig() {
    return {
      ...
      measurementId: this.measurementId ? `G-${this.measurementId.split('-')[1]}***` : 'Not configured',
