 ...
 */
export function getModelViewerConfig(
	overrides?: Partial<ModelViewerConfig>,
): ModelViewerConfig {
	return {
...
export const MODEL_VIEWER_ENVIRONMENT = {
	...
	storage: {
		...
		preloadEnabled:
			import.meta.env.VITE_MODEL_VIEWER_PRELOAD_ENABLED !== "false",
		cacheDuration: parseInt(
			import.meta.env.VITE_GLTF_CACHE_DURATION || "31536000",
			10,
		),
	},
...
export function validateModelViewerConfig(config: ModelViewerConfig): {
	...
	if (
		...
		(config.exposure < 0.1 || config.exposure > 5.0)
	) {
		warnings.push(
			`Exposure value ${config.exposure} is outside recommended range 0.1-5.0`,
		);
	}
	...
	if (
		...
		warnings.push(
			`Shadow intensity ${config.shadowIntensity} is outside valid range 0.0-2.0`,
		);
	}
	...
	if (
		...
		!validPolicies.includes(config.interactionPolicy)
	) {
		errors.push(
