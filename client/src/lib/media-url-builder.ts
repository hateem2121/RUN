...
export type MediaAssetWithMetadata = Pick<MediaAsset, 'id' | 'mimeType' | 'originalName' | 'filename'>;
...
export class MediaUrlBuilder {
  ...
  static buildContentUrl(id: number | undefined | null): string | null {
    if (!id || id <= 0) {
      if (import.meta.env.DEV) {
        console.error('[MediaUrlBuilder] Invalid asset ID for content URL:', id);
      }
      return null;
    }
  ...
  static buildUrlSafe(id: number | undefined | null, fallback?: string): string {
    ...
    if (import.meta.env.DEV) {
      console.warn('[MediaUrlBuilder] Using fallback for invalid asset ID:', id);
    }
   ...
   */
  private static createAlternativeAsset(asset?: MediaAssetWithMetadata): MediaAssetWithMetadata | undefined {
  ...
  private static getDefaultModelFallback(): string {
    ...
    const pngFallback = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==';
  ...
  static isValidUrl(url: string | null | undefined): boolean {
    ...
    try {
      ...
      if (url.startsWith('/api/media/')) {
        ...
        return pathParts.length >= 4 && pathParts[3] !== undefined && !isNaN(parseInt(pathParts[3]));
      }
      ...
      if (import.meta.env.DEV) {
        console.warn('[MediaUrlBuilder] URL validation error:', error);
      }
      return false;
    }
  }
