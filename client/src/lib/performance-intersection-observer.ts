...
export function useAnimationIntersectionObserver(
	...
	const shouldAnimate = useMemo(() => {
		return (
			enableAnimations &&
			(intersectionResult.isIntersecting || intersectionResult.hasIntersected)
		);
	}, [
		enableAnimations,
		intersectionResult.isIntersecting,
		intersectionResult.hasIntersected,
	]);
...
export function useImageIntersectionObserver(
	...
	const shouldLoad =
		intersectionResult.isIntersecting || intersectionResult.hasIntersected;
	const shouldPreload =
		(intersectionResult.entry?.intersectionRatio || 0) >= preloadThreshold;
...
export function useBatchIntersectionObserver<T extends Element>(
	...
	const [results, setResults] = useState(
		new Map<RefObject<T | null>, IntersectionResult>(),
	);
			...
			const handleIntersection = (entry: IntersectionObserverEntry) => {
				setResults((prev) => {
					...
					newResults.set(ref, {
						...
						hasIntersected:
							currentResult.hasIntersected || entry.isIntersecting,
						entry,
					});
