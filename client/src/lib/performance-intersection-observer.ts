...
export function useAnimationIntersectionObserver(
  ...
  const shouldAnimate = useMemo(() => {
    return enableAnimations && (intersectionResult.isIntersecting || intersectionResult.hasIntersected);
    ...
    refs.forEach(ref => {
      ...
      const handleIntersection = (entry: IntersectionObserverEntry) => {
        setResults(prev => {
          ...
          const currentResult = newResults.get(ref) || { isIntersecting: false, hasIntersected: false };
...
export function withPerformanceOptimization<P extends Record<string, unknown> = Record<string, unknown>>(
  ...
  const WrappedComponent: React.FC<P> = (props: P) => {
    ...
    if (!hasIntersected) {
      return React.createElement('div', { ref: containerRef, className: 'min-h-[200px]' });
    }
