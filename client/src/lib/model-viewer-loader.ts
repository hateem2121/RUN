  ...
  loadingPromise = new Promise(async (resolve, reject) => {
    try {
      ...
      if (typeof window !== "undefined" && typeof window.createImageBitmap === "function") {
        try {
          console.log(
            "[Model Viewer Loader] 🛡️ Disabling ImageBitmap to prevent DevTools fetch conflicts...",
          );
          const win = window as unknown as Record<string, unknown>;
          delete win.createImageBitmap;
          console.log(
            "[Model Viewer Loader] ✅ ImageBitmap disabled - will use HTMLImageElement fallback",
          );
        } catch (error) {
      ...
      if (typeof globalThis !== "undefined") {
        ...
        if (typeof process === "undefined") {
          ...
          console.log(
            `[Model Viewer Loader] 🎯 Configuring Lit for production mode (was: ${originalEnv})`,
          );
        }
      }
