    ...
    for (const link of links) {
      ...
      if (href && href.includes('/assets/')) {
        try {
          ...
          console.warn('Could not measure stylesheet size for:', href);
        }
      }
    }
...
Recommendations:
${score < 90 ? '- Consider further optimizing bundle sizes\n- Implement more aggressive lazy loading\n- Optimize image loading strategies' : '- Performance is excellent!\n- Continue monitoring for regressions'}
    `.trim();
  }
...
export const measureAsyncOperation = async (operationName: string, fn: () => Promise<any>) => {
  const start = performance.now();
  const result = await fn();
  const end = performance.now();
  console.log(`[Performance] ${operationName} duration: ${(end - start).toFixed(2)}ms`);
  return result;
};
  ...
  if ('memory' in performance) {
    ...
    setTimeout(() => {
      ...
      if (increase > 5 * 1024 * 1024) { // 5MB increase
        console.warn(`[Performance] Potential memory leak detected: ${(increase / 1024 / 1024).toFixed(2)}MB increase`);
      }
