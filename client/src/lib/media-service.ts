...
export class MediaService {
  ...
  private static generateIdBasedUrl(asset: MediaAsset): string | null {
    ...
    if (asset.id >= 1000000000000) {
      console.warn('[MediaService] ID too large, skipping to prevent database overflow:', asset.id);
      return null;
    }
  ...
  private static generateFallbackUrl(asset: MediaAsset): string | null {
    if (!asset.filename || typeof asset.filename !== 'string') {
      console.warn('[MediaService] No valid filename for asset:', asset.id);
      return null;
    }
    
    // Validate filename doesn't contain 'undefined'
    if (asset.filename.includes('undefined') || asset.filename.includes('null')) {
      console.warn('[MediaService] Invalid filename detected:', asset.filename);
      return null;
    }
    
    // DEPRECATED: Filename-based URLs are unsafe and deprecated
    console.warn('[MediaService] Filename-based URLs deprecated, returning null for safety');
    return null;
  }
  ...
  static getVideoThumbnailUrl(asset: MediaAsset): string | null {
    ...
    if (asset.thumbnailFilename && asset.thumbnailFilename.length > 0) {
      // DEPRECATED: Filename-based URLs are unsafe and deprecated
      console.warn('[MediaService] Thumbnail filename URLs deprecated, returning null for safety');
      return null;
    }
