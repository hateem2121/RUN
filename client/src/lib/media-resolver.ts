...
export class MediaResolver {
	...
	static getAssetUrl(id?: number, fallbackId?: number): string | null {
		const asset =
			MediaResolver.getAsset(id) || MediaResolver.getAsset(fallbackId);
		...
		if (asset.id >= 1000000000000) {
			warnLog(
				"[MediaResolver] ID too large, skipping to prevent database overflow:",
				asset.id,
			);
			return null;
		}
	...
	static getRandomAssetId(type?: string): number | null {
		...
		const filteredAssets = type
			? MediaResolver.getAssetsByType(type)
			: MediaResolver.assets;
