...
class PerformanceTracker {
	...
	private recordMetric(metric: WebVitalsMetric) {
		...
		const existingIndex = this.metrics.webVitals.findIndex(
	...
	getHealthStatus() {
		const poorMetrics = this.metrics.webVitals.filter(
			(m) => m.rating === "poor",
		);
		const needsImprovement = this.metrics.webVitals.filter(
