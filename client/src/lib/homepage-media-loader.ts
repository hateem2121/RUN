...
type BatchMediaAsset = Pick<
  MediaAsset,
  "id" | "filename" | "type" | "mimeType" | "url"
> & {
  size: number | null;
  content?: string;
  cached?: boolean;
...
export function useHomepageMediaLoader(
  ...
  const { data: initialBatch, isLoading: isInitialLoading } =
    useQuery<MediaBatch>({
      queryKey: [
  ...
  const loadMoreAssets = useCallback(async () => {
    ...
    try {
      ...
      console.error("[HomepageMediaLoader] Failed to load more assets:", error);
    } finally {
