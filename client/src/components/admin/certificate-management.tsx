...
const getCertificateTypeIcon = (type: string | undefined) => {
  ...
  switch (type.toLowerCase()) {
    case "sustainability": return <Award className="w-4 h-4" />;
    case "compliance": return <Shield className="w-4 h-4" />;
    case "quality": return <CheckCircle className="w-4 h-4" />;
    case "safety": return <Shield className="w-4 h-4" />;
    case "environmental": return <Award className="w-4 h-4" />;
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <Label className="text-sm text-neutral-600">
            Select all {certificates.length} certificate{certificates.length === 1 ? '' : 's'}
          </Label>
        </div>
      ...
      {viewMode === "grid" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {certificates.map((certificate) => (
            <Card key={certificate.id} className={`group relative transition-all duration-200 hover:shadow-lg ${selectedCertificates.includes(certificate.id) ? 'ring-2 ring-blue-500 bg-blue-50' : ''}`}>
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <Checkbox
                      checked={selectedCertificates.includes(certificate.id)}
                      onCheckedChange={(checked) => handleCheckboxChange(checked as boolean, certificate.id)}
                    />
                    <div className="flex items-center space-x-2">
                      {getCertificateTypeIcon(certificate.type || undefined)}
                      <div>
                        <h3 className="font-semibold text-lg text-neutral-900">{certificate.name}</h3>
                        <Badge variant="outline" className="text-xs mt-1">
                          {certificate.status ? certificate.status.charAt(0).toUpperCase() + certificate.status.slice(1) : 'No Type'}
                        </Badge>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-sm text-neutral-500">
                    <p><strong>Created:</strong> {certificate.createdAt ? new Date(certificate.createdAt).toLocaleDateString() : 'N/A'}</p>
                  </div>
              ...
              <div className="flex items-center space-x-3 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2 mb-1">
                    <h3 className="font-medium text-neutral-900 truncate">{certificate.name}</h3>
                    <Badge variant="outline" className="text-xs">
                      {certificate.status ? certificate.status.charAt(0).toUpperCase() + certificate.status.slice(1) : 'No Type'}
                    </Badge>
                  </div>
                  <p className="text-sm text-neutral-600 truncate">
                    {certificate.issuingOrganization && `Issued by: ${certificate.issuingOrganization}`}
                  </p>
                </div>
              </div>
      ...
      {viewMode === "detailed" && (
        <div className="space-y-4">
          {certificates.map((certificate) => (
            <Card key={certificate.id} className={`transition-all duration-200 hover:shadow-lg ${selectedCertificates.includes(certificate.id) ? 'ring-2 ring-blue-500 bg-blue-50' : ''}`}>
              <CardContent className="p-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex items-start space-x-4 mb-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          {getCertificateTypeIcon(certificate.type || undefined)}
                          <h3 className="text-xl font-semibold text-neutral-900">{certificate.name}</h3>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <p className="text-neutral-600">{certificate.createdAt ? new Date(certificate.createdAt).toLocaleDateString() : 'N/A'}</p>
                          </div>
                          <div>
                  ...
                  <div className="flex flex-col space-y-2">
                    ...
                    <Button onClick={() => onDuplicate(certificate)} variant="outline" className="w-full">
                      <Copy className="mr-2 h-4 w-4" />
                      Duplicate
                    </Button>
            ...
            {analytics.expired > 0 && (
              <div className="p-4 rounded-lg border bg-yellow-50 border-yellow-200">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-neutral-900 mb-1">Expired Certificates</h4>
                    <p className="text-neutral-600 mb-2">You have {analytics.expired} expired certificate{analytics.expired === 1 ? '' : 's'} that need renewal.</p>
            ...
            {analytics.expiringSoon > 0 && (
              <div className="p-4 rounded-lg border bg-blue-50 border-blue-200">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-neutral-900 mb-1">Certificates Expiring Soon</h4>
                    <p className="text-neutral-600 mb-2">{analytics.expiringSoon} certificate{analytics.expiringSoon === 1 ? '' : 's'} will expire within the next 3 months.</p>
                    <p className="text-sm font-medium text-neutral-700">
            ...
            {analytics.total > 0 && (analytics.active / analytics.total) * 100 < 80 && (
              <div className="p-4 rounded-lg border bg-yellow-50 border-yellow-200">
                <div className="flex items-start space-x-3">
                  <AlertTriangle className="w-5 h-5 text-yellow-600 mt-0.5" />
                  <div className="flex-1">
                    <h4 className="font-medium text-neutral-900 mb-1">Low Active Certificate Ratio</h4>
                    <p className="text-neutral-600 mb-2">Only {Math.round((analytics.active / analytics.total) * 100)}% of your certificates are currently active.</p>
                    <p className="text-sm font-medium text-neutral-700">
            ...
            {analytics.total === 0 && (
              <div className="p-4 rounded-lg border bg-blue-50 border-blue-200">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-neutral-900 mb-1">No Certificates Found</h4>
                    <p className="text-neutral-600 mb-2">Start building your certification portfolio for better compliance and sustainability.</p>
                    <p className="text-sm font-medium text-neutral-700">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Button onClick={onCreate} className="h-auto p-4 flex-col items-start">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-sm font-medium text-neutral-600">Total Certificates</span>
            </div>
            <div className="text-2xl font-bold text-neutral-900">{analytics.total}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-sm font-medium text-neutral-600">Expired</span>
            </div>
            ...
            {Object.entries(analytics.typeDistribution).map(([type, count]) => (
              <div key={type} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-neutral-600 w-8 text-right">{count as number}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {Object.entries(analytics.issuingBodyDistribution)
              ...
              .slice(0, 5)
              .map(([issuer, count]) => (
                <div key={issuer} className="flex items-center justify-between p-3 bg-neutral-50 rounded-lg">
                  <span className="font-medium">{issuer}</span>
                  <Badge variant="outline">{count as number} certificate{(count as number) === 1 ? '' : 's'}</Badge>
                </div>
              ))}
          </div>
        </CardContent>
...
export default function CertificateManagement() {
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: async ({ ids, updates }: { ids: number[]; updates: any }) => {
      return await Promise.all(
        ids.map(id => apiRequest(`/api/certificates/${id}`, { 
          method: "PUT", 
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    updateCertificateMutation.mutate({ id: editingCertificate.id, data: formData });
  };
  ...
  const handleBulkActivate = () => {
    bulkUpdateMutation.mutate({ ids: selectedCertificates, updates: { isActive: true } });
  };

  const handleBulkDeactivate = () => {
    bulkUpdateMutation.mutate({ ids: selectedCertificates, updates: { isActive: false } });
  };
  ...
  const filteredAndSortedCertificates = useMemo(() => {
    ...
    let filtered = certificates.filter(cert => {
      ...
      if (typeFilter !== "all" && typeFilter !== "" && !cert.type?.toLowerCase().includes(typeFilter.toLowerCase())) return false;
  ...
  const analytics = useMemo(() => {
    ...
    return {
      ...
      mostCommonType: Object.entries(typeDistribution).sort(([,a], [,b]) => b - a)[0]?.[0] || "N/A",
      mostCommonIssuer: Object.entries(issuingBodyDistribution).sort(([,a], [,b]) => b - a)[0]?.[0] || "N/A",
    };
  }, [certificates]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-neutral-600 mt-2">Manage compliance and sustainability certifications</p>
          </div>
          <div className="flex items-center space-x-3">
            <Button 
        ...
        <Tabs value={currentTab} onValueChange={setCurrentTab} className="space-y-6">
                      ...
                      <div>
                        ...
                        <Select value={sortBy} onValueChange={setSortBy}>
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="issuingOrganization">Issuing Organization</SelectItem>
                ...
                {selectedCertificates.length > 0 && (
                  <div className="mt-4 flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <EnhancedDialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
                        <EnhancedDialogContent contentType="form">
                          ...
                          <EnhancedDialogBody>
                            <p className="text-sm text-neutral-600">
                              Are you sure you want to delete {selectedCertificates.length} certificate{selectedCertificates.length === 1 ? '' : 's'}? 
                          ...
                          <EnhancedDialogFooter>
                            ...
                            <Button onClick={() => { handleBulkDelete(); setShowBulkDeleteDialog(false); }} className="bg-red-600 hover:bg-red-700 text-white">
                              Delete
                            </Button>
                          </EnhancedDialogFooter>
          ...
          <TabsContent value="insights" className="space-y-6">
            <CertificateInsights analytics={analytics} onCreate={() => setIsCreateDialogOpen(true)} />
          </TabsContent>
        </Tabs>
        ...
        <EnhancedDialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <EnhancedDialogContent contentType="form">
            <EnhancedDialogHeader>
              <EnhancedDialogTitle className="font-neue-stance">Create New Certificate</EnhancedDialogTitle>
            </EnhancedDialogHeader>
            <EnhancedDialogBody>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-sm text-muted-foreground">Or use URL instead:</div>
                    <Input
                      type="url"
                ...
                <div>
                  ...
                  />
                  <p className="text-sm text-muted-foreground mt-1">Link to the certificate document (PDF, etc.)</p>
                </div>
            ...
            <EnhancedDialogFooter>
              ...
              <Button onClick={(e) => { e.preventDefault(); handleSubmit(e as any); }} disabled={createCertificateMutation.isPending}>
        ...
        <EnhancedDialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <EnhancedDialogContent contentType="form">
            <EnhancedDialogHeader>
              <EnhancedDialogTitle className="font-neue-stance">Edit Certificate</EnhancedDialogTitle>
            </EnhancedDialogHeader>
            <EnhancedDialogBody>
            ...
            <EnhancedDialogFooter>
              ...
              <Button onClick={(e) => { e.preventDefault(); handleEditSubmit(e as any); }} disabled={updateCertificateMutation.isPending}>
