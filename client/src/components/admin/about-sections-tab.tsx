...
export function AboutSectionsTab() {
	...
	const handleEdit = (section: AboutSection) => {
		...
		if (section.mediaIds && section.mediaIds.length > 0) {
			...
			const missingIds = section.mediaIds.filter(
				(id) => !media.find((m) => m.id === id),
			);
			...
			if (missingIds.length > 0) {
				...
				Promise.all(
					...
					const reconciled = section.mediaIds!.filter((id) =>
						validIds.includes(id),
					);
					const orderedMedia = reconciled.map(
						(id) => allMedia.find((m) => m.id === id)!,
					);
				...
				<CardHeader>
					<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
						<div>
							...
							<CardDescription>
								Manage manufacturing process, factory gallery, and custom
								sections
							</CardDescription>
						</div>
						<Button
