...
export function ProductAdvancedFilters({
  ...
  const {
    ...
    isSearching,
    // clearSearch
  } = useDebouncedSearch(products, ['name', 'sku', 'description', 'shortDescription', 'tags'], 300);
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    const filtered = searchFilteredProducts.filter(product => {
      ...
      const matchesFabric = fabricFilter === 'all' ||
        product.fabricId?.toString() === fabricFilter;
    ...
    return filtered;
  }, [products, categories, fabrics, searchQuery, statusFilter, categoryFilter,
    fabricFilter, featuredFilter, sortBy, sortOrder]);

  // Update filtered products when filters change
  React.useEffect(() => {
    onFilteredProductsChange(filteredAndSortedProducts);
  }, [searchFilteredProducts, statusFilter, categoryFilter, fabricFilter, featuredFilter, sortBy, sortOrder, onFilteredProductsChange]);
  ...
  const hasActiveFilters = searchQuery || statusFilter !== 'all' ||
    categoryFilter !== 'all' || fabricFilter !== 'all' ||
    featuredFilter !== 'all' || sortBy !== 'name' || sortOrder !== 'asc';
        ...
        <div className="relative">
          ...
          {isSearching && (
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600" aria-label="Searching..."></div>
            </div>
          )}
        </div>
        ...
        {hasActiveFilters && (
          <div className="flex items-center space-x-2">
            ...
            <div className="flex space-x-1">
              {searchQuery && <Badge variant="outline" className="text-xs">Search</Badge>}
              {statusFilter !== 'all' && <Badge variant="outline" className="text-xs">Status</Badge>}
              {categoryFilter !== 'all' && <Badge variant="outline" className="text-xs">Category</Badge>}
              {fabricFilter !== 'all' && <Badge variant="outline" className="text-xs">Fabric</Badge>}
              {featuredFilter !== 'all' && <Badge variant="outline" className="text-xs">Featured</Badge>}
              {(sortBy !== 'name' || sortOrder !== 'asc') && <Badge variant="outline" className="text-xs">Sort</Badge>}
            </div>
          </div>
        )}
      </div>
