...
export function ProductBulkOperations({ 
  ...
  const bulkOperationMutation = useMutation({
    mutationFn: async ({ action, productIds, data }: { 
      action: string; 
      productIds: number[]; 
    ...
    onError: (error: Error | unknown, variables) => {
      console.error(`❌ Bulk ${variables.action} error:`, error);
      const errorMessage = error instanceof Error ? error.message : `Failed to perform bulk ${variables.action}`;
      toast({
        title: "Error",
  ...
  const getConfirmationMessage = () => {
    ...
    return { title: '', description: '', actionText: '', variant: 'default' as const };
  };
