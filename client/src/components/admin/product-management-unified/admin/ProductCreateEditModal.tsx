...
export function ProductCreateEditModal({ product, isOpen, onClose }: ProductCreateEditModalProps) {
  ...
  const invalidateProductCaches = (productData: { urlPath?: string | null; slug?: string | null }, originalUrlPath?: string | null) => {
    ...
    if (productData.urlPath) {
      queryClient.invalidateQueries({ queryKey: ['product-complete', productData.urlPath] });
    }
    if (productData.slug) {
      queryClient.invalidateQueries({ queryKey: ['product-complete', productData.slug] });
    }
    
    // For updates, also invalidate the original path if it changed
    if (originalUrlPath && originalUrlPath !== productData.urlPath) {
      queryClient.invalidateQueries({ queryKey: ['product-complete', originalUrlPath] });
    }
  ...
  const createProductMutation = useMutation({
    ...
    onSuccess: (newProduct) => {
      try {
        logger.debug('Product created successfully', { productId: newProduct.id, name: newProduct.name });
  ...
  const updateProductMutation = useMutation({
    ...
    onSuccess: (updatedProduct) => {
      try {
        logger.debug('Product updated successfully', { productId: updatedProduct.id, name: updatedProduct.name });
  ...
  const { data: productsResponse } = useQuery<{ data: Product[], pagination?: PaginationMeta }>({
  ...
  const { data: mediaAssets = [] } = useQuery<MediaAsset[]>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/media?${buildMediaApiParams(ADMIN_MEDIA_QUERIES.MAX_ASSETS)}`);
      if (!response.ok) {
    ...
    select: (data: MediaApiResponse): MediaAsset[] => {
      ...
      console.warn('Unexpected media API response structure:', data);
      return [];
    },
  ...
  const validateField = (field: string, value: unknown): string => {
    switch (field) {
      ...
      case 'leadTime':
        if (typeof value === 'string' && value && value.trim().length > 0 && value.trim().length < 3) {
  ...
  const validateFormPure = useCallback((data: typeof formData) => {
    ...
    const fieldsToValidate = [
      'name', 'sku', 'description', 'categoryId', 'fabricId', 
      'selectedFiberComposition', 'primaryImageId', 'minimumOrderQuantity', 'leadTime'
    ];
    ...
    if (Object.keys(data.technicalSpecs || {}).length === 0) {
      errors['technicalSpecs'] = 'Technical specifications help B2B customers make informed decisions';
      warningCount++;
    }
  ...
  }, [isOpen, isLoading, isEditing, onClose]);
    ...
    Object.keys(sections).forEach(sectionKey => {
      ...
      section.completed = section.fields.reduce((count: number, field: string) => {
        ...
        if (field === 'specifications' && Array.isArray(value)) {
          return count + (value.length > 0 && value.some((spec: unknown) => 
            typeof spec === 'object' && spec !== null && 
            'name' in spec && 'value' in spec && 
  ...
  const progressData = useMemo(() => calculateFormProgress(), [formData]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const productData: InsertProduct = {
      ...
      fiberComposition: formData.selectedFiberComposition && formData.selectedFiberComposition.length > 0
      ...
      videos: formData.videos.length > 0 ? formData.videos.map(id => ({ id, type: 'video' })) : undefined,
      modelFileId: formData.modelFileId,
      
      // Specifications & Features
      specifications: formData.specifications.length > 0 ? formData.specifications : undefined,
      technicalSpecs: Object.keys(formData.technicalSpecs || {}).length > 0 ? formData.technicalSpecs : undefined,
      tags: formData.tags.length > 0 ? formData.tags : undefined,
      careInstructions: formData.careInstructions.length > 0 ? formData.careInstructions : undefined,
      
      // B2B Details
      minimumOrderQuantity: formData.minimumOrderQuantity ? parseInt(formData.minimumOrderQuantity, 10) : undefined,
      leadTime: formData.leadTime || undefined,
      customWeight: formData.customWeight || undefined,
      customFit: formData.customFit || undefined,
      
      // Customization
      customizationOptions: formData.customizationOptions && formData.customizationOptions.length > 0 
      ...
      relatedProductIds: formData.relatedProductIds && formData.relatedProductIds.length > 0 ? formData.relatedProductIds : undefined,
      ...
      >
        ...
        <EnhancedDialogHeader>
          ...
          <EnhancedDialogDescription id="product-form-description">
            {isEditing ? 'Update product information and media assets' : 'Add a new product to your catalog with category, fabric, and media selections'}
          </EnhancedDialogDescription>
        </EnhancedDialogHeader>
            ...
            {Object.entries(progressData.sections).map(([key, section]) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-medium text-gray-700 mb-1">{typedSection.name}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Suspense fallback={<div className="p-4 border rounded-lg animate-pulse bg-gray-50">Loading...</div>}>
            <BasicInfoSection
              formData={{
                name: formData.name,
                sku: formData.sku,
          ...
          <Suspense fallback={<div className="p-4 border rounded-lg animate-pulse bg-gray-50">Loading...</div>}>
            <CategoryFabricSection
              formData={{
                ...
                fabricId: formData.fabricId,
                sizeChartId: formData.sizeChartId,
                selectedFiberComposition: Array.isArray(formData.selectedFiberComposition) ? formData.selectedFiberComposition.join(', ') : '',
              }}
              formErrors={formErrors}
          ...
          <Suspense fallback={<div className="p-4 border rounded-lg animate-pulse bg-gray-50">Loading...</div>}>
            <CertificationsSection
              ...
              products={Array.isArray(allProducts) ? allProducts.filter((p: Product) => p.id !== product?.id) : []}
            />
          </Suspense>
            ...
            {!validationSummary.isValid && (
              <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <span className="text-sm text-red-700">
                  Fix {validationSummary.errorCount} error{validationSummary.errorCount !== 1 ? 's' : ''} 
                  {validationSummary.warningCount > 0 && ` and ${validationSummary.warningCount} warning${validationSummary.warningCount !== 1 ? 's' : ''}`} before submitting
                </span>
              </div>
            )}
            
            <div className="flex justify-end gap-3">
              <Button data-testid="product-form-cancel-button" type="button" variant="outline" onClick={onClose}>
          ...
          <StandardMediaSelectionDialog
            ...
            title={
              mediaPickerMode === 'primary' ? 'Select Primary Image' :
              mediaPickerMode === 'images' ? 'Select Product Images' :
              mediaPickerMode === 'videos' ? 'Select Product Videos' :
              mediaPickerMode === 'model' ? 'Select 3D Model' :
              'Select Media'
            }
