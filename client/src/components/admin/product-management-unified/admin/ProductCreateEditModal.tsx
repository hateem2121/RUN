...
export function ProductCreateEditModal({
	product,
	isOpen,
	onClose,
}: ProductCreateEditModalProps) {
	const isEditing = !!product;

	// Phase 2: Use optimized form management with useReducer
	const { formData, updateField, updateMultipleFields } =
	...
	const [mediaPickerMode] = useState<"images" | "primary" | "videos" | "model">(
		"images",
	);
	...
	const validateFormPure = useCallback((data: typeof formData) => {
		...
		fieldsToValidate.forEach((field) => {
			...
			if (error) {
				errors[field] = error;
				if (
					["name", "sku", "categoryId", "fabricId", "primaryImageId"].includes(
						field,
					)
				) {
					errorCount++;
		...
		if (data.certificateIds.length === 0) {
			errors["certificates"] =
				"Consider adding sustainability certificates for B2B credibility";
			warningCount++;
		}
	...
	const calculateFormProgress = useCallback(() => {
		...
		Object.keys(sections).forEach((sectionKey) => {
			...
			section.completed = section.fields.reduce(
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const productData: InsertProduct = {
			...
			fiberComposition:
				...
				formData.selectedFiberComposition.length > 0
			...
			certificateIds:
				formData.certificateIds.length > 0
					? formData.certificateIds
					: undefined,
			accessoryIds:
				formData.accessoryIds.length > 0 ? formData.accessoryIds : undefined,
			relatedProductIds:
	...
	const handleInputChange = useCallback(
		(field: string, value: unknown) => {
			updateField(
				...
				value as ProductFormFieldValue,
			);
	...
	const handleMediaSelection = (value: number | number[]) => {
		...
		if (mediaPickerMode === "images") {
			...
			updateField(
				"primaryImageId",
				(selectedIds.length > 0 ? selectedIds[0] : null) ?? null,
			);
	...
	const normalizeSpecifications = (
		specs: string[] | Record<string, any> | undefined,
	): string[] => {
		if (!specs) return [];
		...
		if (Array.isArray(specs)) {
			...
			specsArray = Object.values(specs).map((v) =>
						...
						<div className="grid grid-cols-2 md:grid-cols-3 gap-2">
							{Object.entries(progressData.sections).map(([key, section]) => {
								...
								const percentage = Math.round(
									(typedSection.completed / typedSection.total) * 100,
								);
						...
						>
							<CategoryFabricSection
								formData={{
									...
									fabricId: formData.fabricId,
									sizeChartId: formData.sizeChartId,
									selectedFiberComposition: Array.isArray(
										formData.selectedFiberComposition,
									)
