...
export function useSmartValidation(config: Partial<ValidationConfig> = {}) {
  ...
  const validateCriticalField = useCallback((field: string, value: ProductFormFieldValue): string | null => {
    // Basic validation from unified system (sync version)
    switch (field) {
      case 'name':
        if (typeof value !== 'string' || !value || value.trim().length === 0) return 'Product name is required';
      ...
      case 'sku':
        if (typeof value !== 'string' || !value || value.trim().length === 0) return 'SKU is required';
        if (!/^[A-Z0-9_-]+$/i.test(value)) return 'SKU can only contain letters, numbers, hyphens and underscores';
  ...
  const validateNonCriticalField = useCallback((field: string, value: ProductFormFieldValue): string | null => {
    switch (field) {
      case 'description':
        if (typeof value === 'string' && value.length > 5000) return 'Description must be less than 5000 characters';
        return null;
      
      case 'minimumOrderQuantity':
        if (typeof value === 'string' && value && (isNaN(Number(value)) || parseInt(value) < 1)) return 'MOQ must be a positive number';
        return null;
      
      case 'leadTime':
        if (typeof value === 'string' && value.length > 100) return 'Lead time must be less than 100 characters';
        return null;
      
      case 'metaTitle':
        if (typeof value === 'string' && value.length > 60) return 'Meta title should be less than 60 characters for SEO';
        return null;
      
      case 'metaDescription':
        if (typeof value === 'string' && value.length > 160) return 'Meta description should be less than 160 characters for SEO';
        return null;
  ...
  const validateField = useCallback((field: string, value: ProductFormFieldValue, immediate = false): string | null | Promise<string | null> => {
    // Check cache first
  ...
  const validateForm = useCallback(async (formData: Record<string, ProductFormFieldValue>): Promise<ValidationResult> => {
