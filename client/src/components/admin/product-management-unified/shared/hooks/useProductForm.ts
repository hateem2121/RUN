...
type ProductFormAction =
  | { type: 'SET_FIELD'; field: keyof ProductFormState; value: ProductFormFieldValue }
  | { type: 'SET_MULTIPLE_FIELDS'; fields: Partial<ProductFormState> }
  | { type: 'RESET_FORM' }
  | { type: 'LOAD_PRODUCT'; product: Product }
  | { type: 'GENERATE_SLUG'; name: string }
  | { type: 'ADD_TO_ARRAY'; field: keyof ProductFormState; value: ProductFormFieldValue }
  ...
  switch (action.type) {
    ...
    case 'LOAD_PRODUCT': {
      const product = action.product as Product & {
        ...
        technicalSpecs?: Record<string, string>;
        careInstructions?: string[];
        customizationOptions?: Array<{ name?: string; value?: string; [key: string]: unknown }>;
        relatedProductIds?: number[];
      };
      ...
      return {
        ...
        videos: Array.isArray(product.videos) 
          ? product.videos.map(v => typeof v === 'number' ? v : (typeof v === 'object' && v?.id ? v.id : 0)).filter(id => id !== 0)
          : [],
...
export function useProductForm(product?: Product | null) {
  ...
  const updateField = useCallback((field: keyof ProductFormState, value: ProductFormFieldValue) => {
  ...
  const addToArray = useCallback((field: keyof ProductFormState, value: ProductFormFieldValue) => {
