...
export function ProductDetailsPanel({ product, onClose, onEdit }: ProductDetailsPanelProps) {
  ...
  const allImages = Array.isArray(product.imageIds) ? (product.imageIds as unknown as number[]).map((id: number) => getMediaAsset(id)).filter(Boolean) : [];
  const allVideos = Array.isArray(product.videos) ? (product.videos as unknown as number[]).map((id: number) => getMediaAsset(id)).filter(Boolean) : [];
  ...
  return (
    <div className="space-y-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden">
              <img
                src={primaryImage.id && primaryImage.id < 1000000000000 ? `/api/media/${primaryImage.id}/content` : undefined}
                alt={product.name}
      ...
      {(product.minimumOrderQuantity || product.leadTime || product.customizationOptions?.length) && (
        <Card>
          <CardHeader>
