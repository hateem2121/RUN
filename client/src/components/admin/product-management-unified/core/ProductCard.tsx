...
export const ProductCard = memo(function ProductCard({
  ...
  const invalidateProductCaches = () => {
    ...
    if (product.slug) {
      queryClient.invalidateQueries({ queryKey: ['product-complete', product.slug] });
    }
  ...
  // Get primary media
  const primaryImage = product.primaryImageId ? getMediaAsset(product.primaryImageId) :
    (product.imageIds && product.imageIds.length > 0 && product.imageIds[0] !== undefined ? getMediaAsset(product.imageIds[0]) : null);
  ...
  if (viewMode === 'list') {
    return (
      <>
        <Card
          className={`transition-all duration-200 hover:shadow-md cursor-pointer ${isHovered ? 'border-blue-300 shadow-lg' : ''
            }`}
              ...
              <div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center shrink-0 overflow-hidden">
                ...
                {model3D && (
                  <Button
                    ...
                    size="sm"
                    variant="ghost"
                    className="absolute top-0 right-0 h-6 w-6 p-0 bg-white/90 hover:bg-white"
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  >
                    {show3DPreview ? <ImageIcon className="h-3 w-3" /> : <Box className="h-3 w-3" />}
                  </Button>
                )}
              </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button data-testid={`product-actions-menu-${product.id}`} variant="ghost" size="sm">
                          <MoreHorizontal className="h-4 w-4" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem data-testid={`view-product-${product.id}`} onClick={(e) => { e.stopPropagation(); onSelect?.(); }}>
                          <Eye className="h-4 w-4 mr-2" />
                          View Details
                        </DropdownMenuItem>
                        <DropdownMenuItem data-testid={`edit-product-${product.id}`} onClick={(e) => { e.stopPropagation(); onEdit?.(); }}>
  ...
  return (
    <>
      <Card
        className={`transition-all duration-200 hover:shadow-lg cursor-pointer relative overflow-hidden ${isHovered ? 'border-blue-300 shadow-xl scale-[1.02]' : ''
          }`}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        onClick={onSelect}
        ...
        <div className="aspect-[4/3] bg-gray-100 relative overflow-hidden">
          {show3DPreview && model3D ? (
            <UnifiedModelViewer
              ...
              showFileInfo={false}
              onError={(error) => {
                console.error('3D model error in ProductCard:', error);
          ...
          <div className="absolute top-1.5 right-1.5 flex gap-1">
            ...
            {model3D && (
              <Badge
                ...
                className={`text-xs cursor-pointer transition-all ${show3DPreview
