...
export function RelationshipIndicators({
	...
	if (visibleIndicators.length === 0) {
		return <div className="text-xs text-gray-400 italic">No relationships</div>;
	}
			...
			<div className="flex flex-wrap gap-1">
				{visibleIndicators.slice(0, 4).map((indicator, index) => {
					...
					return (
						<div
							key={index}
							className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs ${indicator.color}`}
							title={`${indicator.count} ${indicator.label}`}
						>
							<Icon className="h-3 w-3" />
							{indicator.count > 1 && (
								<span className="ml-1 font-medium">{indicator.count}</span>
							)}
						</div>
					);
				})}
