...
export function TechnologyResearchManagement({
  ...
  const updateResearchMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<ResearchFormData> }) =>
      apiRequest(`/api/technology-research/${id}`, { method: "PATCH", body: data }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/technology-research"] });
      setShowResearchDialog(false);
      setEditingResearch(null);
  ...
  const reorderResearchMutation = useMutation({
    mutationFn: (research: { id: number; position: number }[]) =>
      apiRequest("/api/technology-research/reorder", { method: "PATCH", body: { research } }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/technology-research"] });
    },
  });
    ...
    <Card>
      ...
      <EnhancedDialog open={showResearchDialog} onOpenChange={setShowResearchDialog}>
        <EnhancedDialogContent contentType="form">
          ...
          <form onSubmit={handleResearchSubmit} className="flex flex-col flex-1 min-h-0">
            <EnhancedDialogBody className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setResearchForm({ ...researchForm, funding: parseFloat(e.target.value) || 0 })
                  }
                  placeholder="0.00"
                />
              </div>
