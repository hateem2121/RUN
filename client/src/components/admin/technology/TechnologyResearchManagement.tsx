...
export function TechnologyResearchManagement({
	...
	const [editingResearch, setEditingResearch] =
		useState<TechnologyResearch | null>(null);
	...
	const { data: research = [], isPending: researchLoading } = useQuery<
		TechnologyResearch[]
	>({
	...
	const deleteResearchMutation = useMutation({
		mutationFn: (id: number) =>
			apiRequest(`/api/technology-research/${id}`, { method: "DELETE" }),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["/api/technology-research"] });
			toast({
				title: "Success",
	...
	const handleRemoveProject = (index: number) => {
		setResearchForm({
			...researchForm,
			currentProjects: researchForm.currentProjects.filter(
				(_, i) => i !== index,
			),
		});
	};
			...
			<CardHeader>
				<div className="flex justify-between items-center">
					<div>
						...
						<CardDescription>
							Manage your R&D projects and initiatives
						</CardDescription>
					</div>
					<Button
						onClick={() => {
			...
			<CardContent>
				{loading ? (
					...
					<div className="text-center py-8 text-gray-500">
						No research items added yet. Click "Add Research Item" to showcase
						your R&D efforts.
					</div>
				) : (
					<DndContext
			...
			>
				<EnhancedDialogContent contentType="form">
					<EnhancedDialogHeader>
						...
						<EnhancedDialogDescription>
							{editingResearch
								? "Update the research item details"
						...
						<EnhancedDialogBody className="space-y-4">
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setResearchForm({ ...researchForm, title: e.target.value })
									}
							...
							<div className="grid grid-cols-2 gap-4">
								...
								<div>
									...
									<Select
										...
										onValueChange={(
											value: "Ongoing" | "Completed" | "Planned",
									...
									<Input
										...
										value={
											researchForm.startDate
												? new Date(researchForm.startDate)
														.toISOString()
														.split("T")[0]
												: ""
										}
									...
									<Input
										...
										value={
											researchForm.expectedCompletion
												? new Date(researchForm.expectedCompletion)
														.toISOString()
														.split("T")[0]
												: ""
										}
							...
							<div>
								...
								<Select
									...
									onValueChange={(value) =>
										setResearchForm({ ...researchForm, icon: value })
									}
								>
									<SelectTrigger>
										<SelectValue />
