...
function SortableEquipmentItem({
	...
	return (
		<div
			ref={setNodeRef}
			style={style}
			className="bg-white rounded-lg border p-4 mb-3 shadow-sm-xs"
		>
...
export function TechnologyEquipmentManagement({
	...
	const [editingEquipment, setEditingEquipment] =
		useState<TechnologyEquipment | null>(null);
	...
	const deleteEquipmentMutation = useMutation({
		mutationFn: (id: number) =>
			apiRequest(`/api/technology-equipment/${id}`, { method: "DELETE" }),
		onSuccess: () => {
			getQueryClient().invalidateQueries({
				queryKey: ["/api/technology-equipment"],
			});
	...
	const handleRemoveCertification = (index: number) => {
		setEquipmentForm({
			...equipmentForm,
			certifications: equipmentForm.certifications.filter(
				(_, i) => i !== index,
			),
		});
	};
				...
				<CardHeader>
					<div className="flex justify-between items-center">
						<div>
							...
							<CardDescription>
								Manage your advanced manufacturing equipment
							</CardDescription>
						</div>
						<Button
							onClick={() => {
				...
				<CardContent>
					{equipmentLoading || isLoading ? (
						...
						<div className="text-center py-8 text-gray-500">
							No equipment added yet. Click "Add Equipment" to showcase your
			...
			>
				<EnhancedDialogContent contentType="form">
					<EnhancedDialogHeader>
						...
						<EnhancedDialogDescription>
							Showcase your advanced manufacturing technology and equipment
							capabilities.
						</EnhancedDialogDescription>
					</EnhancedDialogHeader>
					<form
								...
								<Input
									...
									value={
										equipmentForm.installationDate
											? new Date(equipmentForm.installationDate)
													.toISOString()
													.split("T")[0]
											: ""
									}
							...
							<div>
								...
								{Object.keys(equipmentForm.specifications).length > 0 && (
									<div className="mt-2 space-y-1">
										{Object.entries(equipmentForm.specifications).map(
											([key, value]) => (
												<div
													key={key}
								...
								{equipmentForm.imageId && (
									<div className="mt-2 text-sm text-gray-600">
										...
										<Button
											...
											onClick={() =>
												setEquipmentForm({ ...equipmentForm, imageId: null })
											}
										>
											Remove
										</Button>
