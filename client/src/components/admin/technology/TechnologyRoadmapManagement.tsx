...
export function TechnologyRoadmapManagement({
	...
	const [editingRoadmap, setEditingRoadmap] =
		useState<TechnologyRoadmap | null>(null);
	...
	const { data: roadmap = [], isPending: roadmapLoading } = useQuery<
		TechnologyRoadmap[]
	>({
			...
			<CardContent>
				{loading ? (
					...
					<div className="text-center py-8 text-gray-500">
						No roadmap milestones added yet. Click "Add Roadmap Milestone" to
						showcase your technology development plans.
					</div>
				) : (
					<div className="space-y-4">
						{roadmap.map((item) => (
							<div
								key={item.id}
								className="bg-white border rounded-lg p-4 shadow-sm-xs"
							>
										...
										{roadmapForm.imageId && (
											<div className="mt-2 text-sm text-gray-600 flex items-center justify-between bg-gray-50 p-2 rounded">
												...
												<Button
													...
													onClick={() =>
														setRoadmapForm({ ...roadmapForm, imageId: null })
													}
												>
													<X className="w-4 h-4" />
												</Button>
										...
										{roadmapForm.videoId && (
											<div className="mt-2 text-sm text-gray-600 flex items-center justify-between bg-gray-50 p-2 rounded">
												...
												<Button
													...
													className="h-6 w-6 p-0 text-red-600"
													onClick={() =>
														setRoadmapForm({ ...roadmapForm, videoId: null })
													}
												>
													<X className="w-4 h-4" />
												</Button>
