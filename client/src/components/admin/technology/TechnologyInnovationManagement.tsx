...
export function TechnologyInnovationManagement({
	...
	const [editingInnovation, setEditingInnovation] =
		useState<TechnologyInnovation | null>(null);
	...
	const [showInnovationImagePicker, setShowInnovationImagePicker] =
		useState(false);
	...
	const updateInnovationMutation = useMutation({
		mutationFn: ({
			...
			id: number;
			data: Partial<InnovationFormData>;
		}) =>
	...
	const handleRemoveRelatedProduct = (index: number) => {
		setInnovationForm({
			...innovationForm,
			relatedProducts: innovationForm.relatedProducts.filter(
				(_, i) => i !== index,
			),
		});
	};
	...
	return (
		<>
				...
				<CardContent>
					{innovationsLoading || isLoading ? (
						...
						<div className="text-center py-8 text-gray-500">
							No innovations added yet. Click "Add Innovation" to showcase your
							first technology innovation.
						</div>
					) : (
						<DndContext
							sensors={sensors}
			...
			{/* Innovation Dialog */}
			<EnhancedDialog
				open={showInnovationDialog}
				onOpenChange={setShowInnovationDialog}
			>
					...
					<form onSubmit={handleInnovationSubmit} className="contents">
							...
							<div>
								...
								{Object.keys(innovationForm.technicalDetails).length > 0 && (
									<div className="mt-2 space-y-1">
										{Object.entries(innovationForm.technicalDetails).map(
											([key, value]) => (
												<div
													key={key}
						...
						<EnhancedDialogFooter>
							...
							<Button
								type="submit"
								disabled={
									createInnovationMutation.isPending ||
									updateInnovationMutation.isPending
								}
							>
								{createInnovationMutation.isPending ||
								updateInnovationMutation.isPending
