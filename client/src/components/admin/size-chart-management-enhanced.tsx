...
const validateMeasurements = (measurements: Record<string, any>) => {
	...
	const completeness =
		total > 0 ? Math.round(((total - incomplete) / total) * 100) : 0;
		...
		<div className="space-y-4 max-h-96 overflow-y-auto">
			{sizeCharts.map((chart) => {
				...
				return (
					<div
						key={chart.id}
						className="group p-4 border border-neutral-200 rounded-lg hover:border-blue-300 hover:shadow-md transition-all duration-200"
					>
								...
								{validation.total > 0 && (
									<div className="mb-2">
										...
										<p className="text-xs text-neutral-500 mt-1">
											{validation.total - validation.incomplete}/
											{validation.total} measurements complete
										</p>
									</div>
								)}
							</div>
...
export default function SizeChartManagementEnhanced() {
	...
	const loadTemplate = (chartType: string, region: string) => {
		...
		const measurements =
			commonMeasurements[normalizedType as keyof typeof commonMeasurements];
	...
	const renderMeasurementTable = (measurements: Record<string, any>) => {
		...
		const measurementTypes =
			firstSize &&
			measurements[firstSize] &&
			typeof measurements[firstSize] === "object"
			...
			<div className="flex items-center justify-between mb-8">
				<div>
					...
					<p className="text-neutral-600 mt-2">
						Create and manage size charts for different regions and product
						categories
					</p>
				</div>
							...
							<form onSubmit={handleSubmit} className="space-y-6">
								...
								<div className="pt-4 border-t">
									...
									<p className="text-sm text-neutral-600 mb-3">
										Enter measurements in centimeters (cm). For size numbers
										(e.g., EU Size, UK Size), enter the numeric value only.
									</p>
									<div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
										<div>
											<Label
												htmlFor="measurement-size"
									...
									{Object.keys(formData.measurements).length > 0 && (
										<div className="space-y-3">
											{Object.entries(formData.measurements).map(
												([size, measurements]) => (
													<div
														key={size}
							...
							<div className="space-y-4">
								<div className="flex items-center gap-4">
									...
									<Badge variant="outline">
										{previewChart.type
											? previewChart.type.charAt(0).toUpperCase() +
												previewChart.type.slice(1)
											: "Unknown"}
									</Badge>
