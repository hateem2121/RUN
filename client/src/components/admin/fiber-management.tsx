...
export default function FiberManagement() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "type" | "created" | "status" | "sustainability">(
    "name",
  );
  ...
  const deleteFiberMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest(`/api/fibers/${id}`, { method: "DELETE" });
      return response;
    },
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: async ({
      ...
      const promises = ids.map((id) =>
        apiRequest(`/api/fibers/${id}`, { method: "PATCH", body: JSON.stringify(updates) }),
      );
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: number[]) => {
      const promises = ids.map((id) => apiRequest(`/api/fibers/${id}`, { method: "DELETE" }));
      return Promise.all(promises);
    },
    onSuccess: (_, ids) => {
      getQueryClient().invalidateQueries({ queryKey: ["/api/fibers"] });
    ...
    const properties =
      typeof fiber.properties === "object" && fiber.properties
        ? Object.entries(fiber.properties)
            ...
            .join(", ")
        : (fiber.properties ?? "");
    const propertiesArray = properties ? properties.split(", ").filter((p) => p.trim()) : [];
  ...
  // Sorting handlers
  const handleSort = (field: "name" | "type" | "created" | "status" | "sustainability") => {
    if (sortBy === field) {
    ...
    fibers
      ?.filter((fiber) => {
        ...
        const matchesSustainability =
          ...
          (filterSustainability === "high" && (fiber.sustainabilityScore || 0) >= 4) ||
          (filterSustainability === "moderate" && (fiber.sustainabilityScore || 0) === 3) ||
        ...
        return matchesSearch && matchesType && matchesStatus && matchesSustainability;
      })
      .sort((a, b) => {
        let comparison = 0;
        ...
        switch (sortBy) {
          ...
          case "status":
            ...
            break;
          case "sustainability":
            comparison = (a.sustainabilityScore || 0) - (b.sustainabilityScore || 0);
            break;
        }
  ...
  const sustainabilityStats = useMemo(() => {
    ...
    const averageScore =
      ratedCount > 0
        ? ratedFibers.reduce((sum, f) => sum + (f.sustainabilityScore || 0), 0) / ratedCount
        : 0;

    const scoreDistribution = {
      high: ratedFibers.filter((f) => (f.sustainabilityScore || 0) >= 4).length,
      moderate: ratedFibers.filter((f) => (f.sustainabilityScore || 0) === 3).length,
      low: ratedFibers.filter(
        (f) => (f.sustainabilityScore || 0) >= 1 && (f.sustainabilityScore || 0) <= 2,
      ).length,
    };
    ...
    const fibersWithNotes = fibers.filter((f) => f.environmentalImpact?.trim()).length;
      ...
      <div>
        ...
        <div className="flex gap-2 mb-3">
          ...
          <Button
            type="button"
            onClick={addProperty}
            disabled={!newProperty.trim() || propertyList.includes(newProperty.trim())}
            size="sm"
          >
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-white dark:bg-green-900 p-4 rounded-lg border border-green-200 dark:border-green-700">
                ...
                <div className="text-sm text-green-600 dark:text-green-400">Total Fibers</div>
              </div>
              ...
              <div className="bg-white dark:bg-green-900 p-4 rounded-lg border border-green-200 dark:border-green-700">
                ...
                <div className="text-sm text-green-600 dark:text-green-400">With Impact Notes</div>
              </div>
            </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => handleSort("created")}>
                Sort by Created {sortBy === "created" && (sortOrder === "asc" ? "↑" : "↓")}
              </DropdownMenuItem>
              ...
              <DropdownMenuItem onClick={() => handleSort("sustainability")}>
                ...
                {sortBy === "sustainability" && (sortOrder === "asc" ? "↑" : "↓")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        {selectedFibers.size > 0 && (
          <div className="flex items-center gap-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button size="sm" variant="ghost" onClick={() => setSelectedFibers(new Set())}>
                Clear Selection
              </Button>
            </div>
          </div>
      ...
      <EnhancedDialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        <Label className="text-sm font-medium text-gray-600">Fiber Name</Label>
                      ...
                      {detailFiber.createdAt && (
                        <div>
                          ...
                          <p className="text-base">
                            {new Date(detailFiber.createdAt).toLocaleDateString("en-US", {
                              year: "numeric",
                              month: "long",
                              day: "numeric",
                              hour: "2-digit",
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div>
                        <Label className="text-sm font-medium text-gray-600">Used in Fabrics</Label>
                        <p className="text-2xl font-bold text-green-600">0</p>
                        <p className="text-xs text-gray-500">No fabric usage tracking yet</p>
                      </div>
                      <div>
                        <Label className="text-sm font-medium text-gray-600">Usage Score</Label>
                        <div className="flex items-center gap-2">
                ...
                {detailFiber.description && (
                  <Card>
                    ...
                    <CardContent>
                      <p className="text-gray-700 leading-relaxed">{detailFiber.description}</p>
                    </CardContent>
                  </Card>
                )}
                      ...
                      <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {getPropertiesArray(detailFiber.properties).map((prop: string) => (
                            <div key={prop} className="p-3 bg-gray-50 rounded-lg border">
                              <div className="font-medium text-gray-900 text-sm">{prop}</div>
                            </div>
                          ))}
                        </div>
                      </CardContent>
                              ...
                              {[1, 2, 3, 4, 5].map((score) => (
                                <Star
                                  ...
                                  className={`h-5 w-5 fill-current ${
                                    (detailFiber.sustainabilityScore || 0) >= score
                                      ? "text-yellow-500"
                                      : "text-gray-300"
                                  }`}
                                />
      ...
      <EnhancedDialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <EnhancedDialogContent contentType="form">
          <EnhancedDialogHeader>
            <EnhancedDialogTitle className="font-neue-stance">Add New Fiber</EnhancedDialogTitle>
            <EnhancedDialogDescription>
              Create a new fiber type for material traceability within fabrics
            </EnhancedDialogDescription>
          </EnhancedDialogHeader>
          <EnhancedDialogBody>{renderFiberForm(handleSubmit)}</EnhancedDialogBody>
          <EnhancedDialogFooter>
            <Button
              type="button"
              variant="outline"
      ...
      <EnhancedDialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <EnhancedDialogContent contentType="form">
          <EnhancedDialogHeader>
            <EnhancedDialogTitle className="font-neue-stance">Edit Fiber</EnhancedDialogTitle>
            <EnhancedDialogDescription>
              Update fiber information and properties
            </EnhancedDialogDescription>
          </EnhancedDialogHeader>
          <EnhancedDialogBody>{renderFiberForm(handleEditSubmit)}</EnhancedDialogBody>
          <EnhancedDialogFooter>
            <Button
              type="button"
              variant="outline"
