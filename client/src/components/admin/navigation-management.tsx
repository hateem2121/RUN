...
export default function NavigationManagement() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: ["/api/navigation-items"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      apiRequest(`/api/navigation-items/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: ["/api/navigation-items"] });
        ...
        <div className="mb-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Drag and drop to reorder items. Active items appear in the
