...
export const IconSelector = React.memo(function IconSelector({
	...
	const imageAssets = mediaAssets.filter(
		(asset: MediaAsset) => asset.type === "image",
	);
	...
	const getCurrentIconDisplay = () => {
		...
		if (typeof value === "number") {
			const mediaAsset = imageAssets.find(
				(asset: MediaAsset) => asset.id === value,
			);
			if (mediaAsset) {
				return (
					<div className="flex items-center gap-2">
					...
					<EnhancedDialogContent contentType="form" preferredSize="2xl">
						<EnhancedDialogHeader>
							...
							<p className="text-sm text-muted-foreground mt-1">
								Choose from predefined icons or upload a custom image to
						...
						<div className="flex-1 overflow-hidden">
							...
							>
								<TabsList
									className={`grid w-full ${allowCustom ? "grid-cols-2" : "grid-cols-1"}`}
								>
