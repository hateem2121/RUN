...
export const AdvancedOptionsTab: React.FC<AdvancedOptionsTabProps> = ({
	...
	const removeCareInstruction = (index: number) => {
		setFormData((prev) => ({
			...prev,
			careInstructions: (prev.careInstructions || []).filter(
				(_, i) => i !== index,
			),
		}));
	};
					...
					<div>
						...
						<div className="grid grid-cols-5 gap-2 mt-2">
							...
							<Input
								...
								className="col-span-2"
								onKeyDown={(e) =>
									e.key === "Enter" && (e.preventDefault(), addTechnicalSpec())
								}
							/>
								...
								{filteredAccessories.map((acc) => (
									<div key={acc.id} className="flex items-center space-x-2">
										<Checkbox
											...
											onCheckedChange={(checked) =>
												handleAccessoryChange(acc.id, Boolean(checked))
											}
										/>
