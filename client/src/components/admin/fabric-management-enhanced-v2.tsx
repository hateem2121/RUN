...
const getButtonTestId = (action: string, target: string) =>
...
const getRepeatedTestId = (type: string, name: string, index: number) =>
  `${type}-${name}-${index}`;
...
export default function FabricManagementEnhancedV2() {
    ...
    let filtered = fabrics.filter((fabric) => {
      ...
      if (searchTerm) {
        ...
        const matchesSearch =
          ...
          (fabric.properties &&
            JSON.stringify(fabric.properties)
              .toLowerCase()
      ...
      if (filters.weightCategory !== "all" && fabric.weight) {
        ...
        if (weight !== null) {
          ...
          if (
            filters.weightCategory === "medium" &&
            (weight < 150 || weight > 300)
          )
            return false;
      ...
      if (filters.sustainability !== "all") {
        ...
        if (
          filters.sustainability === "highSustainability" &&
          (!score || score < 4)
        )
          return false;
        if (filters.sustainability === "mediumSustainability" && score !== 3)
          return false;
        if (
          filters.sustainability === "lowSustainability" &&
          (!score || score > 2)
        )
          return false;
  ...
  const [selectedSwatchAsset, setSelectedSwatchAsset] =
  ...
  const handleMediaSelect = (assets: MediaAsset | MediaAsset[]) => {
    ...
    console.log("Media selection attempt:", selectedAsset);

    if (selectedAsset && selectedAsset.id) {
      console.log("Media asset selected:", selectedAsset);
      setFormData((prev) => ({
        ...prev,
      ...
      console.warn("Invalid media asset selected:", selectedAsset);

      toast({
        title: "Selection Error",
        description:
          "Invalid media asset selected. Please try selecting again.",
        variant: "destructive",
      });
    }
  };
  ...
  const fetchMediaAssetById = async (
    id: number,
  ...
  const handleEdit = async (fabric: Fabric) => {
    ...
    const convertedData: EnhancedFormData = {
      ...
      shrinkageTolerancePercentage:
        props.shrinkageTolerancePercentage?.toString() || "",
  ...
  const handleSubmit = () => {
    ...
    const apiData = {
      ...
      enhancedMoistureManagement: cleanEmptyStrings(
        formData.enhancedMoistureManagement,
      ),
      ...
      shrinkageTolerancePercentage: cleanEmptyStrings(
        formData.shrinkageTolerancePercentage,
      ),
      ...
      certificationIds:
        formData.certificationIds?.filter(
          ...
          : undefined,
      certificationTags:
        formData.certificationTags?.filter(
          (id) => id !== null && id !== undefined,
        ).length > 0
          ? formData.certificationTags?.filter(
              (id) => id !== null && id !== undefined,
      ...
      washCareInstructions:
        ...
        (formData.washCareInstructions.careSymbols?.length > 0 ||
          ...
          ? {
              careSymbols:
                formData.washCareInstructions.careSymbols?.filter(
                  (item) => item !== "",
                ) || [],
              instructions: cleanEmptyStrings(
                formData.washCareInstructions.instructions,
              ),
              restrictions:
                formData.washCareInstructions.restrictions?.filter(
                  (item) => item !== "",
                ) || [],
            }
          : undefined,
      visualSwatchId:
        formData.visualSwatchId && typeof formData.visualSwatchId === "number"
          ...
          : formData.visualSwatchId &&
              typeof formData.visualSwatchId === "string"
            ? parseInt(formData.visualSwatchId)
            : undefined,
    };
    ...
    <div className="p-6 mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Comprehensive fabric management with structured properties and
            enterprise features
          </p>
        </div>
        <Button
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Active Fabrics</p>
          ...
          <CardContent className="p-6">
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                Get started by creating your first fabric with comprehensive
                properties
              </p>
              <Button
                onClick={() => setIsCreateModalOpen(true)}
            ...
            return (
              ...
              >
                <CardContent className="p-6">
                  ...
                  <Collapsible
                    open={isExpanded}
                    onOpenChange={() => toggleCardExpanded(fabric.id)}
                  >
                    <CollapsibleContent>
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-4 border-t">
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                <Badge
                                  variant={
                                    fabric.isActive ? "default" : "secondary"
                                  }
                                >
                                  {fabric.isActive ? "Active" : "Inactive"}
                                </Badge>
                          ...
                          {fabric.certifications &&
                            fabric.certifications.length > 0 && (
                              <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {fabric.certifications?.map(
                                    (certName: string, idx: number) => {
                        ...
                        <div className="space-y-4">
                          ...
                          {fabric.properties?.performanceFeatures &&
                            fabric.properties.performanceFeatures.length >
                              0 && (
                              <div className="space-y-2">
      ...
      >
        <EnhancedDialogContent contentType="form">
          <EnhancedDialogHeader>
            <EnhancedDialogTitle>
              {editingFabric ? "Edit Fabric" : "Create New Fabric"}
            </EnhancedDialogTitle>
          </EnhancedDialogHeader>
          ...
          <EnhancedDialogBody>
            <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Enter single value (e.g., 200) or range (e.g.,
                          180-220)
                        </div>
                      </div>
                    </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {formData.compositions.map(
                    (composition, compositionIndex) => (
                      <div
                        key={compositionIndex}
                        className={`p-4 border rounded-lg ${composition.isDefault ? "bg-blue-50 border-blue-200" : "bg-gray-50"}`}
                      >
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (newCompositions[compositionIndex]) {
                                  newCompositions[compositionIndex].name =
                                    e.target.value;
                                  setFormData((prev) => ({
                                    ...prev,
                        ...
                        <div className="space-y-2">
                              ...
                              >
                                <div className="relative">
                                  <Select
                                    value={
                                      fiber.fiberId
                                        ? fiber.fiberId.toString()
                                        : ""
                                    }
                                    onValueChange={(value) => {
                                      console.log(
                                        "Fiber selection changed:",
                                        value,
                                      );
                                      const newCompositions = [
                                        ...formData.compositions,
                                      ...
                                      if (
                                        ...
                                        newCompositions[
                                          compositionIndex
                                        ].fibers[fiberIndex].fiberId = value
                                          ? parseInt(value)
                                          : null;
                                        setFormData((prev) => ({
                                          ...prev,
                                  ...
                                  >
                                    <SelectTrigger
                                      ...
                                      onClick={() =>
                                        console.log(
                                          "SelectTrigger clicked - fibers available:",
                                          fibers.length,
                                        )
                                      }
                                    >
                                    ...
                                    <SelectContent className="z-modal-nested max-h-72 overflow-y-auto min-w-0 w-full max-w-xs">
                                      {fibers && fibers.length > 0 ? (
                                        fibers.map((fiber) => (
                                          <SelectItem
                                            key={fiber.id}
                                            value={fiber.id.toString()}
                                            className="h-12 px-4 text-sm cursor-pointer hover:bg-gray-50"
                                          >
                                            {fiber.name}
                                          </SelectItem>
                                        ))
                                        ...
                                        <SelectItem
                                          value="no-fibers"
                                          disabled
                                          className="h-12 px-4 text-sm text-gray-400"
                                        >
                                          No fibers available
                                        </SelectItem>
                                      )}
                          ...
                          <div className="flex justify-end">
                            <span className="text-sm font-medium">
                              Total:{" "}
                              {(() => {
                                const total = composition.fibers.reduce(
                                  (sum, fiber) => {
                                ...
                                const hasNonNumeric = composition.fibers.some(
                                  (fiber) =>
                                    isNaN(parseFloat(fiber.percentage)),
                                );
                            ...
                            <Input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                  ...
                                  if (
                                    value &&
                                    !formData.keyApplications.includes(value)
                                  ) {
                                    setFormData((prev) => ({
                                      ...prev,
                                      keyApplications: [
                ...
                <Card>
                  ...
                  <CollapsibleContent>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="stretchPercentage">
                            Stretch Percentage (%)
                          </Label>
                          <div className="space-y-2">
                            <Input
                            ...
                            />
                            <div className="text-xs text-muted-foreground">
                              Enter single value (e.g., 150) or range (e.g.,
                              140-160)
                            </div>
                          </div>
                        </div>
                        <div>
                          <Label htmlFor="airPermeability">
                          ...
                          <div className="space-y-2">
                            ...
                            />
                            <div className="text-xs text-muted-foreground">
                              Enter single value (e.g., 50) or range (e.g.,
                              45-55)
                            </div>
                          </div>
                        </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          ...
                          />
                          <div className="text-xs text-muted-foreground">
                            Enter single value (e.g., 10000) or range (e.g.,
                            9500-10500)
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              ...
                              />
                              <div className="text-xs text-muted-foreground">
                                Enter single value (e.g., 25) or range (e.g.,
                                20-30)
                              </div>
                            </div>
                          </div>
                          <div>
                            <Label htmlFor="dryingTime">
                              Drying Time (min)
                            </Label>
                            <div className="space-y-2">
                              <Input
                              ...
                              />
                              <div className="text-xs text-muted-foreground">
                                Enter single value (e.g., 60) or range (e.g.,
                                55-65)
                              </div>
                            </div>
                          </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <div className="flex flex-wrap gap-2">
                            {formData.performanceFeatures.map(
                              (feature, index) => (
                                <Badge
                                  key={index}
                                  variant="secondary"
                                  ...
                                  <X
                                    ...
                                    onClick={() => {
                                      setFormData((prev) => ({
                                        ...prev,
                                        performanceFeatures:
                                          prev.performanceFeatures.filter(
                                            (_, i) => i !== index,
                                          ),
                                      }));
                                    }}
                          ...
                          <Select
                            onValueChange={(value) => {
                              if (
                                !formData.performanceFeatures.includes(value)
                              ) {
                                setFormData((prev) => ({
                                  ...prev,
                                  performanceFeatures: [
                            ...
                            <Input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                  ...
                                  if (
                                    value &&
                                    !formData.performanceFeatures.includes(
                                      value,
                                    )
                                  ) {
                                    setFormData((prev) => ({
              ...
              >
                <Card>
                  ...
                  <CollapsibleContent>
                    <CardContent className="space-y-4">
                      <div>
                        <Label htmlFor="yarnCountConstruction">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          <Label htmlFor="pillingGrade">
                            Pilling Grade (ISO 1-5)
                          </Label>
                          <div className="space-y-2">
                            <Select
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            <Label htmlFor="shrinkagePercentage">
                              Percentage (%)
                            </Label>
                            <div className="space-y-2">
                              <Input
                              ...
                              />
                              <div className="text-xs text-muted-foreground">
                                Enter single value (e.g., 3) or range (e.g.,
                                2-4)
                              </div>
                            </div>
                          </div>
                          <div>
                            ...
                            <div className="space-y-2">
                              ...
                              />
                              <div className="text-xs text-muted-foreground">
                                Enter single value (e.g., 40) or range (e.g.,
                                30-50)
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            onValueChange={(value) => {
                              ...
                              if (
                                !formData.certificationTags.includes(certId)
                              ) {
                                setFormData((prev) => ({
                                  ...prev,
                                  certificationTags: [
                          ...
                          >
                            ...
                            <SelectContent className="z-modal-nested max-h-72 overflow-y-auto min-w-0 w-full max-w-xs">
                              {certificates
                                .filter(
                                  (cert) =>
                                    !formData.certificationTags.includes(
                                      cert.id,
                                    ),
                                )
                                .map((cert) => (
                            ...
                            <Input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                  ...
                                  if (
                                    value &&
                                    !formData.endOfLifeOptions.includes(value)
                                  ) {
                                    setFormData((prev) => ({
                                      ...prev,
                                      endOfLifeOptions: [
              ...
              <Collapsible
                open={sectionsOpen.care}
                onOpenChange={() => toggleSectionOpen("care")}
              >
                <Card>
                  <CollapsibleTrigger asChild>
                    <CardHeader className="cursor-pointer hover:bg-muted/50">
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              onValueChange={(value) => {
                                if (
                                  !formData.washCareInstructions.careSymbols.includes(
                                    value,
                                  )
                                ) {
                                  setFormData((prev) => ({
                            ...
                            <div className="flex items-center gap-2 p-2 border rounded">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-xs text-muted-foreground truncate">
                                  {selectedSwatchAsset.originalName ||
                                    selectedSwatchAsset.filename}
                                </div>
                              </div>
                              <Button
                                variant="ghost"
      ...
      >
        <EnhancedDialogContent contentType="form">
          <EnhancedDialogHeader>
            <EnhancedDialogTitle>Delete Fabric</EnhancedDialogTitle>
          </EnhancedDialogHeader>
          <EnhancedDialogBody>
            <p className="text-sm text-neutral-600">
              Are you sure you want to delete "{fabricToDelete?.name}"? This
          ...
          <EnhancedDialogFooter>
            ...
            <Button
              ...
              onClick={() =>
                fabricToDelete && deleteMutation.mutate(fabricToDelete.id)
              }
