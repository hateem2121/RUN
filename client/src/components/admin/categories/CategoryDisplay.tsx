...
import { ChevronDown, ChevronRight, Edit2, GripVertical, Image, MoreVertical, Package, Trash2 } from "lucide-react";
    ...
    {category.primaryImageId ? (
      <img
        ...
        onError={(e) => {
          ...
          parent.innerHTML = '<div class="w-full h-full flex items-center justify-center text-gray-400"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg></div>';
        }}
      />
    ) : (
...
const CategoryActions = memo(({ category, onEdit, onDelete, isLoading }: {
  category: Category;
  onEdit: (category: Category) => void;
  onDelete: (category: Category) => void;
  isLoading?: boolean;
...
const CategoryTableRow = memo((props: CategoryDisplayProps) => {
  const { category, isSelected, productCount, parentName, isLoading, onToggleSelection, onEdit, onDelete, enableDragDrop = true } = props;
  ...
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: category.id, disabled: !enableDragDrop });
...
const CategoryGridCard = memo((props: CategoryDisplayProps) => {
  const { category, isSelected, productCount, isLoading, onToggleSelection, onEdit, onDelete, enableDragDrop = true } = props;
...
const CategoryTreeItem = memo((props: CategoryDisplayProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mr-3" style={{ paddingLeft: `${indent}px` }}>
        {enableDragDrop && (
          <button
            className="cursor-grab active:cursor-grabbing text-gray-400 hover:text-gray-600 transition-colors p-1"
            {...attributes}
