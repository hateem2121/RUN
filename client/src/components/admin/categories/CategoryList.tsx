...
interface CategoryListProps {
  ...
  onReorder?: (reorderData: Array<{id: number, sortOrder: number, parentId?: number | null}>) => void;
}
...
const CategoryTableView = memo(({ categories, ...props }: CategoryListProps & { activeId?: number | null }) => (
...
// Grid view component
const CategoryGridView = memo(({ categories, ...props }: CategoryListProps & { activeId?: number | null }) => (
...
const CategoryTreeView = memo(({ categories, ...props }: CategoryListProps & { activeId?: number | null }) => {
  // Build hierarchy
  ...
  const renderTreeItem = (category: Category & { children?: Category[] }, depth = 0): React.ReactNode[] => {
...
export default function CategoryList(props: CategoryListProps) {
  ...
  return (
    ...
    >
      ...
      <DragOverlay>
        {activeCategory && (
          <div className="bg-white border-2 border-blue-300 rounded-lg shadow-lg p-2 opacity-90">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-gray-100 rounded flex items-center justify-center">
                {(activeCategory as any).imageUrl ? (
                  <img src={(activeCategory as any).imageUrl} alt="" className="w-full h-full object-cover rounded" />
                ) : (
