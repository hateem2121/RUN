...
export function MediaCleanupPanel() {
  ...
  const [cleanupStatus, setCleanupStatus] = useState<'idle' | 'scanning' | 'cleaning' | 'complete' | 'error'>('idle');
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await apiRequest('/api/admin/cleanup/trigger', { method: 'POST', body: { autoClean: false } });
  ...
  const handleCleanup = async () => {
    ...
    try {
      const response = await apiRequest('/api/admin/cleanup/trigger', { method: 'POST', body: { autoClean: true } });
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            {lastReport && lastReport.orphanedFiles.length + lastReport.brokenReferences.length > 0 && (
              <Button
          ...
          {cleanupStatus !== 'idle' && (
            <Alert className={
              cleanupStatus === 'error' ? 'border-red-200 bg-red-50' :
                cleanupStatus === 'complete' ? 'border-green-200 bg-green-50' :
                  'border-blue-200 bg-blue-50'
            }>
              <div className="flex items-center gap-2">
                {cleanupStatus === 'error' && <AlertTriangle className="w-4 h-4 text-red-500" />}
                {cleanupStatus === 'complete' && <CheckCircle className="w-4 h-4 text-green-500" />}
                {(cleanupStatus === 'scanning' || cleanupStatus === 'cleaning') && <Clock className="w-4 h-4 text-blue-500" />}
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">Orphaned Files</div>
              </div>
              ...
              <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">Broken References</div>
              </div>
