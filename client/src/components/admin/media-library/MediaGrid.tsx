...
const MediaGridItem = React.memo(
  ({
    ...
    console.log(`\ud83d\uddbc\ufe0f [IMG ${asset.id}] signedUrl=${signedUrl}, using=${imageUrl}`);
    if (isVideo) {
      console.log(`[MediaGridItem ${asset.id}] Video URL:`, {
        signedUrl,
        fallback: MediaUrlBuilder.buildUrlSafe(asset.id),
        final: videoUrl,
      });
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className={cn(
              "flex items-center justify-center w-6 h-6 rounded-full border-2 cursor-pointer transition-all",
              isSelected
        ...
        <div className="absolute top-2 right-2 z-20 flex gap-1">
          ...
          {typeof asset === "object" &&
            ...
            (asset as Record<string, unknown>).isOptimized === true && (
              <Badge variant="outline" className="text-xs shadow-sm-xs">
                Optimized
              </Badge>
            )}
        </div>
        ...
        <div
          className="aspect-square bg-gray-100 flex items-center justify-center cursor-pointer relative group w-full"
        ...
        >
          {isImage ? (
            isSvg ? (
              ...
              <img
                ...
                onError={(e) => {
                  const img = e.target as HTMLImageElement;
                  console.error(`[MediaGridItem ${asset.id}] Image load error for:`, img.src);
                  // Prevent infinite retry loop - set to placeholder on error
                  if (!img.src.includes("data:image/svg+xml")) {
                    img.src =
            ...
            <div className="relative w-full h-full bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20">
              ...
              <div
                className={`w-full h-full ${selectionMode ? "pointer-events-none" : "pointer-events-auto"}`}
              >
                <React.Suspense
                  fallback={
                ...
                >
                  <UnifiedModelViewer
                    ...
                    onError={() => {
                      ...
                      console.warn("3D model preview failed for asset:", asset.id);
                    }}
                  />
                </React.Suspense>
              </div>
              ...
              {/* 3D Model badge */}
              <div className="absolute bottom-1 right-1 bg-purple-500 text-white text-xs px-1 py-0.5 rounded text-[10px] font-medium">
                3D
              </div>
            </div>
          ) : (
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 backdrop-blur-[0px] group-hover:backdrop-blur-[2px] transition-all duration-300 ease-out flex items-center justify-center opacity-0 group-hover:opacity-100 pointer-events-none">
            <div className="flex gap-2 pointer-events-auto transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
              <Button
                size="sm"
                variant="secondary"
                className="rounded-full w-10 h-10 p-0 hover:scale-110 transition-transform shadow-lg bg-white/90 hover:bg-white text-black"
                onClick={(e) => {
                  e.stopPropagation();
                  onClick(asset, index);
                }}
    ...
    onMutate: async (deletedIds) => {
      ...
      getQueryClient().setQueriesData(
        ...
        (oldData: unknown) => {
          ...
          if (!dataRecord?.data || !Array.isArray(dataRecord.data)) return oldData;
...
export default function MediaGrid({
  ...
  useEffect(() => {
    ...
    console.log("  - page param:", params.get("page"), "(sent to backend)");
    console.log("  - limit param:", params.get("limit"), "(sent to backend)");
  ...
  }, [selectionMode, isStandalone, state.currentPage, state.totalPages, apiUrl]);
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      ...
      const timeoutId = setTimeout(() => {
        // Don't abort - just log slow queries for monitoring
        console.warn(`[MediaGrid] Slow media query: ${apiUrl} taking >30s`);
      }, 30000);
      ...
      try {
        ...
        console.log("✅ FETCH SUCCESS:", {
          url: apiUrl,
          page: params.get("page"),
          receivedItems: jsonData?.data?.length || 0,
          totalPages: jsonData?.meta?.pages || 0,
          totalItems: jsonData?.meta?.total || 0,
          timestamp: new Date().toISOString(),
        });
        ...
        if (err?.name === "AbortError" || err?.message?.includes("aborted")) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.debug("[MediaGrid] Query aborted (dialog closed or navigation)", {
              apiUrl,
    ...
    retry: (failureCount, error) => {
      ...
      if (err?.message?.includes("429")) {
        console.warn(`[MediaGrid] Rate limit hit, retry attempt ${failureCount}/3`);
    ...
    retryDelay: (attemptIndex, error) => {
      ...
      if (retryAfter) {
        ...
        if (!Number.isNaN(retrySeconds)) {
          console.log(`[MediaGrid] Retry-After header: ${retrySeconds}s`);
      ...
      if (err?.message?.includes("429")) {
        ...
        console.log(`[MediaGrid] 429 backoff delay: ${delay}ms`);
        return delay;
      }
  ...
  // 🔍 PAGINATION DEBUG: Log all pagination data for debugging
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  console.log("📊 PAGINATION DATA AUDIT");
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  console.log("📥 Raw Response:");
  console.log("  - metaRaw (backend format):", metaRaw);
  console.log("  - paginationRaw (legacy format):", paginationRaw);
  ...
  console.log("  - metaRaw.pages:", metaRaw?.pages);
  console.log("  - paginationRaw.totalPages:", paginationRaw?.totalPages);
  console.log("✅ Final Pagination Object:");
  console.log("  - totalPages:", pagination.totalPages);
  console.log("  - total:", pagination.total);
  console.log("  - page:", pagination.page);
  console.log("  - limit:", pagination.limit);
  console.log("🎯 Display Data:");
  console.log("  - displayAssets.length:", displayAssets.length);
  ...
  useEffect(() => {
    if (mediaResponse && status === "success") {
      ...
      console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
      console.log("📤 Request:");
      console.log("  - page param:", requestPage);
      console.log("  - limit param:", requestLimit);
      console.log("  - frontend currentPage:", state.currentPage);
      console.log("📥 Response:");
      console.log("  - backend page:", pagination.page);
      console.log("  - backend limit:", pagination.limit);
      console.log("  - backend totalPages:", pagination.totalPages);
      console.log("  - backend total items:", pagination.total);
      console.log("  - items received:", displayAssets.length);
      console.log("🔍 Consistency Check:");
      console.log(
        "  - Request page = Response page?",
        pageMatch ? "✓" : "✗",
        `(${requestPage} === ${pagination.page})`,
      );
      console.log(
        "  - Request limit = Response limit?",
        limitMatch ? "✓" : "✗",
        `(${requestLimit} === ${pagination.limit})`,
      );
      console.log(
        "  - Frontend page = Backend page?",
        frontendMatch ? "✓" : "✗",
        `(${state.currentPage} === ${pagination.page})`,
      );
      if (!pageMatch || !limitMatch || !frontendMatch) {
        console.error("🚨 DESYNC DETECTED! Frontend and backend pagination mismatch!");
      }
      console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mediaResponse, status, state.currentPage, displayAssets.length]);
  ...
  }, [state.searchTerm, state.selectedType, state.sortBy, state.sortOrder]);
  ...
  const { data: batchContent } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log("🔍 [DEBUG] Batch fetch complete:", {
        requestedIds: ids,
      ...
      for (const result of results) {
        if (result.success && result.url) {
          ...
          console.log(`✅ [DEBUG] Mapped ${result.id} -> ${result.url.substring(0, 60)}...`);
        } else {
          console.warn(`❌ [DEBUG] Failed to map ${result.id}:`, result);
        }
      }

      console.log("🗺️ [DEBUG] Final URL map:", urlMap);
      console.log("📊 [DEBUG] URL map size:", Object.keys(urlMap).length);
  ...
  const handleAssetSelect = useCallback(
    (assetId: number, asset?: MediaAsset) => {
      console.log("🎯 ASSET SELECTION CLICK:", {
        assetId,
        hasAsset: !!asset,
        hasExternalCallback: !!onAssetSelect,
        selectionMode,
      });
      ...
      if (onAssetSelect) {
        console.log(
          "📞 Calling external onAssetSelect callback for asset:",
          assetId,
          asset?.filename,
        );
    ...
    [toggleAsset, onAssetSelect, selectionMode],
  );
  ...
  if (error) {
    return (
      <div className="error-card-base p-6 rounded-lg">
  ...
  return (
    <div className={cn("space-y-4 min-h-0", needsBottomPadding && "pb-24")}>
      ...
      {(() => {
        ...
        console.log("  - OLD Condition: !selectionMode && totalPages > 1 ❌ (was hiding in modal)");
        console.log("  - NEW Condition: totalPages > 1 ✅ (shows in both standalone & modal)");
        console.log(
          "  - totalPages > 1 =",
        ...
        console.log("  - Will render pagination:", shouldShowPagination ? "✅ YES" : "❌ NO");
        console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

        return shouldShowPagination;
      })() && (
        <div
          className="flex items-center justify-between py-6 px-4 border-t"
