  ...
  const handleImageBitmapError = (event: PromiseRejectionEvent) => {
    ...
    if (
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn("[PHASE 3 FIX] Handled ImageBitmapLoader error gracefully:", {
          message,
        ...
        (oldData: unknown) => {
          ...
          if (Array.isArray(oldData)) {
            return oldData.map((item: unknown) =>
              typeof item === "object" && item !== null && "id" in item && item.id === assetId
                ? { ...(item as object), originalName: updates.name, tags: updates.tags }
                : item,
            );
          }
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        ...
                        {selectedAsset.type === "model" && (
                          <span className="text-xs block mt-1">{/* Size info simplified */}</span>
                        )}
                      </p>
                    </div>
                    <div>
