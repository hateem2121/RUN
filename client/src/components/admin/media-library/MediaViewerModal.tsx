...
export default function MediaViewerModal() {
	...
	const deleteMutation = useMutation({
		...
		onMutate: async (assetId: number) => {
			...
			await getQueryClient().cancelQueries({
				predicate: (q) =>
					Array.isArray(q.queryKey) && q.queryKey[0] === "apimedia",
			});
					...
					if (
						...
						if (nestedData.data && Array.isArray(nestedData.data)) {
							...
							if (
								...
								const paginationData = nestedData.pagination as Record<
									string,
									unknown
								>;
								paginationUpdate = {
	...
	const editMutation = useMutation({
		...
		onError: (error, variables, context) => {
			...
			if (
				context?.prevSelectedAsset &&
				selectedAsset?.id === variables.assetId
			) {
	...
	return (
		<>
			...
			>
				<EnhancedDialogContent
					className="z-modal flex flex-col"
				...
				>
					<EnhancedDialogHeader className="shrink-0 border-b pb-4">
						...
						<EnhancedDialogDescription>
							View and manage your media assets
						</EnhancedDialogDescription>
					</EnhancedDialogHeader>

					<div className="flex-1 flex min-h-0 overflow-hidden pt-4">
						{/* Media display */}
						<div className="flex-1 flex items-center justify-center bg-muted/20 rounded-lg mr-4 max-w-[calc(100%-17rem)] max-h-full">
							{selectedAsset && (
								...
								{selectedAsset && (
									<div className="space-y-2 text-sm">
										...
										<div>
											...
											<p className="text-muted-foreground">
												{new Date(
													selectedAsset.uploadedAt || Date.now(),
												).toLocaleDateString()}
											</p>
										</div>
									</div>
