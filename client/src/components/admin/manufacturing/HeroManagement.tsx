...
export function HeroManagement({ mediaAssets }: HeroManagementProps) {
	...
	const { data: specificVideo } = useQuery<MediaAsset>({
		...
		queryFn: async (): Promise<MediaAsset> => {
			...
			const res = await apiRequest(`/api/media/${heroData.videoId}`, {
				method: "GET",
			});
			return res.data;
		},
	...
	const { data: specificBackground } = useQuery<MediaAsset>({
		...
		queryFn: async (): Promise<MediaAsset> => {
			...
			const res = await apiRequest(`/api/media/${heroData.backgroundMediaId}`, {
				method: "GET",
			});
			return res.data;
		},
		enabled:
			...
			!mediaAssets?.find(
				(asset: MediaAsset) => asset.id === heroData.backgroundMediaId,
			),
	});
						...
						<div className="grid grid-cols-2 gap-4">
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setHeroData({ ...heroData, bottomCtaTitle: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setHeroData({ ...heroData, bottomCtaText: e.target.value })
									}
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setHeroData({ ...heroData, bottomCtaLink: e.target.value })
									}
									placeholder="e.g., /contact"
								/>
							</div>
							...
							<div className="flex items-center gap-2 mt-1">
								...
								>
									...
									{finalSelectedBackground
										? finalSelectedBackground.filename
										: "Select Background"}
								</Button>
								{finalSelectedBackground && (
									<Button
										...
										size="sm"
										onClick={() =>
											setHeroData({ ...heroData, backgroundMediaId: null })
										}
									>
										Clear
									</Button>
