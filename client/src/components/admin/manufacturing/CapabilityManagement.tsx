						...
						{capability.equipment && capability.equipment.length > 0 && (
							<div className="mt-2">
								...
								<div className="text-xs text-gray-600">
									...
									{capability.equipment.length > 3 &&
										` +${capability.equipment.length - 3} more`}
								</div>
							</div>
						)}
					</div>
...
export function CapabilityManagement({
	mediaAssets,
}: CapabilityManagementProps) {
	const [editingCapability, setEditingCapability] =
		useState<ManufacturingCapability | null>(null);
	...
	const [showCapabilityImagePicker, setShowCapabilityImagePicker] =
		useState(false);
	...
	const finalSelectedCapabilityImage =
		selectedCapabilityImage || specificCapabilityImage;
	...
	const handleCapabilitySubmit = (e: React.FormEvent) => {
		...
		const formData = {
			...capabilityForm,
			name:
				capabilityForm.name || capabilityForm.title || "Untitled Capability",
	...
	const handleEditCapability = (capability: ManufacturingCapability) => {
		...
		setCapabilityForm({
			...
			specifications:
				(capability.specifications as unknown as Specification[]) ?? [],
	...
	const handleRemoveSpecification = (index: number) => {
		const newSpecifications = capabilityForm.specifications.filter(
			(_, i) => i !== index,
		);
	...
	const getPreviewCapability = (): ManufacturingCapability => {
		return {
			...
			specifications: capabilityForm.specifications as unknown as Record<
				string,
				any
			>,
			name:
			...
			<CardContent>
				{capabilitiesLoading ? (
					...
					<div className="text-center py-8 text-gray-500">
						No manufacturing capabilities found. Create your first capability to
						get started.
					</div>
				) : (
					<DndContext
				...
				>
					<EnhancedDialogContent
						contentType="form"
						className={showPreview ? "max-w-6xl w-full" : "max-w-xl w-full"}
					>
						<EnhancedDialogHeader>
							<div className="flex items-center justify-between">
								<div>
									...
									<EnhancedDialogDescription>
										Configure manufacturing capability details and
										specifications
									</EnhancedDialogDescription>
								</div>
								<Button
						...
						>
							<EnhancedDialogBody className="space-y-4 px-1">
								<div className={showPreview ? "flex gap-6" : ""}>
									<div
										className={showPreview ? "flex-1 space-y-4" : "space-y-4"}
									>
										...
										<div>
											...
											<div className="space-y-2 mb-4">
												{capabilityForm.specifications.map(
										...
										<div className="flex-1 border-l pl-6">
											<div className="sticky top-0 space-y-4">
												...
												<div className="p-4 bg-muted/50 rounded-lg text-xs text-muted-foreground">
													<p>
														This preview shows how the capability card will
														appear on the public site. The layout size may
