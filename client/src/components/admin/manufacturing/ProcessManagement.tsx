...
export function ProcessManagement({ mediaAssets }: ProcessManagementProps) {
	const [editingProcess, setEditingProcess] =
		useState<ManufacturingProcess | null>(null);
	...
	const { data: specificProcessMedia } = useQuery({
		...
		queryFn: async () => {
			...
			const response = await fetch(
				`/api/media?ids=${processForm.mediaIds.join(",")}`,
			);
			...
			<CardContent>
				{processesLoading ? (
					...
					<div className="text-center py-8 text-gray-500">
						No manufacturing processes found. Create your first process to get
						started.
					</div>
				) : (
					<DndContext
							...
							<EnhancedDialogBody className="space-y-4 px-1">
								...
								>
									...
									<div className="space-y-4">
										...
										<div className="grid grid-cols-2 gap-4">
											<div>
												<Label htmlFor="process-efficiency">
													Efficiency (%)
												</Label>
											...
											<div className="flex items-center gap-2 mt-1">
												...
												{selectedProcessMedia.length > 0 && (
													<Button
														...
														variant="ghost"
														onClick={() =>
															setProcessForm({ ...processForm, mediaIds: [] })
														}
										...
										<div className="sticky top-0 self-start">
											<LivePreviewGrid>
												<ProcessCard
													...
													mediaAssets={[
														...mediaAssets,
														...finalSelectedProcessMedia,
													]} // Ensure we have context of both existing and picked media
												/>
											</LivePreviewGrid>
											<div className="mt-4 text-xs text-muted-foreground flex items-center gap-2">
												<LayoutTemplate className="w-4 h-4" />
												Drawing from {finalSelectedProcessMedia.length}{" "}
												connected media assets
											</div>
										</div>
									)}
								</div>
