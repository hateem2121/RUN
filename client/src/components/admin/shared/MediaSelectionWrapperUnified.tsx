...
const getAssetIcon = (type: string) => {
  switch (type) {
    ...
    case 'model': return <FileIcon className="h-4 w-4" />;
...
const getAssetTypeColor = (type: string) => {
  switch (type) {
    case 'image': return 'bg-emerald-100 text-emerald-800 border-emerald-200 hover:bg-emerald-200';
    case 'video': return 'bg-purple-100 text-purple-800 border-purple-200 hover:bg-purple-200';
    case 'model': return 'bg-blue-100 text-blue-800 border-blue-200 hover:bg-blue-200';
...
export function MediaSelectionWrapperUnified({ 
  ...
  const handleAssetSelect = (assetId: number, asset?: MediaAsset) => {
    ...
    console.log('🔹 Asset Object Provided:', !!asset);
    console.log('🔹 Selection Mode:', selectionMode);
    console.log('🔹 Current Selection IDs:', Array.from(state.selectedAssets));
    ...
    if (selectedAsset) {
      console.log('✅ Asset available:', {
        ...
        source: asset ? 'PROVIDED' : 'FOUND_IN_CONTEXT'
      });
      ...
      setSelectedAssetsCache(prev => {
        ...
        console.log('📦 Cached asset. Total cached:', updated.size);
        return updated;
      });
    } else {
      console.error('❌ Asset NOT found!', {
        ...
        wasProvided: !!asset,
        availableIds: assets.map(a => a.id).slice(0, 10)
      });
    }
    ...
    if (selectionMode === 'single') {
      ...
      if (currentSelection.has(assetId)) {
        ...
        console.log('➕ Adding asset to selection');
        currentSelection.add(assetId);
        selectAssets(Array.from(currentSelection));
      } else {
        console.warn('⚠️ Max selection limit reached:', maxSelection);
      }
    }
    
    console.log('🔹 New Selection IDs:', Array.from(state.selectedAssets));
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  };

  const handleConfirmSelection = async () => {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('✅ CONFIRM SELECTION TRIGGERED');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('🔹 Selected Asset IDs:', selectedAssetIds);
    console.log('🔹 Selection Mode:', selectionMode);
    console.log('🔹 Assets in context (current page):', assets.length);
    console.log('🔹 Cached assets:', selectedAssetsCache.size);
    ...
    if (selectedAssetIds.length > 0) {
      ...
      const { foundAssets, missingIds } = selectedAssetIds.reduce<{
        ...
        if (foundAsset) {
          console.log(`✅ Found asset ${id} in CACHE:`, foundAsset.filename);
        ...
        if (foundAsset) {
          console.log(`✅ Found asset ${id} in CURRENT PAGE:`, foundAsset.filename);
          acc.foundAssets.push(foundAsset);
          return acc;
        }
        
        console.warn(`⚠️ Asset ${id} NOT FOUND in cache or current page - will fetch from API`);
      ...
      if (missingIds.length > 0) {
        console.log(`🌐 Fetching ${missingIds.length} missing assets from API:`, missingIds);
        try {
          ...
          console.log(`✅ Fetched ${fetchedAssets.length} assets from API`);
      ...
      console.log('📦 Selected assets resolved:', foundAssets.length, 'of', selectedAssetIds.length);
      console.log('📋 Asset details:', foundAssets.map(a => ({
        id: a.id,
        filename: a.filename,
        type: a.type
      })));
      ...
      if (foundAssets.length > 0) {
        ...
        try {
          onSelect(result);
          console.log('🟢 onSelect executed successfully!');
          console.log('🟢 Parent should now have:', selectionMode === 'single' ? '1 asset' : `${foundAssets.length} assets`);
        } catch (error) {
          console.error('🔴 ERROR calling onSelect:', error);
        }
      } else {
        console.error(' NO ASSETS RESOLVED! Selection lost!');
        console.error('❌ This should not happen with API fallback');
      }
    } else {
      console.warn('⚠️ No asset IDs selected');
    }
        ...
        {selectionMode === 'multiple' && selectedAssetsForDisplay.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-sm font-semibold text-gray-700">Selected Assets</h4>
              ...
              {selectedAssetsForDisplay.map((asset: MediaAsset) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium truncate" title={asset.originalName || asset.filename}>
                        ...
                        ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-blue-700 bg-blue-100 px-2 py-1 rounded-md text-sm font-medium">
                              {selectedAssetsForDisplay[0]?.originalName || selectedAssetsForDisplay[0]?.filename || 'Asset'}
                            </span>
                          </div>
                        )
                        : (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <span className="text-sm text-gray-600 font-medium">
                    Choose {selectionMode === 'single' ? 'a media asset' : 'media assets'} from above
                  </span>
                  <div className="text-xs text-gray-400 mt-0.5">
                    {selectionMode === 'multiple' ? `Up to ${maxSelection} assets` : 'Single selection'}
                  </div>
                </div>
              </div>
            )}
