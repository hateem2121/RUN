				...
				<div className="space-y-4">
					...
					<div className="grid grid-cols-3 gap-3">
						{SUSTAINABILITY_ICONS.map((icon) => {
							...
							return (
								...
								>
									<div
										className={`p-2 rounded-lg ${isSelected ? "bg-green-100" : "bg-white"}`}
									>
										<IconComponent
											className={`w-6 h-6 ${isSelected ? "text-green-600" : "text-gray-600"}`}
										/>
									</div>
									<span
					...
					<div className="border-t pt-4">
						...
						<div className="flex items-center gap-3 mt-2 p-3 bg-gray-50 rounded-lg">
							{(() => {
								const PreviewIcon =
									SUSTAINABILITY_ICONS.find((i) => i.name === selectedIcon)
										?.component || Leaf;
								return (
									<>
