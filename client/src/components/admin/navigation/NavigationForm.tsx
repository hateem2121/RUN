				...
				{formData.iconType === "fallback" && (
					<div>
						...
						<Select
							...
							onValueChange={(value) =>
								setFormData({ ...formData, fallbackIcon: value })
							}
						>
							<SelectTrigger>
							...
							{formData.mediaIconId && (
								<div className="flex items-center space-x-2 p-2 bg-gray-50 dark:bg-neutral-800 rounded-md">
									<div className="w-8 h-8 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
										<img
											src={
												imageAssets.find((a) => a.id === formData.mediaIconId)
													?.url ||
												(formData.mediaIconId &&
												formData.mediaIconId < 1000000000000
													? `/api/media/${formData.mediaIconId}/content`
													: undefined)
											}
											alt={
												imageAssets.find((a) => a.id === formData.mediaIconId)
													?.originalName || "Navigation icon"
											}
									...
									<Button
										...
										size="sm"
										onClick={() =>
											setFormData({ ...formData, mediaIconId: null })
										}
