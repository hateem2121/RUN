...
const TABLER_ICONS = [
  ...
  { value: "IconShoppingCart", label: "Shopping Cart", component: IconShoppingCart },
  ...
  { value: "IconCertificate", label: "Certificates", component: IconCertificate },
...
export function NavigationForm({ item, onSubmit, onCancel }: NavigationFormProps) {
  ...
  const { data: mediaResponse } = useQuery<{success: boolean, data: {data: MediaAsset[], pagination: any}}>({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!validation.success) {
      ...
      console.error("[Navigation Form] Validation errors:", validation.error.issues);
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input
          ...
          onChange={(e) => setFormData({ ...formData, title: e.target.value, label: e.target.value })}
          required
        />
      </div>
      ...
      <div>
        ...
        <Select 
          ...
          onValueChange={(value: 'media' | 'fallback') => setFormData({ ...formData, iconType: value })}
        >
          <SelectTrigger>
            <SelectValue />
          </SelectTrigger>
      ...
      <div>
        ...
        <Select 
          ...
          onValueChange={(value: 'small' | 'medium' | 'large') => setFormData({ ...formData, iconSize: value })}
        >
          <SelectTrigger>
              ...
              <SelectValue placeholder="Choose an icon">
                {formData.fallbackIcon && (
                  <div className="flex items-center gap-2">
                    {(() => {
                      const selectedIcon = TABLER_ICONS.find(icon => icon.value === formData.fallbackIcon);
                      if (selectedIcon) {
                ...
                return (
                  <SelectItem key={icon.value} value={icon.value}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-500 ml-auto">{icon.value}</span>
                    </div>
                  </SelectItem>
                );
              })}
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            {formData.mediaIconId && (
              <div className="flex items-center space-x-2 p-2 bg-gray-50 dark:bg-neutral-800 rounded-md">
                <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                  <img
                    src={imageAssets.find(a => a.id === formData.mediaIconId)?.url || (formData.mediaIconId && formData.mediaIconId < 1000000000000 ? `/api/media/${formData.mediaIconId}/content` : undefined)}
                    alt={imageAssets.find(a => a.id === formData.mediaIconId)?.originalName || "Navigation icon"}
                    className="w-full h-full object-cover"
                  />
                </div>
                <span className="text-sm text-gray-600 dark:text-gray-300" data-testid="text-media-icon-preview">
      ...
      <div className="flex gap-2 pt-4">
        <Button type="submit">
          {item ? "Update" : "Create"} Navigation Item
        </Button>
