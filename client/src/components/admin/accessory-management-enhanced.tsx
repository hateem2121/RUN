...
const getCategoryColor = (category: string) => {
	...
	return (
		colors[category] || "bg-neutral-100 text-neutral-700 border-neutral-200"
	);
};
	...
	return (
		<div className="space-y-4 max-h-96 overflow-y-auto">
			{activeAccessories.map((accessory) => (
				<div
					key={accessory.id}
					className="group p-4 border border-neutral-200 rounded-lg hover:border-purple-300 hover:shadow-md transition-all duration-200"
				>
						...
						<div className="flex-1 min-w-0">
							...
							<div className="flex flex-wrap gap-2 mb-2">
								{accessory.category && (
									<Badge
										variant="outline"
										className={`text-xs flex items-center gap-1 ${getCategoryColor(accessory.category)}`}
									>
										{getCategoryIcon(accessory.category)}
										{accessory.category.charAt(0).toUpperCase() +
											accessory.category.slice(1)}
									</Badge>
								)}
							</div>
										...
										<div className="space-y-1">
											{Object.entries(accessory.specifications)
												...
												.map(([key, value]) => (
													<div
														key={key}
														className="text-xs text-neutral-700 bg-neutral-50 px-2 py-1 rounded"
													>
...
export default function AccessoryManagementEnhanced() {
	...
	const [previewAccessory, setPreviewAccessory] = useState<Accessory | null>(
		null,
	);
			...
			<div className="flex items-center justify-between mb-8">
				<div>
					...
					<p className="text-neutral-600 mt-2">
						Manage printing services, hardware, finishing touches, and packaging
						options
					</p>
				</div>
			</div>
									...
									{specifications.length > 0 && (
										<div className="space-y-3">
											{specifications.map((spec) => (
												<div
													key={spec.id}
													className="grid grid-cols-1 md:grid-cols-2 gap-3 p-3 bg-neutral-50 rounded-lg"
												>
													<div>
														<Input
						...
						{previewAccessory && (
							<div className="space-y-4">
								<div className="flex items-center gap-4">
									...
									<Badge
										variant={
											previewAccessory.isActive ? "default" : "secondary"
										}
									>
									...
									Object.keys(previewAccessory.specifications).length > 0 && (
										<div>
											...
											<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
												{Object.entries(previewAccessory.specifications).map(
													([key, value]) => (
														<div
															key={key}
