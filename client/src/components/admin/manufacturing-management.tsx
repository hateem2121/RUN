...
export default function ManufacturingManagement() {
	...
	const mediaAssets = Array.isArray(mediaResponse?.data?.data)
		...
		<div className="space-y-6">
			...
			{isMediaLoading ? (
				<div className="flex items-center justify-center h-32 space-x-2">
					<div className="w-4 h-4 rounded-full bg-manufacturing-primary animate-bounce [animation-delay:-0.3s]"></div>
					<div className="w-4 h-4 rounded-full bg-manufacturing-primary animate-bounce [animation-delay:-0.15s]"></div>
					<div className="w-4 h-4 rounded-full bg-manufacturing-primary animate-bounce"></div>
				</div>
			) : (
				<Tabs
					defaultValue="hero"
					className="space-y-4 animate-in fade-in-50 slide-in-from-bottom-4 duration-500"
				>
