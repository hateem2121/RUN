...
export function CrossPageDashboard() {
	...
	const { data: manufacturingProcesses = [] } = useQuery<
		ManufacturingProcess[]
	>({
	...
	const sustainableFabrics = fabrics.filter(
		(f) =>
			f.sustainabilityScore && parseInt(f.sustainabilityScore.toString()) >= 4,
	);
	const avgManufacturingEfficiency =
		manufacturingProcesses.length > 0
			...
			: 0;
	const activeInnovations = technologyInnovations.filter(
		(i) => i.isActive,
	).length;
			...
			? sustainabilityGoals.reduce((acc, g) => {
					const progress =
						g.currentProgress && g.target
							? (parseFloat(g.currentProgress.toString()) /
									parseFloat(g.target)) *
								100
							: 0;
					return acc + progress;
							...
							<div className="space-y-3">
								<div>
									<div className="flex justify-between text-sm mb-1">
										...
										<span className="font-medium">
											{sustainabilityProgress.toFixed(0)}%
										</span>
									</div>
								...
								<div>
									...
									<Progress
										value={
											fabrics.length > 0
												? (sustainableFabrics.length / fabrics.length) * 100
												: 0
										}
										className="h-2"
									/>
								...
								{sustainabilityMetrics.find(
									(m) => m.name === "Carbon Reduction",
								) && (
							...
							<div className="space-y-3">
								<div>
									<div className="flex justify-between text-sm mb-1">
										...
										<span className="font-medium">
											{avgManufacturingEfficiency.toFixed(0)}%
										</span>
									</div>
									<Progress
										value={avgManufacturingEfficiency}
										className="h-2"
									/>
								</div>
									...
									<Progress
										value={
											manufacturingQuality.length > 0
												? (manufacturingQuality.filter((q) => q.isActive)
														.length /
								...
								<div className="p-3 bg-blue-50 rounded-lg">
									<p className="text-sm font-medium text-blue-700">
										{manufacturingProcesses.filter((p) => p.isActive).length}{" "}
										Active Processes
									</p>
								</div>
							</div>
							...
							<div className="space-y-3">
								...
								<div className="p-3 bg-purple-50 rounded-lg">
									<p className="text-sm font-medium text-purple-700">
										{technologyResearch.filter((r) => r.isActive).length} Active
										Research Projects
									</p>
								</div>
							</div>
					...
					<div className="space-y-6">
						...
						<div className="p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg">
							...
							<p className="text-sm text-gray-600 mb-3">
								Sustainable fabrics processed with{" "}
								{avgManufacturingEfficiency.toFixed(0)}% efficient manufacturing
							...
							<div className="grid grid-cols-2 gap-4">
								<div className="text-center p-3 bg-white rounded">
									<div className="text-2xl font-bold text-green-600">
										{Math.round(
											(sustainableFabrics.length * avgManufacturingEfficiency) /
												100,
										)}
									</div>
								...
								<div className="text-center p-3 bg-white rounded">
									<div className="text-2xl font-bold text-blue-600">
										{(
											(avgManufacturingEfficiency * sustainabilityProgress) /
											100
										).toFixed(0)}
										%
									</div>
						...
						<div className="p-4 bg-gradient-to-r from-purple-50 to-green-50 rounded-lg">
							...
							<p className="text-sm text-gray-600 mb-3">
								...
								{sustainabilityGoals.filter((g) => g.isActive).length}{" "}
							...
							<div className="grid grid-cols-2 gap-4">
								...
								<div className="text-center p-3 bg-white rounded">
									<div className="text-2xl font-bold text-green-600">
										{
											technologyResearch.filter((r) =>
												r.researchArea?.includes("Sustainability"),
											).length
										}
									</div>
									<div className="text-xs text-gray-600">
						...
						<div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
							...
							<p className="text-sm text-gray-600 mb-3">
								...
								{Math.floor(manufacturingProcesses.length * 0.7)}{" "}
