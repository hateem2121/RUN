...
export const OptimizedHeroMedia = memo(function OptimizedHeroMedia({
  ...
  const handleLoad = useCallback(() => {
    ...
    if (import.meta.env.DEV) {
      console.log(`[OptimizedHeroMedia] Loaded asset ${asset.id}: ${asset.originalName}`);
    }
  }, [asset.id, asset.originalName, onLoad]);

  const handleError = useCallback(() => {
    // setHasError(true);
    const error = new Error(`Failed to load hero media asset ${asset.id}: ${asset.originalName}`);
    if (import.meta.env.DEV) {
      console.error('[OptimizedHeroMedia] Media loading error:', error);
    }
    onError?.(error);
    ...
    if (asset.type === 'video' && videoRef.current && isLoaded) {
      ...
      if (playPromise !== undefined) {
        playPromise.catch((error) => {
          // Auto-play prevented by browser - this is expected, ignore silently
          console.debug('[OptimizedHeroMedia] Autoplay prevented (non-critical):', error.message);
        });
      }
    }
  }, [asset.type, isLoaded]);
