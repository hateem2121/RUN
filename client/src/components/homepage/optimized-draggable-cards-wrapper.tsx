...
export function convertProcessCardsToDraggableCards(
  ...
  return processCards.map((card, index) => {
    ...
    const finalImageUrl = mediaUrl || FALLBACK_IMAGES[index % FALLBACK_IMAGES.length];

    return {
      title: card.title,
      description: card.description,
      image: finalImageUrl,
      className: IMPROVED_POSITIONING_PATTERNS[index % IMPROVED_POSITIONING_PATTERNS.length],
    };
  });
}
...
const DraggableCardsLoadingFallback = memo(function DraggableCardsLoadingFallback({
  className,
}: {
  className?: string;
}) {
  return (
    <div
      className={`${className} flex items-center justify-center rounded-lg border-2 border-neutral-200 border-dashed bg-neutral-50/50`}
    >
      <div className="p-8 text-center">
        <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-2 border-neutral-300 border-t-neutral-600"></div>
        <p className="text-neutral-500 text-sm">Loading Interactive Cards...</p>
      </div>
    </div>
  );
});
        ...
        {shouldRender ? (
          <DraggableCardContainer className="relative min-h-[500px] sm:min-h-[600px] md:min-h-[700px]">
            {items.map((item, index) => (
              <DraggableCardBody key={`card-${index}`} className={item.className}>
                <div className="relative h-full w-full overflow-hidden">
                  {item.image && (
                    <img src={item.image} alt={item.title} className="h-3/4 w-full object-cover" />
                  )}
                  <div className="p-4">
...
export function OptimizedDraggableCardsWrapper(props: OptimizedDraggableCardsWrapperProps) {
  return (
