...
export const DotGrid = memo(function DotGrid({
  ...
  useEffect(() => {
    ...
    let rafId: number;
    let isDrawing = true; // MEMORY LEAK PREVENTION: Animation control flag
    const proxSq = proximity * proximity;

    const draw = () => {
      // SAFETY: Stop animation if component unmounted or drawing disabled
      if (!isDrawing) return;

      const canvas = canvasRef.current;
      if (!canvas) {
        isDrawing = false; // Stop animation if canvas is gone
        return;
      }
      const ctx = canvas.getContext("2d");
      if (!ctx) return;
      ...
      // SAFETY: Only continue animation if still drawing
      if (isDrawing) {
    ...
    draw();
    return () => {
      // MEMORY LEAK PREVENTION: Clean shutdown
      isDrawing = false;
      if (rafId) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      // SAFETY: Skip if component unmounted
      if (!isMounted) return;
      ...
      // FIXED: Safe getBoundingClientRect call with null check
    ...
    return () => {
      // MEMORY LEAK PREVENTION: Mark component as unmounted
      isMounted = false;
