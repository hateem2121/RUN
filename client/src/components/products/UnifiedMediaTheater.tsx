...
export function UnifiedMediaTheater({
	...
	const sortedMedia = [...media]
		...
		.map((asset) => ({
			...
			url: asset.url?.startsWith("/api/media/")
				? asset.url
	...
	const is3DModel =
		currentMedia?.type === "3d_model" || currentMedia?.type === "model";
		...
		if (thumbnailContainerRef.current && sortedMedia.length > 1) {
			...
			if (selectedThumb) {
				const scrollLeft =
					...
					(container.offsetWidth - selectedThumb.offsetWidth) / 2;
				container.scrollTo({
					left: scrollLeft,
	...
	const handlePrevious = () => {
		...
		onInteraction?.({
			type: "interact",
			mediaType:
				(currentMedia?.type as "image" | "video" | "3d_model") || "image",
		...
		return (
			<div className={cn("space-y-4", className)}>
				<Tabs
					value={activeTab}
					onValueChange={(value) =>
						setActiveTab(value as "gallery" | "3d" | "video")
					}
					className="w-full"
				>
						...
						{model3DAssets.length > 0 ? (
							<div className="space-y-4">
								{model3DAssets.map((asset) => (
									<div
										key={asset.id}
										className="bg-gray-100 rounded-lg aspect-square relative"
									>
										...
										<LazyUnifiedModelViewer
											asset={{
												...asset,
												filename:
													asset.filename || `${productName}_3d_model.gltf`,
												originalName:
													asset.originalName ||
													asset.filename ||
						...
						{videoAssets.length > 0 ? (
							<div className="space-y-4">
								{videoAssets.map((asset) => (
									<div
										key={asset.id}
										className="relative bg-black rounded-lg aspect-video"
									>
										<video
								...
								{isVideo ? (
									<div className="relative w-full h-full">
										<video
											...
											src={
												currentMedia.url ||
												`/api/media/${currentMedia.id}/content`
											}
											...
											asset={useMemo(
												() => ({
													...
													filename:
														...
														`${productName}_main_model.gltf`,
													originalName:
														currentMedia.originalName ||
														currentMedia.filename ||
														productName,
													fileSize:
														currentMedia.fileSize ?? currentMedia.size ?? null,
													size:
														currentMedia.size ?? currentMedia.fileSize ?? null,
							...
							<button
								...
								onClick={() => {
									...
									onInteraction?.({
										type: "interact",
										mediaType:
											(item.type as "image" | "video" | "3d_model") || "image",
