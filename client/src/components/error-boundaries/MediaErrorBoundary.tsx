...
export class MediaErrorBoundary extends Component<MediaErrorBoundaryProps, MediaErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error(`[MediaErrorBoundary] Error in ${this.props.context || 'media component'}:`, error);
    console.error('[MediaErrorBoundary] Error info:', errorInfo);
        ...
        <Card className="w-full max-w-md mx-auto my-4">
          ...
          <CardContent className="text-center space-y-4">
            <p className="text-sm text-muted-foreground">
              {this.props.context ? `Error in ${this.props.context}` : 'Something went wrong loading this media content.'}
            </p>
...
export const MediaGalleryErrorBoundary: React.FC<{ children: ReactNode }> = ({ children }) => (
  <MediaErrorBoundary context="Media Gallery" onError={(error) => console.error('[MediaGallery] Error:', error)}>
    {children}
  </MediaErrorBoundary>
);
    ...
    fallback={
      <div className="flex items-center justify-center h-64 bg-muted rounded-lg">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">Media viewer unavailable</p>
        </div>
      </div>
    }
  >
...
export const MediaUploadErrorBoundary: React.FC<{ children: ReactNode }> = ({ children }) => (
  <MediaErrorBoundary 
    ...
    showRetry={true}
    onError={(error) => console.error('[MediaUpload] Error:', error)}
  >
    {children}
  </MediaErrorBoundary>
);