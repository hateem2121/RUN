...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('[Enhanced Animation Error Boundary]', {
        component: this.props.componentName || 'Unknown',
        error: error.message,
        stack: error.stack,
        errorInfo,
        gsapCleanup: this.props.enableGsapCleanup !== false,
  ...
  private performGsapCleanup = () => {
    try {
      if (typeof window !== 'undefined') {
        ...
        console.log('[Animation Error Boundary] GSAP cleanup completed');
      }
    } catch (cleanupError) {
      console.warn('[Animation Error Boundary] GSAP cleanup failed:', cleanupError);
    }
  };
  ...
  private logAnimationError = async (error: Error) => {
    try {
      ...
      console.warn('[Animation Error Boundary] Error logging failed:', logError);
    }
  };
    ...
    return (
      <div className="relative rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
        <div className="flex items-center justify-center mb-4">
          {isRecovering ? (
            <Zap className="h-6 w-6 text-blue-500 mr-2 animate-pulse" />
          ) : (
