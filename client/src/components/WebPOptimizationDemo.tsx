...
export function WebPOptimizationDemo() {
	...
	const mediaAssets = Array.isArray(mediaResponse?.data)
		? mediaResponse.data
		: [];
	...
	const optimizationStats: OptimizationStats = {
		...
		optimizedImages: largeImages.filter(
			(asset: MediaAsset) => asset?.metadata?.webpVariants,
		).length,
		...
		optimizationStats.totalOriginalSize > 0
			? Math.round(
					((optimizationStats.totalOriginalSize -
						optimizationStats.totalOptimizedSize) /
						optimizationStats.totalOriginalSize) *
						100,
				)
			: 0;
								...
								{optimizationStats.largestSaving.savings > 0 && (
									<div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
										<div className="text-sm font-medium text-green-800">
											Biggest Success:{" "}
											{optimizationStats.largestSaving.filename}
										</div>
										<div className="text-xs text-green-600">
											{formatBytes(
												optimizationStats.largestSaving.originalSize,
											)}{" "}
											→{" "}
											{formatBytes(
												optimizationStats.largestSaving.optimizedSize,
											)}
											...
											{asset.metadata?.compressionSavings ? (
												<div>
													...
													<div className="font-medium text-green-600">
														{formatBytes(
															asset.metadata.compressionSavings.optimizedSize,
														)}
												...
												(loadTimes.original > 0 || loadTimes.webp > 0) && (
													<div className="text-xs space-y-1 p-2 bg-gray-50 rounded">
														{loadTimes.original > 0 && (
															<div>
																Original: {loadTimes.original.toFixed(0)}ms
															</div>
														)}
														{loadTimes.webp > 0 && (
															<div className="text-green-600">
																...
																{loadTimes.original > 0 && (
																	<span className="ml-1">
																		...
																		{Math.round(
																			(1 -
																				loadTimes.webp / loadTimes.original) *
																				100,
																		)}
																		% faster)
																	</span>
