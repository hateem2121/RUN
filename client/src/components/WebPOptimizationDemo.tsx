...
export function WebPOptimizationDemo() {
  ...
  const largeImages = mediaAssets.filter((asset: MediaAsset) =>
  ...
  const optimizationStats: OptimizationStats = {
    ...
    optimizedImages: largeImages.filter((asset: MediaAsset) =>
      asset?.metadata?.webpVariants
    ).length,
    totalOriginalSize: largeImages.reduce((sum: number, asset: MediaAsset) => sum + (asset?.size || 0), 0),
  ...
  optimizationStats.averageSavings = optimizationStats.totalOriginalSize > 0 ?
    Math.round(((optimizationStats.totalOriginalSize - optimizationStats.totalOptimizedSize) / optimizationStats.totalOriginalSize) * 100) : 0;
    ...
    try {
      ...
      if (asset?.metadata?.webpVariants?.webp?.medium?.url) {
        ...
        try {
          ...
          console.warn('WebP load test failed:', error);
        }
      }
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className="text-sm text-gray-600">Total Large Images</div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className="text-sm text-gray-600">WebP Optimized</div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-orange-600">
                  {formatBytes(optimizationStats.totalOriginalSize - optimizationStats.totalOptimizedSize)}
                </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{optimizationStats.optimizedImages} / {optimizationStats.totalImages}</span>
                </div>
                <Progress
                  value={optimizationStats.totalImages > 0 ? (optimizationStats.optimizedImages / optimizationStats.totalImages) * 100 : 0}
                  className="h-2"
                />
                {optimizationStats.largestSaving.savings > 0 && (
                  <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <div className="text-xs text-green-600">
                      {formatBytes(optimizationStats.largestSaving.originalSize)} → {formatBytes(optimizationStats.largestSaving.optimizedSize)}
                ...
                {largeImages.slice(0, 5).map((asset: MediaAsset) => (
                  <div key={asset.id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-medium truncate">{asset.filename}</span>
                      {asset.metadata?.webpVariants && (
                ...
                {largeImages.slice(0, 6).map((asset: MediaAsset) => (
                  <div key={asset.id} className="border rounded-lg p-3">
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedImage?.id === asset.id && (loadTimes.original > 0 || loadTimes.webp > 0) && (
                        <div className="text-xs space-y-1 p-2 bg-gray-50 rounded">
                          ...
                          {loadTimes.webp > 0 && (
                            <div className="text-green-600">
                              ...
                              {loadTimes.original > 0 && (
                                <span className="ml-1">
                                  ({Math.round((1 - loadTimes.webp / loadTimes.original) * 100)}% faster)
                                </span>
                              )}
                            </div>
                          )}
