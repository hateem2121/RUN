...
export function SpecificationAccordion({
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent, sectionId: string) => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      const nextIndex = (currentIndex + direction + specifications.length) % specifications.length;
      const nextSection = specifications[nextIndex];

      if (nextSection) {
        setFocusedSection(nextSection.id);
        const nextElement = containerRef.current?.querySelector(`[data-section="${nextSection.id}"]`) as HTMLElement;
        nextElement?.focus();
      }
    }
  ...
  useEffect(() => {
    if (focusedSection) {
      const element = containerRef.current?.querySelector(`[data-section="${focusedSection}"]`) as HTMLElement;
      element?.focus();
    }
  }, [focusedSection]);
...
export const createFabricSpecifications = (fabricData: any, productData?: any): SpecificationSection => {
  const technicalSpecs = productData?.technicalSpecs || {};

  // Debug logging for custom fields
  console.log('[SpecificationAccordion] DEBUG - Product data:', {
    customWeight: productData?.customWeight,
    customFit: productData?.customFit,
    hasProductData: !!productData,
    productKeys: productData ? Object.keys(productData) : 'null'
  });

  // Debug current values being used
  const weightValue = productData?.customWeight || fabricData?.weight || technicalSpecs['weight'] || technicalSpecs['fabric_weight'] || '120g/m²';
  const fitValue = productData?.customFit || technicalSpecs['fit'] || technicalSpecs['fit_type'] || 'Athletic Slim';

  console.log('[SpecificationAccordion] Final values - Weight:', weightValue, 'Fit:', fitValue);
  ...
  items.push(
    {
      label: 'Weight',
      value: productData?.customWeight || fabricData?.weight || technicalSpecs['weight'] || technicalSpecs['fabric_weight'] || '120g/m²'
    },
    {
      label: 'Fit',
      value: productData?.customFit || technicalSpecs['fit'] || technicalSpecs['fit_type'] || 'Athletic Slim'
    },
    {
      label: 'Construction',
      value: technicalSpecs['construction'] || technicalSpecs['seam_construction'] || 'Flatlock seams, reinforced stress points'
    }
  );
...
export const createTechnologySpecifications = (productData?: any): SpecificationSection => {
  ...
  if (Object.keys(technicalSpecs).length > 0) {
    const technologyKeys = Object.keys(technicalSpecs).filter(key =>
...
export const createCustomizationSpecifications = (productData?: any): SpecificationSection => {
  ...
  if (!customizationOptions || customizationOptions.length === 0) {
    return {
      ...
      items: [
        { label: 'Standard Options', value: 'Contact us for customization details' }
      ]
    };
  }
...
export const createSustainabilitySpecifications = (productData?: any): SpecificationSection => {
  ...
  const sustainabilityKeys = Object.keys(technicalSpecs).filter(key =>
...
export const createCareSpecifications = (productData?: any): SpecificationSection => {
  ...
  if (careInstructions.length > 0) {
    ...
    const careCategories = {
      washing: careInstructions.filter((instruction: string) =>
        instruction.toLowerCase().includes('wash') || instruction.toLowerCase().includes('clean')
      ),
      drying: careInstructions.filter((instruction: string) =>
        instruction.toLowerCase().includes('dry') || instruction.toLowerCase().includes('tumble')
      ),
      ironing: careInstructions.filter((instruction: string) =>
        instruction.toLowerCase().includes('iron') || instruction.toLowerCase().includes('press')
      ),
      bleaching: careInstructions.filter((instruction: string) =>
        instruction.toLowerCase().includes('bleach') || instruction.toLowerCase().includes('whiten')
      ),
      general: careInstructions.filter((instruction: string) =>
    ...
    if (careCategories.washing.length > 0) {
      items.push({ label: 'Washing', value: careCategories.washing.join('; ') });
    }
    ...
    if (careCategories.ironing.length > 0) {
      items.push({ label: 'Ironing', value: careCategories.ironing.join('; ') });
    }
    if (careCategories.bleaching.length > 0) {
      items.push({ label: 'Bleaching', value: careCategories.bleaching.join('; ') });
    }
    if (careCategories.general.length > 0) {
      items.push({ label: 'Special Care', value: careCategories.general.join('; ') });
    }
  ...
  const careKeys = Object.keys(technicalSpecs).filter(key =>
  ...
  careKeys.forEach(key => {
    ...
    if (!items.some(item => item.label.toLowerCase().includes(label.toLowerCase().split(' ')[0]!))) {
      items.push({
        label,
        value: technicalSpecs[key]
      });
