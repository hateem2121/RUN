...
const inquiryOptions = [
  { value: 'wholesale', label: 'Wholesale Pricing', description: 'Get bulk pricing information' },
  { value: 'customization', label: 'Custom Branding', description: 'Private label and customization' },
  { value: 'samples', label: 'Sample Request', description: 'Request product samples' },
  { value: 'bulk', label: 'Bulk Orders', description: 'Large quantity orders' },
  { value: 'partnership', label: 'Partnership', description: 'Distribution partnership' }
];
...
export function B2BContactForm({
  ...
  const form = useForm<B2BContactFormData>({
    ...
    defaultValues: {
      ...
      message: prefilledType === 'samples' && productName
  ...
  const handleSubmit = useCallback(async (data: B2BContactFormData) => {
  ...
  const handleRequestSamples = useCallback(() => {
    ...
    if (productName) {
      form.setValue('message', `I would like to request samples of the ${productName} for evaluation.`);
    }
  ...
  const handleDownloadSpecs = useCallback(() => {
    toast({
      title: 'Coming Soon',
      description: 'Technical specifications download will be available soon. Please contact our sales team for detailed specs.',
    });
  }, [toast]);
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">wholesale@runapparel.com</p>
              </div>
            </div>
          </div>
