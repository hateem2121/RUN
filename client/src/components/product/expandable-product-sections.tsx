...
function ExpandableSection({ title, icon, children, defaultOpen = false, accent = "blue" }: ExpandableSectionProps) {
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn("p-2 rounded-lg", `bg-${accent}-100 text-${accent}-700`)}>
              {icon}
            </div>
...
function MaterialSwatch({ name, color, pattern, isSelected = false, onClick }: {
  name: string;
  color?: string;
  pattern?: string;
  isSelected?: boolean;
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {isSelected && <CheckCircle className="h-4 w-4 text-blue-500 absolute top-1 right-1" />}
        </div>
      </div>
    </div>
  );
...
function ColorSwatch({ color, name, isSelected = false, onClick }: {
  color: string;
  name: string;
  isSelected?: boolean;
  onClick?: () => void;
  ...
  return (
    ...
    >
      ...
      />
      <p className="text-xs text-center mt-1 font-medium text-gray-700">{name}</p>
      {isSelected && (
...
export function ExpandableProductSections({ product, context, categoryColor = "blue" }: ProductSectionProps) {
  const [selectedMaterial, setSelectedMaterial] = useState<string | null>(null);
  const [selectedColor, setSelectedColor] = useState<string | null>(null);

  // Mock material and color data - in real app, this would come from product data
  const availableMaterials = [
    { name: "Cotton Blend", color: "#f8f9fa", composition: "80% Cotton, 20% Polyester" },
    { name: "Performance Mesh", color: "#e9ecef", composition: "100% Polyester with moisture-wicking" },
    { name: "Organic Cotton", color: "#f5f5dc", composition: "100% Organic Cotton" },
    { name: "Bamboo Fiber", color: "#f0f8ea", composition: "70% Bamboo, 30% Cotton" }
  ];
