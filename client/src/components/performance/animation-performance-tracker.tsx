...
export function AnimationPerformanceTracker({
  ...
  const startTracking = useCallback(() => {
    ...
    console.log(`📊 AnimationTracker[${componentName}]: Started tracking`);
  ...
  const stopTracking = useCallback(() => {
    ...
    console.log(`📊 AnimationTracker[${componentName}]: Final metrics`, {
      duration: `${metrics.current.animationDuration.toFixed(2)}ms`,
      averageFPS: `${metrics.current.averageFPS}fps`,
      frameDrops: metrics.current.frameDrops,
      memoryPeak: `${(metrics.current.peakMemoryUsage / 1024 / 1024).toFixed(2)}MB`,
      gsapComplexity: metrics.current.gsapComplexity,
      scrollTriggerEvents: metrics.current.scrollTriggerEvents
    });
  ...
  }, [enabled, componentName, trackGSAPComplexity, trackScrollTriggerEvents, onMetricsUpdate]);
...
declare global {
  interface Window {
    animationTrackers: Record<string, {
      start: () => void;
      stop: () => void;
