		...
		switch (contentType) {
			...
			default: {
				...
				const fallback = fallbackSizes[
					nestingLevel as keyof typeof fallbackSizes
				] || {
					width: "max-w-xl",
					height: "max-h-[70vh]",
				};
	...
	return (
		<EnhancedDialogPortal>
			...
			<DialogPrimitive.Content
				...
				className={cn(
					"fixed left-[50%] top-[50%] w-full translate-x-[-50%] translate-y-[-50%]",
					...
					nestingLevel > 0 &&
						"ring-2 ring-primary/20 shadow-2xl nested-modal-content",
					...
					<div ref={contentRef} tabIndex={-1} className="contents">
						...
						<DialogPrimitive.Close
							className={cn(
								...
								nestingLevel > 0 && "bg-background/80 border",
							)}
...
const EnhancedDialogBody = ({
	...
	return (
		<div
			className={cn(
				"flex-1 min-h-0 overflow-y-auto", // SOLE SCROLL OWNERSHIP
				className,
			)}
			{...props}
		>
...
export function NestedDialog({
	...
	return (
		<EnhancedDialog open={open} onOpenChange={onOpenChange}>
			{trigger && (
				<EnhancedDialogTrigger asChild>{trigger}</EnhancedDialogTrigger>
			)}
