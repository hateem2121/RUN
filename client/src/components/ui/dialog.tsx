...
function getZIndexClass(nestingLevel: number, isOverlay: boolean = false): string {
  if (isOverlay) {
    ...
    switch (nestingLevel) {
      case 0:
        return "z-modal-backdrop"; // Base overlay: z-index 400
      case 1:
        return "z-modal-overlay-1"; // Level 1 overlay: z-index 410
      case 2:
        return "z-modal-overlay-2"; // Level 2 overlay: z-index 420
      case 3:
        return "z-modal-overlay-3"; // Level 3 overlay: z-index 430
      default:
    ...
    switch (nestingLevel) {
      case 0:
        return "z-modal"; // Base content: z-index 500
      case 1:
        return "z-modal-nested"; // Level 1 content: z-index 550
      case 2:
        return "z-modal-nested-2"; // Level 2 content: z-index 560
      case 3:
        return "z-modal-nested-3"; // Level 3 content: z-index 570
      default:
        return "z-modal-nested-3"; // Maximum content nesting
    }
  }
}
...
const DialogContent = ({
  ...
  const modalSizeAndLayout = React.useMemo(() => {
    ...
    switch (contentType) {
      case "media-library":
        // Media library needs definite height for DialogBody scroll container
        return {
          width: nestingLevel === 0 ? "max-w-6xl" : "max-w-vw-90",
          height: "h-modal-lg",
          padding: "p-4",
          layout: "flex flex-col overflow-hidden", // Prevent content overflow, let DialogBody scroll
        };
  ...
  const positionStyles = React.useMemo(() => {
    ...
    return {
      ...
      // ARCHITECTURAL FIX: Remove maxHeight constraint for media-library to let h-modal-lg class work
  ...
  return (
    <DialogPortal>
      <DialogOverlay nestingLevel={nestingLevel} />
