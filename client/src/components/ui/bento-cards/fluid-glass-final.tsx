...
const FluidGlass = memo(function FluidGlass({
  ...
  const modeProps =
    mode === "lens" ? lensProps : mode === "bar" ? barProps : cubeProps;
  ...
  const initializeThreeJS = useCallback(() => {
    ...
    const animate = () => {
      ...
      if (glassRef.current) {
        // Smooth mouse follow
        glassRef.current.position.x +=
          (mouseRef.current.x * 5 - glassRef.current.position.x) * 0.1;
        glassRef.current.position.y +=
          (mouseRef.current.y * 5 - glassRef.current.position.y) * 0.1;
