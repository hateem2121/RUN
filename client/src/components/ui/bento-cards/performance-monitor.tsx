...
export function PerformanceMonitor({
	...
	const generateRecommendations = (metrics: PerformanceMetrics) => {
		...
		if (metrics.lcp > 2500) {
			recommendations.push(
				"Optimize image loading and reduce server response times",
			);
		}
		if (metrics.fid > 100) {
			recommendations.push(
		...
		if (metrics.memory > 50) {
			recommendations.push(
				"Implement memory cleanup and optimize Three.js instances",
			);
		}
		...
		<AnimatePresence>
			{isVisible && (
				<motion.div
					...
					exit={{ opacity: 0, y: 20 }}
					className="fixed top-4 right-4 w-80 bg-white/95 rounded-lg shadow-xl border border-gray-200 z-dock"
				>
					<div className="p-4">
						...
						<div className="mb-4 p-3 bg-gray-50 rounded-lg">
							<div className="flex items-center justify-between">
								...
								<span
									className={`text-lg font-bold ${getScoreColor(metrics.score)}`}
								>
