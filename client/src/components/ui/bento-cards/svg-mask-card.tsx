  ...
  useEffect(() => {
    const loadSvgMask = async () => {
      ...
      try {
        ...
        const svgWithAspectRatio = svgText.replace('<svg ', '<svg preserveAspectRatio="xMidYMid slice" ');
        ...
        console.error('[SVG Mask] Failed to load:', error);
      ...
      try {
        if (contentType === 'video') {
          ...
          video.onloadedmetadata = () => {
            ...
            console.log('[Content Media] Video metadata preloaded');
          };
          video.onerror = () => {
            setContentLoadError(true);
            console.warn('[Content Media] Video preload failed silently - using fallback');
          ...
          img.onload = () => {
            // setIsContentLoaded(true);
            console.log('[Content Media] Image preloaded');
          };
          img.onerror = () => {
            setContentLoadError(true);
            console.warn('[Content Media] Image preload failed silently - using fallback');
          };
          img.src = actualContentMediaUrl;
        }
      } catch (error) {
        console.error('[Content Media] Preload error:', error);
        setContentLoadError(true);
      }
    };
  ...
  return (
    <section
      ...
      style={{
        ...
        height: "auto",
        maxHeight: "600px",
        backgroundColor: hasError || contentLoadError ? '#f3f4f6' : actualContentMediaUrl ? 'transparent' : 'transparent',
    ...
    >
      ...
      {/* Video content */}
      {!hasError && !contentLoadError && (contentType === 'video' || contentType === 'unknown') && actualContentMediaUrl && (
        <video
          autoPlay
          muted
          loop
