...
export function MemoryCleanup() {
  useEffect(() => {
    const handleMemoryPressure = (event: CustomEvent) => {
      ...
      console.warn(`[Memory Cleanup] Pressure event from ${source}: ${usedMB}MB`);
      ...
      if ('gc' in window && typeof window.gc === 'function') {
        try {
          ...
          console.warn('[Memory Cleanup] GC not available');
        }
      }
      ...
      const clearLargeObjects = () => {
        ...
        modelViewers.forEach((viewer, index) => {
          if (index > 2) { // Keep only first 2 models loaded
            (viewer as any).src = '';
          }
        });
        ...
        images.forEach((img) => {
          ...
          if (!isVisible && img.src.includes('proxy') && img.dataset.size) {
            ...
            if (sizeBytes > 5 * 1024 * 1024) { // Clear images > 5MB
              img.src = '';
            }
          }
        });

        console.log('[Memory Cleanup] Cleared large objects from DOM');
      };
    ...
    const memoryMonitor = setInterval(() => {
      if ('memory' in performance && performance.memory) {
        ...
        if (usedMB > 300) {
          console.log(`[Memory Monitor] ${usedMB.toFixed(1)}MB used / ${limitMB.toFixed(1)}MB limit`);
        }
