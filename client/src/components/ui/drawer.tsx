...
const DrawerOverlay = ({
  ...
  <DrawerPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-modal-backdrop bg-black/80", // Using legacy z-50 equivalent for background
      className,
    )}
    {...props}
  />
...
const DrawerContent = ({
  ...
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "fixed inset-x-0 bottom-0 z-modal mt-24 flex h-auto flex-col rounded-t-[10px] border border-border bg-background", // Using z-50 equivalent
        className,
      )}
      {...props}
    >
