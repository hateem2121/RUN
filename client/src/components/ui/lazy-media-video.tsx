...
export function LazyMediaVideo({
  ...
  const handleVideoLoad = useCallback(() => {
    ...
    setHasError(false);
    onLoad?.();
    console.log(`[LazyMediaVideo] Successfully loaded asset ${asset.id}: ${asset.originalName}`);
  ...
  const handleVideoError = useCallback(() => {
    ...
    console.error("[LazyMediaVideo] Video loading error:", error);
    onError?.(error);
    ...
    if (videoRef.current) {
      ...
      if (playPromise !== undefined) {
        playPromise
          ...
          .catch((error) => {
            // Auto-play was prevented or interrupted - ignore silently
            console.debug("[LazyMediaVideo] Play interrupted (non-critical):", error.message);
          });
      } else {
        setShowPlayButton(false);
      }
    ...
    <div ref={containerRef} className={cn("relative overflow-hidden", className)} style={style}>
      ...
      {!isLoaded && (
        ...
        >
          ...
          {hasError && <span className="text-sm text-red-500">Failed to load video</span>}
          {!isLoading && !hasError && (
      ...
      {showPlayButton && isLoaded && (
        ...
        >
          <div className="bg-white/90 rounded-full p-4 hover:bg-white transition-all duration-200">
