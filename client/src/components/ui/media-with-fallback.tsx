...
function MediaPlaceholder({ type, className, error }: MediaPlaceholderProps) {
  return (
    <div className={cn("bg-gray-100 flex flex-col items-center justify-center p-8 rounded-lg", className)}>
      {type === "image" ? (
...
export function MediaWithFallback({
  ...
  const handleError = () => {
    console.warn(`[MediaWithFallback] Failed to load ${type}:`, src);
  ...
  if (isLoading && hasError) {
    return <MediaPlaceholder type={type} className={className} error="Failed to load" />;
  }
