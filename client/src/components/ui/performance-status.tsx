    ...
    <div className={`performance-status ${className}`}>
      ...
      >
        ...
        <div className="flex items-center space-x-1">
          ...
          <span className={`text-xs font-medium ${getScoreColor(metrics.score)}`}>
            {metrics.score}/100
          </span>
        </div>
      ...
      {showDetailed && (
        ...
        >
          <div className="bg-black/10 rounded p-2">
            ...
            <div className="text-white font-medium">{metrics.loadTime.toFixed(0)}ms</div>
          </div>
          <div className="bg-black/10 rounded p-2">
            <div className="text-white/50">Memory</div>
            <div className="text-white font-medium">{metrics.memoryUsage.toFixed(1)}MB</div>
          </div>
          <div className="bg-black/10 rounded p-2">
          ...
          <div className="bg-black/10 rounded p-2">
            ...
            <div className="text-white font-medium">{metrics.networkRequests}</div>
          </div>
        </motion.div>
      )}
    </div>
