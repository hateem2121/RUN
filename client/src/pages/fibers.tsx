...
export default function Fibers() {
  ...
  const groupedFibers = filteredFibers.reduce((acc, fiber) => {
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {typeFibers.map(fiber => {
                          ...
                          return (
                            ...
                            >
                              <Card className="h-full hover:shadow-lg transition-shadow-sm cursor-pointer"
                                ...
                                <CardContent>
                                  {fiber.properties && getPropertiesArray(fiber.properties).length > 0 && (
                                    <div className="flex flex-wrap gap-1 mb-3">
                                      {getPropertiesArray(fiber.properties).slice(0, 3).map((prop, idx) => (
                                        <Badge key={idx} variant="secondary" className="text-xs">
                                          {prop}
                                        </Badge>
                                      ))}
                                      {getPropertiesArray(fiber.properties).length > 3 && !isExpanded && (
                                        <Badge variant="outline" className="text-xs">
                                    ...
                                    {isExpanded && (
                                      ...
                                      >
                                        <div className="pt-3 space-y-3 border-t">
                                          {fiber.description && (
                                            <p className="text-sm text-gray-700">{fiber.description}</p>
                                          )}
                                          
                                          {fiber.properties && getPropertiesArray(fiber.properties).length > 3 && (
                                            <div className="space-y-1">
                                              <p className="text-xs font-semibold text-gray-700">All Properties:</p>
                                              <div className="flex flex-wrap gap-1">
                                                {getPropertiesArray(fiber.properties).map((prop, idx) => (
                                                  <Badge key={idx} variant="secondary" className="text-xs">
                                                    {prop}
                                                  </Badge>
                                                ))}
                                          ...
                                          {fiber.sustainabilityScore && (
                                            <div className="space-y-2">
                                              <p className="text-xs font-semibold text-gray-700">Sustainability Assessment:</p>
