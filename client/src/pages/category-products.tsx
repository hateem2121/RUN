...
export default function CategoryProductsPage() {
	...
	const { data: productsResponse, isLoading } = useQuery<{
		...
		queryFn: () =>
			apiRequest(`/api/products?category=${category?.id}&active=true`),
		enabled: !!category?.id,
	});
					...
					<>
						...
						{viewMode !== "small" && isHovered && (
							<>
								...
								>
									<ChevronLeft
										className={cn("w-4 h-4", isNavigating && "opacity-50")}
									/>
								</button>
								<button
									onClick={goToNext}
										...
										<div className="p-4 flex items-center justify-between">
											<div className="flex items-center gap-3">
												{subcat.imageUrl && (
													<img
														src={
															getOptimizedMediaUrl(parseInt(subcat.imageUrl)) ||
															subcat.imageUrl
														}
														alt={subcat.name}
												...
												<div>
													...
													<p className="text-sm text-gray-600">
														{
															products.filter((p) => p.categoryId === subcat.id)
																.length
														}{" "}
														products
													</p>
				...
				{isLoading ? (
					...
					<div className={cn("grid", gridClasses[viewMode])}>
						{sortedProducts.map((product) => {
							return (
								<Link
									href={`/categories/${slug}/${product.slug || product.id}`}
									key={product.id}
								>
