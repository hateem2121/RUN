...
export function LogoAnimationSettings() {
  ...
  const saveSettingsMutation = useMutation({
    mutationFn: async (data: Partial<LogoAnimationSettings>) => {
      return apiRequest('/api/logo-animation-settings', { method: 'PATCH', body: JSON.stringify(data) });
    },
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: ['/api/logo-animation-settings'] });
      toast({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Logo Animation Settings</h1>
          <p className="text-gray-500 mt-2">Configure SVG logo animations for the homepage landing</p>
        </div>
        <Button
          onClick={handleSave}
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-gray-500">Show animated logo on homepage loading</p>
                </div>
                <Switch
              ...
              <div className="space-y-2">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => setSettings({ ...settings, showFrequency: checked })}
                />
                <p className="text-sm text-gray-500">Show frequency indicator</p>
              </div>

              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label>Enable Skip Button</Label>
                  <p className="text-sm text-gray-500">Allow users to skip the animation</p>
                </div>
                <Switch
                  checked={settings.skipButtonEnabled ?? false}
                  onCheckedChange={(checked) => setSettings({ ...settings, skipButtonEnabled: checked })}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  <Slider
                    ...
                    onValueChange={([value]) => value !== undefined && setSettings({ ...settings, animationDurationMultiplier: value.toFixed(1) })}
                    className="flex-1"
                  />
                  <span className="w-12 text-sm text-right">{parseFloat(settings.animationDurationMultiplier ?? "1.0").toFixed(1)}x</span>
                </div>
                <p className="text-sm text-gray-500">Overall animation speed (0.5x = slower, 2x = faster)</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="draw" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure the initial line drawing effect using createDrawable</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  <Slider
                    ...
                    className="flex-1"
                  />
                  <span className="w-16 text-sm text-right">{settings.duration ?? 2000}ms</span>
                </div>
                <p className="text-sm text-gray-500">Total animation duration</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  <Slider
                    ...
                    onValueChange={([value]) => value !== undefined && setSettings({ ...settings, drawStagger: value.toFixed(0) })}
                    className="flex-1"
                  />
                  <span className="w-16 text-sm text-right">{parseFloat(settings.drawStagger ?? "100").toFixed(0)}ms</span>
                </div>
                <p className="text-sm text-gray-500">Delay between drawing each path element</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-500">Animation easing curve</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-gray-500">Show floating animated elements after logo draw</p>
                </div>
                <Switch
                  checked={settings.motionEnabled ?? true}
                  onCheckedChange={(checked) => setSettings({ ...settings, motionEnabled: checked })}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setSettings({ ...settings, motionElements: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                ...
                />
                <p className="text-sm text-gray-500">Elements to animate (string array)</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  <Slider
                    ...
                    onValueChange={([value]) => value !== undefined && setSettings({ ...settings, motionSpeed: value.toFixed(0) })}
                    className="flex-1"
                    disabled={!settings.motionEnabled}
                  />
                  <span className="w-16 text-sm text-right">{parseFloat(settings.motionSpeed ?? "3000").toFixed(0)}ms</span>
                </div>
                <p className="text-sm text-gray-500">Duration of motion path animations</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="flex h-10 w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm placeholder:text-gray-500 focus:outline-hidden focus:ring-2 focus:ring-gray-950 focus:ring-offset-2"
                />
                <p className="text-sm text-gray-500">Additional CSS class for custom styling</p>
              </div>

              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label>Debug Mode</Label>
                  <p className="text-sm text-gray-500">Show animation phase indicators and timing info</p>
                </div>
                <Switch
              ...
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
                <h4 className="font-medium text-amber-900 mb-2">Animation Phases</h4>
                ...
                <p className="text-sm text-amber-700 mt-3">
                  Animations load homepage content in the background to improve perceived performance.
                </p>
              </div>
            </CardContent>
          </Card>
