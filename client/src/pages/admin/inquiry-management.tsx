...
import { apiRequest, getOptimizedQueryOptions, getQueryClient, queryKeys } from "@/lib/queryClient";
    ...
    onSuccess: () => {
      getQueryClient().invalidateQueries({
        predicate: (query) => {
          ...
          return typeof key === 'string' && key.startsWith('/api/admin/inquiries');
        }
      });
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card data-testid="card-stats-new">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-3xl">{stats?.byStatus?.new || 0}</CardTitle>
            </CardHeader>
          </Card>
          <Card data-testid="card-stats-read">
            <CardHeader className="pb-2">
              <CardDescription>Read</CardDescription>
              <CardTitle className="text-3xl">{stats?.byStatus?.read || 0}</CardTitle>
            </CardHeader>
          </Card>
          <Card data-testid="card-stats-responded">
            <CardHeader className="pb-2">
              <CardDescription>Responded</CardDescription>
              <CardTitle className="text-3xl">{stats?.byStatus?.responded || 0}</CardTitle>
            </CardHeader>
          </Card>
          <Card data-testid="card-stats-recent">
            <CardHeader className="pb-2">
              <CardDescription>Last 7 Days</CardDescription>
              <CardTitle className="text-3xl">{stats?.recentCount || 0}</CardTitle>
            </CardHeader>
          </Card>
        </div>
          ...
          <CardContent>
            {isLoading ? (
              <div className="text-center py-8">
                ...
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Loading inquiries...</p>
              </div>
            ) : inquiries.length === 0 ? (
                    ...
                    <TableBody>
                      {inquiries.map((inquiry: Inquiry) => (
                        <TableRow key={inquiry.id} data-testid={`row-inquiry-${inquiry.id}`}>
                          ...
                          <TableCell>
                            <button
                              onClick={() => setEditingStatus({ id: inquiry.id, status: inquiry.status, notes: inquiry.adminNotes || "" })}
        ...
        <Dialog open={!!selectedInquiry} onOpenChange={() => setSelectedInquiry(null)}>
          <DialogContent className="max-w-2xl" data-testid="dialog-inquiry-detail">
            <DialogHeader>
              <DialogTitle>Inquiry Details</DialogTitle>
              <DialogDescription>
                Submitted on {selectedInquiry && new Date(selectedInquiry.submittedAt).toLocaleString()}
              </DialogDescription>
            </DialogHeader>
            {selectedInquiry && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>Country</Label>
                    <p className="font-medium" data-testid="text-detail-country">{selectedInquiry.country || "-"}</p>
                  </div>
                  <div>
                    <Label>Preferred Platform</Label>
                    <p className="font-medium">{selectedInquiry.preferredPlatform || "-"}</p>
                  </div>
                  <div>
                    <Label>Status</Label>
                    <Badge className={statusColors[selectedInquiry.status] || ""}>{selectedInquiry.status}</Badge>
                  </div>
                </div>
                <div>
                  <Label>Message</Label>
                  <p className="mt-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-md whitespace-pre-wrap" data-testid="text-detail-message">
          ...
          <DialogContent data-testid="dialog-edit-status">
            ...
            {editingStatus && (
              <div className="space-y-4">
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="outline" onClick={() => setEditingStatus(null)} data-testid="button-cancel-edit">
                    Cancel
                  </Button>
                  <Button
        ...
        <AlertDialog open={!!deleteInquiryId} onOpenChange={() => setDeleteInquiryId(null)}>
          <AlertDialogContent data-testid="dialog-delete-confirm">
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => deleteInquiryId && deleteMutation.mutate(deleteInquiryId)}
