  ...
  return (
    <div className="space-y-2 max-h-60 overflow-y-auto border rounded-lg p-3">
      {activeCertificates.length === 0 ? (
        <p className="text-sm text-muted-foreground">No active certificates available. Add certificates in the Certificates management page.</p>
      ) : (
        activeCertificates.map((cert) => (
          ...
          >
            <div className={`w-4 h-4 border rounded ${selectedIds.includes(cert.id) ? 'bg-primary border-primary' : 'border-input'}`}>
