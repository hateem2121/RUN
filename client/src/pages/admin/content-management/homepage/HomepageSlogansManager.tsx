...
import type { HomepageSection, HomepageSlogan } from "@shared/schema";
...
export function HomepageSlogansManager({
	...
	const createSloganMutation = useMutation({
		mutationFn: async (data: {
			...
			return await apiRequest("/api/homepage-slogans", {
				method: "POST",
				body: data,
			});
		},
	...
	const updateSloganMutation = useMutation({
		mutationFn: async ({
			...
			id: number;
			data: Partial<HomepageSlogan>;
		}) => {
			return await apiRequest(`/api/homepage-slogans/${id}`, {
				method: "PATCH",
				body: data,
			});
		},
		onMutate: async ({ id, data }) => {
			...
			const previousSlogans = getQueryClient().getQueryData([
				"/api/homepage-slogans",
			]);

			// Optimistically update
			getQueryClient().setQueryData<HomepageSlogan[]>(
				["/api/homepage-slogans"],
				(old) => {
					if (!Array.isArray(old)) return old;
					return old.map((slogan) =>
						slogan.id === id ? { ...slogan, ...data } : slogan,
					);
				},
			);
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		const isValid = updates.every((update) => {
			const hasValidId =
				typeof update.id === "number" && Number.isFinite(update.id);
		...
		<div className="space-y-6">
			...
			{sectionData && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Manage the visibility and generic title of this section on the
							homepage.
						</CardDescription>
					</CardHeader>
						...
						<div className="flex items-center justify-between">
							...
							<Switch
								...
								onCheckedChange={(checked) =>
									handleUpdateSection({ isActive: checked })
								}
							/>
						</div>
						<div className="grid gap-2">
							<Label htmlFor="section-title">
								Section Title (Internal/Display)
							</Label>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Manage animated text slogans that appear on the homepage. Drag to
						reorder.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
												...
												<div className="flex items-center gap-3 p-3 border rounded-lg bg-card">
													...
													<div className="flex items-center gap-2">
														...
														<Button
															...
															size="sm"
															onClick={() =>
																deleteSloganMutation.mutate(slogan.id)
															}
														>
