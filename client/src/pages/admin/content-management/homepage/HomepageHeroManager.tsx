...
export function HomepageHeroManager({
	...
	const updateHeroMutation = useMutation({
		...
		onMutate: async (data) => {
			...
			const previousHero = getQueryClient().getQueryData([
				"/api/homepage-hero",
			]);

			// Optimistically update
			getQueryClient().setQueryData<HomepageHero | null>(
				["/api/homepage-hero"],
				(old) => {
					if (!old) return old;
					return { ...old, ...data };
					...
					<div className="grid gap-4">
							...
							<div className="flex items-center gap-2">
								...
								{heroForm.backgroundImageId && (
									<Button
										...
										size="sm"
										onClick={() =>
											setHeroForm({ ...heroForm, backgroundImageId: null })
										}
									>
										<X className="h-4 w-4" />
									</Button>
						...
						<div className="flex justify-end pt-4">
							<Button
								...
								disabled={
									updateHeroMutation.isPending || !heroForm.title?.trim()
								}
							>
