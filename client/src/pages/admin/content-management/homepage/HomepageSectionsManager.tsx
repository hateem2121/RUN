...
export function HomepageSectionsManager({ sections }: Readonly<HomepageSectionsManagerProps>) {
  ...
  const updateSectionMutation = useMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      await getQueryClient().cancelQueries({ queryKey: ["/api/homepage-sections"] });

      // Snapshot previous value
      const previousSections = getQueryClient().getQueryData(["/api/homepage-sections"]);

      // Optimistically update
      getQueryClient().setQueryData<HomepageSection[]>(["/api/homepage-sections"], (old) => {
    ...
    onError: (_err, _variables, context) => {
      if (context?.previousSections) {
        getQueryClient().setQueryData(["/api/homepage-sections"], context.previousSections);
      }
      toast({
        title: "Error",
  ...
  const handleSaveSection = (section: HomepageSection) => {
    ...
    if (typeof section.id !== "number" || !Number.isFinite(section.id)) {
      console.error(
        "[HomepageSectionsManager] Invalid section ID:",
        section.id,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Enable/disable and configure various homepage sections</CardDescription>
      </CardHeader>
      <CardContent>
                ...
                <div className="flex-1">
                  {editingSectionId === section.id ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <Label htmlFor={`section-description-${section.id}`}>Content</Label>
                        <Textarea
                    ...
                    // View Mode
                    <div>
                      <h3 className="font-semibold text-lg">{section.title || section.name}</h3>
