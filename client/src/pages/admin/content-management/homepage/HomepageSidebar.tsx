		...
		<div className="h-full flex flex-col gap-2">
			...
			<ScrollArea className="flex-1 px-2">
				<div className="space-y-1 py-2">
					{menuItems.map((item) => {
						const dbSection = item.isDbSection
							? getSectionData(item.key)
							: null;
								...
								{dbSection && (
									...
									>
										<Switch
											...
											onCheckedChange={(val) =>
												onToggleVisibility(dbSection.id, val)
											}
					...
					{sections
						.filter(
							(s) =>
								!menuItems.some((item) => item.key === s.name.toLowerCase()),
						)
						.map((section) => {
								...
								>
									...
									>
										<Switch
											...
											onCheckedChange={(val) =>
												onToggleVisibility(section.id, val)
											}
											className="scale-75"
										/>
									</div>
