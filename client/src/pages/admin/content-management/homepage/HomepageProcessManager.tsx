...
export function HomepageProcessManager({
  ...
  const createProcessCardMutation = useMutation({
    mutationFn: async (
      ...
      return await apiRequest("/api/homepage-process-cards", { method: "POST", body: data });
    },
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: ["/api/homepage-process-cards/admin"] });
  ...
  const updateProcessCardMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<HomepageProcessCard> }) => {
      return await apiRequest(`/api/homepage-process-cards/${id}`, { method: "PATCH", body: data });
    },
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: ["/api/homepage-process-cards/admin"] });
      getQueryClient().invalidateQueries({ queryKey: ["/api/homepage-batch"] });
  ...
  const deleteProcessCardMutation = useMutation({
    mutationFn: async (id: number) => {
      return await apiRequest(`/api/homepage-process-cards/${id}`, { method: "DELETE" });
    },
    onSuccess: () => {
      getQueryClient().invalidateQueries({ queryKey: ["/api/homepage-process-cards/admin"] });
      getQueryClient().invalidateQueries({ queryKey: ["/api/homepage-batch"] });
          ...
          <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="grid gap-4">
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setProcessCardForm({ ...processCardForm, title: e.target.value })
                        }
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          setProcessCardForm({ ...processCardForm, description: e.target.value })
                        }
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={(val) =>
                            setProcessCardForm({ ...processCardForm, category: val })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue />
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="outline" onClick={() => setShowMediaPicker(true)}>
                          <Plus className="h-4 w-4 mr-2" /> Select Image
                        </Button>
            ...
            <StandardMediaSelectionDialog
              ...
              onSelect={(asset: MediaAsset | MediaAsset[]) => {
                ...
                if (single) setProcessCardForm({ ...processCardForm, iconMediaId: single.id });
              }}
              title="Select Icon"
