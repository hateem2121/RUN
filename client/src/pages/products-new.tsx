...
function useProductsPageData({
	...
	const displayedProducts = useMemo(() => {
		...
		if (selectedFilters.certificates.length > 0) {
			filteredProducts = filteredProducts.filter((p) =>
				p.certificateIds?.some((id: number) =>
					selectedFilters.certificates.includes(id),
				),
			);
		}
		...
		if (selectedFilters.accessories.length > 0) {
			filteredProducts = filteredProducts.filter((p) =>
				p.accessoryIds?.some((id: number) =>
					selectedFilters.accessories.includes(id),
				),
			);
		}
		...
		if (activeTab !== "ALL") {
			const categoryLabel = categoryTabs.find(
	...
	const gearProducts = useMemo(
		() =>
			transformedProducts.filter(
				(p) => p.category && p.category.toLowerCase().includes("gear"),
			),
		[transformedProducts],
	);
...
const Breadcrumbs = ({
	...
	<nav
		...
		className="max-w-[1440px] mx-auto px-6 pt-4 md:pt-6 lg:pt-8"
	>
			...
			{bannerItems.map((item, index) => (
				<div key={index} className="flex items-center space-x-12">
					...
					{index < bannerItems.length - 1 && (
						<AsteriskIcon className="w-10 h-10 shrink-0" />
					)}
				</div>
			))}
		</div>
		...
		>
			<CardContent className="p-0">
				<div className="aspect-[4/3] bg-gray-50/50 overflow-hidden relative flex items-center justify-center">
					{product.imageId &&
					(isHovered && product.hoverImageId ? product.hoverImageId : true) ? (
						<OptimizedImage
							mediaId={
...
const StatsSection = () => {
	const stats = [
		...
		},
		{
			...
			description:
				"Four decades of manufacturing expertise, now powered by 3D design technology.",
		},
		{
			value: "100%",
			label: "SMETA Audited",
...
const GearSection = ({ gearProducts, onProductClick }: GearSectionProps) => {
	...
	const handlePrev = () =>
		setCurrentIndex((p) => (p === 0 ? gearProducts.length - 1 : p - 1));
	const handleNext = () =>
		setCurrentIndex((p) => (p === gearProducts.length - 1 ? 0 : p + 1));
			...
			<div className="max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
				...
				<div>
					...
					<p className="mt-6 text-gray-700 max-w-md">
						From moisture-wicking caps to anatomically designed socks, our gear
						is engineered with the same commitment to quality and sustainability
						as our apparel. Elevate your run with accessories designed to
						perform.
					</p>
					<div className="mt-12">
						{currentProduct && (
		...
		<div className="max-w-[1440px] mx-auto px-6">
			...
			<div className="flex justify-center mt-12">
				<p className="max-w-3xl text-center text-gray-700">
					We are committed to ethical manufacturing and sustainable sourcing.
					Our partnerships with certified suppliers ensure that every product we
					create not only meets the highest performance standards but also
					contributes to a better future for our planet and its people.
				</p>
			</div>
		</div>
	</section>
	...
	<section className="relative bg-black text-white py-32 lg:py-48 px-6">
		...
		<div className="relative max-w-4xl mx-auto text-center">
			...
			<p className="mt-8 text-gray-300 max-w-2xl mx-auto">
				We believe in the power of running to change lives and the
				responsibility to protect the planet we run on. Our apparel is a fusion
				of cutting-edge performance technology and sustainable innovation, built
				to support your every stride and honor our collective future.
			</p>
		</div>
	</section>
);
