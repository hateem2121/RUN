...
export class MigrationService {
   ...
   */
  private async migrateContentEntities(results: Record<string, unknown>, errors: string[]): Promise<void> {
    try {
      ...
      logger.info(`📋 Found content entities - Hero: ${homepageHero ? 1 : 0}, Slogans: ${homepageSlogans?.length || 0}, Sections: ${homepageSections?.length || 0}, Process Cards: ${homepageProcessCards?.length || 0}`);
      ...
      results.contentEntities = (homepageHero ? 1 : 0) + (homepageSlogans?.length || 0) + (homepageSections?.length || 0) + (homepageProcessCards?.length || 0);
   ...
   */
  private async migrateSustainabilityEntities(results: Record<string, unknown>, errors: string[]): Promise<void> {
    try {
      ...
      logger.info(`📋 Found sustainability entities - Hero: ${sustainabilityHero ? 1 : 0}, Metrics: ${sustainabilityMetrics.length}, Manufacturing: ${manufacturingProcesses.length}, Technology: ${technologyInnovations.length}`);
      ...
      results.sustainabilityEntities = (sustainabilityHero ? 1 : 0) + sustainabilityMetrics.length + manufacturingProcesses.length + technologyInnovations.length;
   ...
   */
  private async migrateAdminEntities(results: Record<string, unknown>, errors: string[]): Promise<void> {
    try {
      ...
      logger.info(`📋 Found admin entities - Size Charts: ${sizeCharts.length}, Accessories: ${accessories.length}, Folders: ${folders.length}, Navigation: ${navigationItems.length}`);
      ...
      results.adminEntities = sizeCharts.length + accessories.length + folders.length + navigationItems.length;
  ...
  async reconfigureKeyValueStore(): Promise<{
    ...
    try {
      ...
      const cacheTypes = [
        "homepage", "navigation", "session", "cache",
        "performance", "temp", "settings"
      ];

      // Items to remove (migrated to PostgreSQL/App Storage)
      const migrateTypes = [
        "mediaAssets", "categories", "products", "fabrics",
        "fibers", "certificates", "accessories"
      ];
  ...
  async getMigrationStatus(): Promise<{
    ...
    try {
      ...
      const migratedMedia = mediaAssets.filter(asset =>
        asset.url && asset.url.includes('storage.googleapis.com')
      ).length;
