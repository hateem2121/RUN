...
class IntegrationTestSuite {
  ...
  private async testHomepageLoad(): Promise<TestResult> {
    ...
    try {
      ...
      const requiredKeys = ['hero', 'slogans', 'sections', 'featuredProductsSettings', 'processCards'];
      ...
      const avgDuration = endpointResults.reduce((sum, r) => sum + r.duration, 0) / endpointResults.length;
    ...
    try {
      ...
      return {
        ...
        details: {
          ...
          productDetailResponseTime: productDetailDuration ? productDetailDuration.toFixed(0) + 'ms' : 'N/A'
        }
      };
