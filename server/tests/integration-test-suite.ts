...
class IntegrationTestSuite {
	...
	private async testHomepageLoad(): Promise<TestResult> {
		...
		try {
			...
			const avgDuration =
				endpointResults.reduce((sum, r) => sum + r.duration, 0) /
			...
			return {
				...
				details: {
					...
					endpoints: endpointResults.map(
						(r) => `${r.endpoint}: ${r.duration.toFixed(0)}ms`,
					),
		...
		try {
			...
			if (media.data.data.length > 0) {
				...
				const content = await this.request(
					`/api/media/${firstMedia.id}/content`,
				);
			...
			return {
				...
				details: {
					...
					contentResponseTime: contentDuration
						? contentDuration.toFixed(0) + "ms"
						: "N/A",
				},
			};
		} catch (error) {
	...
	private async testDatabasePerformance(): Promise<TestResult> {
		...
		try {
			...
			if (legacy.avgResponseTime > 1000) {
				throw new Error(
					`Database avg response time too high: ${legacy.avgResponseTime}ms`,
				);
			}
		...
		try {
			...
			return {
				...
				details: {
					...
					databaseAvgResponseTime:
						health.database.avgResponseTime.toFixed(0) + "ms",
	...
	private logResult(result: TestResult): void {
		...
		logger.info(
			`${icon} ${result.name}: ${status} (${result.duration.toFixed(0)}ms)`,
		);
