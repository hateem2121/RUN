...
describe("Admin Operations Integration Tests", () => {
  ...
  describe("Maintenance Routes", () => {
    ...
    it("should trigger media fix with valid body", async () => {
      (adminService.fixCorruptedMedia as any).mockResolvedValue({
        fixedCount: 5,
    ...
    it("should trigger system cleanup", async () => {
      (adminService.triggerCleanup as any).mockResolvedValue({
        cleanedFiles: [],
        orphanedFiles: [],
        spaceSaved: 0,
      });
    ...
    it("should update audit configuration", async () => {
      (adminService.updateAuditConfig as any).mockResolvedValue(true);
  ...
  describe("Restore Operations", () => {
    ...
    it("should restore a deleted category", async () => {
      (adminService.restoreCategory as any).mockResolvedValue(true);
    ...
    it("should restore a deleted product", async () => {
      (adminService.restoreProduct as any).mockResolvedValue(true);
