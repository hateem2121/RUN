...
class PerformanceRegressionSuite {
	...
	private async testGetProductByPathContext(): Promise<TestResult> {
		...
		try {
			...
			const productsResp = await this.request(
				"/api/products?limit=1&active=true",
			);
			if (
				!productsResp.data ||
				!productsResp.data.data ||
				productsResp.data.data.length === 0
			) {
			...
			const productDetail = await this.request(
			...
			if (!productDetail.data.urlPath) {
				return {
					...
					details: {
						message:
							"No urlPath available for testing - endpoint structure validated via other tests",
					},
				};
			}
			...
			const response = await this.request(
				`/api/products/by-path?path=${encodeURIComponent(urlPath)}`,
			);
			...
			const missingFields = expectedContextFields.filter(
			...
			if (context.product) {
				...
				if (missingProductFields.length > 0) {
					throw new Error(
						`Product missing fields: ${missingProductFields.join(", ")}`,
					);
				}
			}
	...
	private async testGetMediaAssetsSearch(): Promise<TestResult> {
		...
		try {
			...
			if (mediaAssets.length > 0) {
				...
				const hasSearchTerm = mediaAssets.some(
					(asset) =>
	...
	private async testGetProductsPerformance(): Promise<TestResult> {
		...
		try {
			...
			return {
				...
				error: passed
					? undefined
					: `p95 (${benchmarkResult.p95.toFixed(2)}ms) exceeds threshold (${threshold}ms)`,
			};
		} catch (error) {
			return {
				name: "getProducts() performance (p95 < 200ms)",
		...
		try {
			...
			if (categories.length > 0) {
				...
				const missingFields = requiredFields.filter(
					(field) => !Object.hasOwn(category, field),
				);

				if (missingFields.length > 0) {
					throw new Error(
						`Category missing fields: ${missingFields.join(", ")}`,
					);
				}
			}
	...
	async runAllTests(): Promise<void> {
		const tests = [
			{ name: "Column Validation", fn: () => this.testGetProductsColumns() },
			{
				...
				fn: () => this.testGetProductByPathContext(),
			},
			...
			{ name: "Media Search", fn: () => this.testGetMediaAssetsSearch() },
			{
				...
				fn: () => this.testGetProductsPerformance(),
			},
			{
				name: "Media Performance",
				fn: () => this.testGetMediaAssetsPerformance(),
			},
			{
				name: "Homepage Performance",
				fn: () => this.testHomepageBatchPerformance(),
			},
			{
				name: "Categories Structure",
				fn: () => this.testGetCategoriesStructure(),
			},
		];
...
if (import.meta.url === `file://${process.argv[1]}`) {
	...
	suite
		...
		.then(() => {
			...
			logger.info(
				`[PerformanceTests] All tests completed. Status: ${passedAll ? "PASS" : "FAIL"}`,
			);
