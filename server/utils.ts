...
export function validateIdParam(req: Request, res: Response, paramName: string = 'id', context: string = 'entity'): number | null {
  const requestInfo = {
    method: req.method,
  ...
  if (!result.isValid) {
    ...
    if (result.originalValue?.includes('-')) {
      logger.error(`[RouteConfusion] ${context.toUpperCase()}_SLUG_TO_ID_ROUTE: Client sent slug '${result.originalValue}' to ID-based endpoint ${requestInfo.url}`);
    }
...
export class UrlPathService {
  ...
  generateBreadcrumbs(product?: { name: string; slug: string }, category?: { name: string; slug: string }): Array<{ name: string; url: string }> {
    const breadcrumbs = [];
    if (category) {
...
export const MediaValidator = {
  validateFilename: (filename: string): boolean => {
    ...
    const allowedExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.mp4', '.webm', '.glb', '.gltf'];
