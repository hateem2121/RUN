...
export class FileSecurityScanner {
   ...
   */
  static scanBuffer(buffer: Buffer, filename: string, mimeType: string): { 
    isSafe: boolean; 
    threats: string[]; 
    confidence: number;
  } {
    ...
    try {
      ...
      if (buffer.length > 500 * 1024 * 1024) { // 500MB max for any file
        threats.push('File exceeds maximum allowed size (500MB)');
        confidence -= 50; // Increased penalty
      } else if (buffer.length > 250 * 1024 * 1024) { // 250MB warning threshold
      ...
      const isMediaFile = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'svg', 'mp4', 'webm', 'glb', 'gltf'].includes(extension || '');
      
      // For media files, only do basic validation to avoid false positives
      if (isMediaFile) {
        console.log(`Security scan: ${filename} identified as media file (${extension})`);
        ...
        if (!foundCriticalThreat) {
          console.log(`Security scan: ${filename} passed media file validation`);
        }
      } else {
        ...
        for (const signature of FileSecurityScanner.MALICIOUS_SIGNATURES) {
          if (signature instanceof Buffer) {
            ...
            if (signature.test(textContent)) {
              threats.push(`Suspicious code pattern detected: ${signature.source.substring(0, 50)}`);
              confidence -= 25;
            }
          }
        }
   ...
   */
  static validateFilename(filename: string): { isValid: boolean; reason?: string } {
    // Check for suspicious characters
    const suspiciousChars = /[<>:"|?*\x00-\x1F]/;
    if (suspiciousChars.test(filename)) {
      return { isValid: false, reason: 'Filename contains suspicious characters' };
    }
