...
export async function processImage(fileBuffer: Buffer, originalFilename: string): Promise<ImageMetadata> {
  try {
    ...
    if (isSvg) {
      ...
      console.log(`SVG file processed: ${originalFilename}, dimensions: ${width}x${height}`);
...
export async function generateResponsiveVariants(
  ...
  async function compressToTarget(
    ...
    while (compressed.length > targetSizeKB * 1024 && quality > 55) { // Reduced from 60 to 55
      quality -= 5;
      compressed = await sharp(buffer)
        .resize(width, null, { fit: 'inside', withoutEnlargement: true })
  ...
  const thumbnailPath = generateOrganizedStoragePath('images', `${randomString}-thumb-${sanitizedBaseName}.webp`);
  const mediumPath = generateOrganizedStoragePath('images', `${randomString}-med-${sanitizedBaseName}.webp`);
  const largePath = generateOrganizedStoragePath('images', `${randomString}-lg-${sanitizedBaseName}.webp`);
  const originalPath = generateOrganizedStoragePath('images', `${randomString}-orig-${sanitizedBaseName}.webp`);
