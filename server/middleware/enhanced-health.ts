		...
		if (circuitStatus.state !== "OPEN") {
			try {
				...
				const probePromise = (async () => {
					...
					const sentinelData = Buffer.from(
						JSON.stringify({
							timestamp: Date.now(),
							probe: "health-check",
						}),
					...
					const readData = await appStorageService.downloadAsset(
						`public/${sentinelKey}`,
					);
				...
				const responseTime = Date.now() - start;
				const successRate =
					circuitStatus.totalSuccesses + circuitStatus.totalFailures > 0
				...
				const responseTime = Date.now() - start;
				const isTimeout =
					probeError instanceof Error && probeError.message.includes("timeout");
				const isPermission =
					probeError instanceof Error &&
					probeError.message.includes("permission");
	...
	try {
		...
		try {
			...
			if (rawStatus) {
				...
				circuitStatus = {
					...rawStatus,
					lastStateChange: rawStatus.lastStateChange
						? rawStatus.lastStateChange
						: null,
				};
		...
		const circuitOpen =
			circuitBreakerAvailable && circuitStatus.state === "OPEN";
		const circuitHalfOpen =
			circuitBreakerAvailable && circuitStatus.state === "HALF_OPEN";
		...
		if (config.app.environment === "production") {
			logger.info(
				`[Health] Overall: ${overallStatus} (${Date.now() - startTime}ms)`,
			);
			...
			checks
				...
				.forEach((check) => {
					logger.error(
						`[Health] ❌ ${check.service}: ${check.details?.error || "Unhealthy"}`,
					);
				});
		}
