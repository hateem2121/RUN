...
class SmartLogger {
  ...
  private constructor() {
    ...
    this.logLevel = levelMap[logging.level] || (this.isDevelopment ? this.LOG_LEVELS.DEBUG : this.LOG_LEVELS.WARN);
  }
  ...
  private readonly SENSITIVE_KEYS = [
    ...
    'private_key', 'privatekey', 'key', 'credential', 'credentials',
    'database_url', 'db_url', 'connection_string', 'connectionstring',
    'smtp_password', 'email_password', 'oauth_secret', 'client_secret'
  ];
  ...
  private formatStructured(
    ...
    const sanitizedMessage = typeof message === 'string' 
      ? this.sanitize(message) as string 
      : message;
    ...
    if (error) {
      ...
      const hasErrorMetadata = typeof metadata === 'object' && metadata !== null && 
   ...
   */
  performance(message: string, metadata?: unknown, error?: Error, isCritical: boolean = false): void {
