...
export async function checkSchemaConsistency(): Promise<SchemaValidationResult> {
  ...
  try {
    ...
    logger.info(`[Schema Validator] Found ${dbTableNames.length} tables in database`);

    // Check for missing tables
    for (const tableName of drizzleTableNames) {
      if (!dbTableNames.includes(tableName)) {
        errors.push(`Missing table: "${tableName}" exists in Drizzle schema but not in database`);
    ...
    for (const tableName of dbTableNames) {
      if (!drizzleTableNames.includes(tableName) && !tableName.startsWith('drizzle_')) {
        warnings.push(`Extra table: "${tableName}" exists in database but not in Drizzle schema`);
      }
    }
    ...
    logger.error('[Schema Validator] Failed to check schema consistency:', error);
    throw error;
  }
}
...
async function validateTableColumns(
  ...
  try {
    ...
    for (const colKey of drizzleColumns) {
      ...
      if (!dbColumn) {
        errors.push(`Missing column: "${tableName}"."${colName}" exists in Drizzle schema but not in database`);
        ...
        if (!typesMatch(drizzleType, dbType)) {
          warnings.push(`Type mismatch: "${tableName}"."${colName}" - Drizzle: ${drizzleType}, Database: ${dbType}`);
          summary.typeMismatches++;
        }

        // Validate nullable constraint
        const isNullable = dbColumn.is_nullable === 'YES';
        const hasNotNull = colDef.notNull !== undefined ? colDef.notNull : false;
        
        if (hasNotNull && isNullable) {
          warnings.push(`Nullability mismatch: "${tableName}"."${colName}" - Drizzle expects NOT NULL but database allows NULL`);
        }
      }
    }
    ...
    for (const dbCol of dbColumns) {
      ...
      if (!exists) {
        warnings.push(`Extra column: "${tableName}"."${dbCol.column_name}" exists in database but not in Drizzle schema`);
      }
    }
  } catch (error) {
    logger.error(`[Schema Validator] Error validating columns for table "${tableName}":`, error);
  }
}
...
async function validateTableIndexes(
  ...
  try {
    ...
    logger.debug(`[Schema Validator] Found ${dbIndexes.length} indexes for table "${tableName}" (informational only)`);
    ...
    if (dbIndexes.length === 0) {
      warnings.push(`No custom indexes found for table "${tableName}" - verify if indexes are needed for query performance`);
    }
    ...
    logger.error(`[Schema Validator] Error validating indexes for table "${tableName}":`, error);
  }
}
...
async function listForeignKeys(
  ...
  try {
    ...
    logger.info(`[Schema Validator] Found ${foreignKeys.length} foreign key constraints (informational only)`);

    // Log foreign keys for reference
    foreignKeys.forEach(fk => {
      logger.debug(`  - ${fk.table_name}.${fk.column_name} → ${fk.foreign_table_name}.${fk.foreign_column_name}`);
    });
  } catch (error) {
...
function typesMatch(drizzleType: string, dbType: string): boolean {
  ...
  for (const [drizzle, dbTypes] of Object.entries(typeMap)) {
    if (dbTypes.some(t => normalizedDb.includes(t)) && normalizedDrizzle.includes(drizzle)) {
      return true;
    }
  }
