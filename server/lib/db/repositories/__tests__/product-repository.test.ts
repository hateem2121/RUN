...
describe("ProductRepository", () => {
  ...
  describe("createProduct", () => {
    it("inserts a new product and returns it", async () => {
      ...
      const result = await repository.createProduct(productData as any);
      expect(result).toEqual(returnedProduct);
    });

    it("throws error if insertion fails", async () => {
      insertChain.then.mockImplementation((res) => res([]));
      await expect(repository.createProduct({} as any)).rejects.toThrow("Failed to create product");
    });
  });
  ...
  describe("createCategory", () => {
    ...
    it("throws error if category creation fails", async () => {
      ...
      await expect(repository.createCategory({} as any)).rejects.toThrow(
  ...
  describe("Error handling", () => {
    ...
    it("handles database errors in createProduct", async () => {
      ...
      await expect(repository.createProduct({} as any)).rejects.toThrow("Insert failed");
    });
