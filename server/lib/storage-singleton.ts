...
class StorageSingleton {
	...
	public static getInstance(): IStorage {
		...
		if (StorageSingleton.isInitializing) {
			throw new Error(
				"Storage singleton is currently being initialized. Please wait.",
			);
		}
		...
		try {
			logger.info(
				"[StorageSingleton] Initializing Direct PostgreSQL Storage...",
			);
			...
			logger.info(
				"[StorageSingleton] ✅ Direct PostgreSQL Storage initialized successfully",
			);

			return StorageSingleton.instance;
		} catch (error) {
			logger.error(
				"[StorageSingleton] ❌ Failed to initialize PostgreSQL storage:",
				error,
			);
			throw new Error(
