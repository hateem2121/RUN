...
export const CacheWarmupRegistry = {
	...
	productCount: (storage: IStorage): CacheWarmupTask => ({
		...
		loader: async () => {
			if (
				"getProductCount" in storage &&
				typeof (storage as any).getProductCount === "function"
			) {
					...
					try {
						if (mediaAsset.storagePath) {
							...
							if (
								...
								mediaAsset.imageVariants?.original
							) {
	...
	featuredProducts: (storage: IStorage): CacheWarmupTask => ({
		...
		loader: async () => {
			...
			const featuredSettings =
				await storage.getHomepageFeaturedProductsSettings();
			...
			const popularProductIds = productsArray
				.map((p: any) => p.id)
				.filter((id: any) => id != null);
			...
			const productIdsToWarm = Array.from(
				new Set(allIds.filter((id) => id != null)),
			);
			...
			for (const productId of productIdsToWarm.slice(0, 15)) {
				...
				try {
					...
					if (product?.urlPath) {
						...
						const productContext = await storage.getProductByPath(
							product.urlPath,
						);
						if (productContext) {
							warmedCount++;
