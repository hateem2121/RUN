  ...
  homepageBatch: (storage: IStorage): CacheWarmupTask => ({
    ...
    loader: async () => {
      ...
      return {
        ...
        featuredProductsSettings: { result: featuredProductsSettings, timestamp },
  ...
  productsSummary: (storage: IStorage): CacheWarmupTask => ({
    ...
    loader: async () => {
      if (
        ...
        return await (storage as any).getProductsSummary(100, 0, { cacheStrategy: "rebuild" });
      }
      return null;
    },
    ttl: 10 * 60 * 1000,
      ...
      try {
        ...
        for (const mediaAsset of mediaArray.slice(0, 15)) {
          try {
            ...
            // Continue warming others even if one fails
            console.warn(`Failed to warm media ${mediaAsset.id}:`, error);
          }
        }

        return { warmed: warmedCount, total: mediaIds.length };
      } catch (error) {
        console.error("Media content warming failed:", error);
    ...
    loader: async () => {
      ...
      for (const productId of productIdsToWarm.slice(0, 15)) {
        ...
        try {
          ...
          console.warn(`Failed to warm product ${productId}:`, error);
        }
      }
