...
class AlertManager {
  ...
  constructor() {
    ...
    this.thresholds = {
      ...
      errorRate: {
        ...
        timeWindowMinutes: parseInt(
          process.env.ALERT_ERROR_WINDOW_MIN || "5",
          10,
        ),
      },
      httpErrorRate: {
      ...
      memory: {
        percentageThreshold: parseFloat(
          process.env.ALERT_MEMORY_PERCENT || "80",
        ),
      },
      dbConnection: {
    ...
    if (this.thresholds.gcPause.enabled && typeof global.gc !== 'undefined') {
      ...
      logger.warn("[AlertManager] GC monitoring enabled but --expose-gc flag not set. GC pause tracking unavailable.");
    }
  ...
  private writeMemorySnapshot(usagePercent: number): string | null {
    ...
    if (now - this.lastHeapSnapshotTime < this.heapSnapshotCooldownMs) {
      const minutesRemaining = Math.ceil((this.heapSnapshotCooldownMs - (now - this.lastHeapSnapshotTime)) / 60000);
      logger.info(`[AlertManager] Heap snapshot skipped - cooldown active (${minutesRemaining} minutes remaining)`);
      return null;
    }
  ...
  private initializeGCMonitoring(): void {
    try {
      this.gcObserver = new PerformanceObserver((list) => {
        ...
        for (const entry of entries) {
          if (entry.entryType === 'gc') {
            ...
            this.gcMetrics.averagePauseTime = this.gcMetrics.totalPauseTime / this.gcMetrics.totalPauses;
            ...
            if (duration > this.thresholds.gcPause.thresholdMs) {
              logger.warn(`[AlertManager] 🐌 GC PAUSE: ${duration.toFixed(2)}ms (threshold: ${this.thresholds.gcPause.thresholdMs}ms, kind: ${kind})`);
              
              // Create alert for long GC pause
              const alert: Alert = {
                id: `alert_${Date.now()}_gc_pause`,
                type: "gc_pause",
                severity: duration > this.thresholds.gcPause.thresholdMs * 2 ? "critical" : "warning",
      ...
      logger.info(`[AlertManager] ✅ GC monitoring initialized (threshold: ${this.thresholds.gcPause.thresholdMs}ms)`);
    } catch (error) {
