...
async function cleanupTempUploads(
	...
	try {
		...
		logger.info(
			`[Lifecycle] Found ${allFiles.length} temp upload files to check`,
		);
		...
		logger.info(
			`[Lifecycle] ${filesToDelete.length} temp files eligible for deletion`,
		);
		...
		for (const filePath of filesToDelete) {
			try {
				if (dryRun) {
					...
					if (deleted % 10 === 0) {
						logger.debug(
							`[Lifecycle] Deleted ${deleted}/${filesToDelete.length} files`,
						);
					}
				}
			} catch (error) {
				errors++;
				logger.error(
					`[Lifecycle] Failed to delete ${filePath}:`,
		...
		logger.error(
			`[Lifecycle] Temp uploads cleanup failed:`,
			serializeError(error),
		);
		errors++;
		return { deleted, freed, errors };
...
async function cleanupOrphanedFiles(
	...
	try {
		...
		logger.info(
			`[Lifecycle] Found ${dbStoragePaths.size} storage paths in database`,
		);
		...
		for (const directory of directories) {
			try {
				...
				logger.info(
					`[Lifecycle] Found ${allFiles.length} files in ${directory}`,
				);
				...
				for (const filePath of allFiles) {
					...
					if (!dbStoragePaths.has(filePath)) {
						...
						try {
							if (dryRun) {
								logger.info(
									`[Lifecycle] [DRY RUN] Would delete orphaned file: ${filePath}`,
								);
						...
						if (deleted >= maxDeletions) {
							logger.info(
								`[Lifecycle] Reached max deletions limit (${maxDeletions})`,
							);
							break;
						}
					}
		...
		logger.error(
			`[Lifecycle] Orphaned files cleanup failed:`,
			serializeError(error),
		);
		errors++;
		return { deleted, freed, errors, checked };
...
async function cleanupOrphanedSessions(): Promise<void> {
	try {
		...
		logger.error(
			`[Lifecycle] Failed to cleanup orphaned sessions:`,
			serializeError(error),
		);
	}
}
...
class StorageLifecycleScheduler {
	...
	start(): void {
		...
		logger.info(
			`[Lifecycle] Starting scheduler (interval: ${this.config.interval}ms)`,
		);

		// Run immediately on startup
		this.runCleanup().catch((error) => {
			logger.error(
				`[Lifecycle] Initial cleanup failed:`,
				serializeError(error),
			);
		});
		...
		this.timer = setInterval(() => {
			this.runCleanup().catch((error) => {
				logger.error(
					`[Lifecycle] Scheduled cleanup failed:`,
					serializeError(error),
				);
			});
