...
async function cleanupTempUploads(
  ...
  logger.info(`[Lifecycle] Starting temp uploads cleanup (maxAge: ${maxAgeHours}h, dryRun: ${dryRun})`);
  ...
  try {
    ...
    logger.info(`[Lifecycle] Temp uploads cleanup complete: ${deleted} deleted, ~${freedMB}MB freed, ${errors} errors`);
...
async function cleanupOrphanedFiles(
  ...
  maxDeletions: number,
  dryRun: boolean
): Promise<{ deleted: number; freed: number; errors: number; checked: number }> {
  logger.info(`[Lifecycle] Starting orphaned files cleanup (directories: ${directories.join(', ')}, dryRun: ${dryRun})`);
  ...
  try {
    ...
    const dbRecords = await db.select({ storagePath: mediaAssets.storagePath })
      .from(mediaAssets)
    ...
    for (const directory of directories) {
      try {
        ...
        for (const filePath of allFiles) {
          ...
          if (!dbStoragePaths.has(filePath)) {
            ...
            try {
              ...
              errors++;
              logger.error(`[Lifecycle] Failed to delete orphaned file ${filePath}:`, serializeError(error));
            }
        ...
        logger.info(`[Lifecycle] Processed ${directory}: ${deleted} orphaned files deleted (${checked} files checked)`);
      } catch (error) {
        errors++;
        logger.error(`[Lifecycle] Failed to process directory ${directory}:`, serializeError(error));
      }
    ...
    const freedMB = (freed / (1024 * 1024)).toFixed(2);
    logger.info(`[Lifecycle] Orphaned files cleanup complete: ${deleted} deleted, ~${freedMB}MB freed, ${errors} errors, ${checked} files checked`);
