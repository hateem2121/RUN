...
export class QueryPerformanceMonitor {
  ...
  trackQuery(startTime: number, success: boolean): void {
    ...
    if (this.legacyQueryHistory.length > this.MAX_LEGACY_HISTORY) {
      ...
      if (this.legacyQueryHistory.length > this.MAX_LEGACY_HISTORY * 1.1) {
        logger.warn(`[QueryPerformanceMonitor] Legacy query history overflow: ${this.legacyQueryHistory.length} > ${this.MAX_LEGACY_HISTORY * 1.1} (10% threshold)`);
      }
   ...
   */
  getMetrics() {
    const timeoutRate = this.totalQueries > 0 
      ? (this.timeouts / this.totalQueries) * 100 
      : 0;
    
    // Calculate avgResponseTime from rolling 100-query window (original behavior)
    const avgResponseTime = this.legacyQueryHistory.length > 0
      ? this.legacyQueryHistory.reduce((sum, time) => sum + time, 0) / this.legacyQueryHistory.length
      : 0;
   ...
   */
  private getThresholdForQuery(operation: string): { threshold: number; shouldAlert: boolean; category: string } {
  ...
  private handleSlowQuery(metrics: QueryMetrics, threshold: number, category: string): void {
    ...
    const shouldAlert = 
      ...
      this.consecutiveSlowQueries >= 3; // Alert on 3 consecutive slow queries
  ...
  getPerformanceStats(): PerformanceStats {
    ...
    const averageResponseTime = alertableMetrics.length > 0
...
export class QueryTracker {
  ...
  complete(): number {
    ...
    if (shouldLogDetails && Object.keys(this.phases).length > 0) {
      ...
      logger.info(
        `🔍 [Phase Breakdown] ${this.operation}`,
        {
