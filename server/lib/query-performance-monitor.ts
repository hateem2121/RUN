...
export class QueryPerformanceMonitor {
	...
	trackQuery(startTime: number, success: boolean): void {
		...
		if (this.legacyQueryHistory.length > this.MAX_LEGACY_HISTORY) {
			...
			this.legacyQueryHistory = this.legacyQueryHistory.slice(
				-this.MAX_LEGACY_HISTORY,
			);
		}
	...
	getMetrics() {
		const timeoutRate =
			this.totalQueries > 0 ? (this.timeouts / this.totalQueries) * 100 : 0;
	...
	recordQuery(metrics: QueryMetrics): void {
		...
		const { threshold, shouldAlert, category } = this.getThresholdForQuery(
			metrics.operation,
		);
	...
	private triggerSlowQueryAlert(
		...
		logger.error(
			`🚨 SLOW QUERY ALERT: ${metrics.operation} exceeded ${threshold}ms threshold (category: ${category})`,
			{
				...
				slowQueryRate:
					(stats.slowQueries / Math.max(stats.totalQueries, 1)) * 100,
	...
	getPerformanceStats(): PerformanceStats {
		...
		const averageResponseTime =
			alertableMetrics.length > 0
				? alertableMetrics.reduce((sum, m) => sum + m.duration, 0) /
	...
	generatePerformanceReport(): {
		...
		const slowQueries = recentMetrics
			.filter((m) => {
				const { threshold, shouldAlert } = this.getThresholdForQuery(
					m.operation,
				);
...
export class QueryTracker {
	...
	complete(): number {
		...
		const shouldLogDetails =
			QueryTracker.sampleCounter % QueryTracker.SAMPLE_RATE === 0;

		if (shouldLogDetails && Object.keys(this.phases).length > 0) {
			const phaseSum = Object.values(this.phases).reduce(
				(sum, ms) => sum + ms,
				0,
			);
