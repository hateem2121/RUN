...
export class TwoTierBatchCache {
	...
	private constructor() {
		...
		logger.info(
			"[TwoTierBatchCache] 🚀 Initialized: Delegating to UnifiedCache with SWR support",
		);
	}
	...
	async get<T>(
		...
		if (options?.bypassCache) {
			logger.debug(
				`[TwoTierBatchCache] Cache bypass requested for ${key} - fetching fresh data`,
			);
			...
			logger.info(
				`[TwoTierBatchCache] ⬆️ BYPASS + CACHED: ${key} (${dbTime.toFixed(2)}ms)`,
			);
		...
		if (options?.swrConfig) {
			...
			switch (result.source) {
				...
				case "loader": {
					benchmark.hit = "MISS";
					const loaderTime =
						result.timings.loaderTime || result.timings.totalTime;
		...
		logger.warn(
			`[TwoTierBatchCache] No SWR config provided for ${key}, using legacy path`,
		);
	...
	logPerformanceReport(context: string): void {
		...
		logger.info(
			`├─ Overall Hit Rate: ${metrics.hitRate.toFixed(2)}% (target: >80%)`,
		);
		...
		if (metrics.hitRate >= 80) {
			logger.info(
				`✅ SUCCESS: Hit rate ${metrics.hitRate.toFixed(2)}% exceeds 80% target`,
			);
		} else {
			logger.warn(
				`⚠️ WARN: Hit rate ${metrics.hitRate.toFixed(2)}% below 80% target`,
			);
		}

		if (metrics.avgDbTime > 0 && metrics.avgDbTime < 300) {
			logger.info(
				`✅ SUCCESS: Avg DB time ${metrics.avgDbTime.toFixed(2)}ms below 300ms target`,
			);
		} else if (metrics.avgDbTime >= 300) {
			logger.warn(
				`⚠️ WARN: Avg DB time ${metrics.avgDbTime.toFixed(2)}ms exceeds 300ms target`,
			);
		}
	}
	...
	private recordL1Time(time: number): void {
		...
		this.metrics.avgL1Time =
			this.responseTimesL1.reduce((a, b) => a + b, 0) /
			this.responseTimesL1.length;
	}
	...
	private recordL2Time(time: number): void {
		...
		this.metrics.avgL2Time =
			this.responseTimesL2.reduce((a, b) => a + b, 0) /
			this.responseTimesL2.length;
	}
	...
	private recordDbTime(time: number): void {
		...
		this.metrics.avgDbTime =
			this.responseTimesDb.reduce((a, b) => a + b, 0) /
			this.responseTimesDb.length;
	}
}
