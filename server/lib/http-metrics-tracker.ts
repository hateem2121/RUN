		...
		return (req: Request, res: Response, next: NextFunction) => {
			...
			res.on("finish", () => {
				...
				const metric: HttpMetric = {
					...
					contentLength:
						parseInt(res.getHeader("content-length") as string) || undefined,
				};
	...
	private recordMetric(metric: HttpMetric): void {
		...
		if (existing) {
			...
			existing.avgDuration =
				Math.round((existing.totalDuration / existing.count) * 100) / 100;
		...
		setInterval(
			() => {
				...
				if (beforeCount !== this.metrics.length) {
					logger.debug(
						`[HttpMetrics] Cleaned ${beforeCount - this.metrics.length} old metrics`,
					);
				}
			},
			15 * 60 * 1000,
