...
export class UnifiedMemoryCache {
  ...
  private constructor() {
    ...
    logger.info("[UnifiedMemoryCache] 🚀 Initialized: Delegating to UnifiedCache (In-Memory)");
  }
  ...
  async get<T>(
    ...
    if (options?.bypassCache) {
      ...
      if (options.swrConfig) {
        await cache
          ...
          .catch((err) =>
            logger.warn(`[UnifiedMemoryCache] Failed to update cache after bypass:`, err),
          );
      }
    ...
    if (options?.swrConfig) {
      const result = await cache.getSWR(`batch:${key}`, fetchFn, options.swrConfig);
      const timingValue =
        result.timings.cacheTime || result.timings.loaderTime || result.timings.totalTime;

      if (result.source === "memory" || (result.source as string) === "stale_memory") {
        // HIT
        benchmark.hit = "L1";
    ...
    await cache
      ...
      .catch((err) => logger.warn(`[UnifiedMemoryCache] Failed to set cache:`, err));
