...
export const CacheKeys = {
  ...
  media: {
    ...
    paginated: (limit: number, offset: number) => `media:assets:${limit}:${offset}`,
    variants: (id: number, options: string) => `media:variants:${id}:${options}`,
  },

  products: {
    list: (filters?: string) => `products:list${filters ? `:${filters}` : ""}`,
    summary: (limit: number, offset: number) => `products:summary:${limit}:${offset}`,
...
export const InvalidationPatterns = {
  ...
  inquiries: "^(batch:)?inquiries:.*",
  fabrics: "^(batch:)?fabrics:.*",
  fibers: "^(batch:)?fibers:.*",
  certificates: "^(batch:)?certificates:.*",
  sizeCharts: "^(batch:)?size_charts:.*",
  accessories: "^(batch:)?accessories:.*",
  computed: "^(batch:)?computed:.*",
  services: "^(batch:)?services:.*|^services$",
  legal: "^(batch:)?legal:.*|^legal$",
};
