...
interface CircuitBreakerMetrics {
  queries: { count: number; totalDuration: number; retries: number; failures: number };
  circuitBreaker: {
    stateChanges: number;
...
export class DatabaseCircuitBreaker {
    ...
    if (this.circuitState === CircuitState.OPEN) {
      ...
      if (timeSinceLastFailure >= this.TIMEOUT_DURATION) {
        ...
        const remainingTime = Math.ceil(
          (this.TIMEOUT_DURATION - timeSinceLastFailure) / 1000
        );
        throw new Error(
          `Circuit breaker OPEN: Database unavailable. Retry in ${remainingTime}s`
        );
      }
    }
  ...
  private recordSuccess(_operationName: string): void {
    ...
    if (this.circuitState === CircuitState.HALF_OPEN) {
      ...
      if (this.successCount >= this.SUCCESS_THRESHOLD) {
        ...
        this.metrics.circuitBreaker.lastStateChange = new Date();
        logger.info(
          `✅ Circuit breaker CLOSED: Database service recovered`
        );
      }
  ...
  private recordFailure(operationName: string, error: Error): void {
    ...
    if (this.isClientError(error)) {
      logger.debug(
        `[Circuit Breaker] Skipping client error - not a database health issue`
      );
      return;
    }
  ...
  private trackRetries(operationName: string, retryCount: number): void {
    ...
    logger.debug(
      `📊 [Metrics] ${operationName} required ${retryCount} retries`
    );
  }
