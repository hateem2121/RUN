...
class PerformanceMonitor {
	...
	public async getSystemPerformance(): Promise<SystemPerformance> {
		...
		return {
			http: {
				...
				slowRequests: httpStats.slowestRequests.filter(
					(r) => r.duration > 1000 && r.duration <= 5000,
				).length,
				verySlowRequests: httpStats.slowestRequests.filter(
					(r) => r.duration > 5000,
				).length,
			},
			cache: {
	...
	public async getHealthStatus(): Promise<HealthStatus> {
		...
		if (perf.http.p95Duration > 1000) {
			checks.http = false;
			issues.push(
				`High p95 latency: ${perf.http.p95Duration}ms (threshold: 1000ms)`,
			);
		}
		...
		if (perf.http.verySlowRequests > 0) {
			checks.http = false;
			issues.push(
				`${perf.http.verySlowRequests} very slow requests (>5s) detected`,
			);
		}
		...
		if (perf.cache.hitRate < 50 && perf.cache.operations > 100) {
			checks.cache = false;
			issues.push(
				`Low cache hit rate: ${perf.cache.hitRate}% (threshold: 50%)`,
			);
		}
		...
		if (perf.memory.percentage > 90) {
			checks.memory = false;
			issues.push(
				`High memory usage: ${perf.memory.percentage}% (threshold: 90%)`,
			);
		}
