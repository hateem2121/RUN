...
export async function emitCacheInvalidation(
	...
	try {
		...
		logger.info(
			`[CacheEvents] Emitted invalidation event: ${pattern} (${reason})`,
		);
	} catch (error) {
 ...
 */
export async function getLatestInvalidationTime(
	pattern: string,
): Promise<number> {
	try {
		const now = Date.now();
		...
		logger.error(
			"[CacheEvents] Failed to get latest invalidation time:",
			error,
		);
		return 0;
	}
