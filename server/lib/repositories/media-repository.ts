...
export class MediaRepository {
  ...
  async getMediaAsset(id: number): Promise<MediaAsset | undefined> {
    ...
    try {
      ...
      logger.debug(
        `[MediaRepo] Failed to get media asset ${id} from cache:`,
        error,
      );
    }
  ...
  async getMediaAssets(
    ...
    logger.debug(
      `[MediaRepo] Cache MISS for key: ${cacheKey} - executing database query`,
    );
    ...
    const result = await dbCircuitBreaker.execute(async () => {
      ...
      // Uses MEDIA_LIST_COLUMNS (16 fields) instead of full schema (25 fields)
  ...
  async updateMediaAsset(
    ...
    if (updated) {
      this.invalidateMediaCacheSelectively("update", updated.id).catch(
        (error) =>
          logger.debug(
            "Smart cache invalidation failed (non-critical):",
            error,
          ),
      );
    }
  ...
  async deleteMediaAsset(id: number): Promise<boolean> {
    ...
    try {
      ...
      logger.info(
        `[MediaRepository] ✅ Cleaned up product array references for media ${id}`,
      );
    } catch (cleanupError) {
      logger.warn(
    ...
    if (!result.length) {
      logger.warn(
        `[MediaRepository] ⚠️ Asset ${id} not found for deletion (cache already cleared)`,
      );
      throw new MediaNotFoundError(id);
    }

    logger.info(
      `[MediaRepository] ✅ Cache-first delete succeeded for asset ${id}`,
    );
    return true;
  }
  ...
  async getMediaAssetsWithCount(
    ...
    if (cached) {
      perfTracker.setCacheHit(true).complete();
      logger.debug(
        `[MediaRepo] Cache HIT for getMediaAssetsWithCount(${limit}, ${offset})`,
      );
      return cached;
    }
  ...
  async updateMediaAssetsTags(
    ...
    if (updatedCount > 0) {
      this.invalidateMediaCacheSelectively("update", updates[0]!.id).catch(
        (error) =>
          logger.debug(
            "Batch cache invalidation failed (non-critical):",
            error,
          ),
      );
    }
  ...
  async getMediaAssetsByIds(ids: string[]): Promise<MediaAsset[]> {
    ...
    try {
      ...
      if (cached) {
        perfTracker.setCacheHit(true).complete();
        logger.debug(
          `[MediaRepo] ✅ Cache HIT for getMediaAssetsByIds (${numericIds.length} IDs)`,
        );
        return cached;
      }
    } catch (error) {
      logger.debug(
        `[MediaRepo] Failed to get batch media assets from cache:`,
        error,
      );
    }
    ...
    if (result.length > 0) {
      try {
        ...
        logger.debug(
          `[MediaRepo] Failed to cache batch media assets:`,
          error,
        );
      }
    }
  ...
  private async invalidateMediaCacheSelectively(
    ...
    await Promise.all(
      selectivePatterns.map((pattern) => unifiedCache.clearPattern(pattern)),
    );
    ...
    if (operation === "update" || operation === "delete") {
      try {
        ...
        logger.debug(
          `[MediaRepo] Failed to clear asset cache for ${mediaId}:`,
          error,
        );
      }
    }
  ...
  private async preloadFirstPageCache(): Promise<void> {
    try {
      Promise.allSettled([
        this.getMediaAssets().catch((err) =>
          logger.debug("Cache preload failed:", err),
        ),
      ]);
    } catch (error) {
