...
export class AccessoryRepository {
  ...
  async getAccessory(id: number): Promise<Accessory | undefined> {
    ...
    const [accessory] = await db.select().from(accessories)
      .where(and(eq(accessories.id, id), isNull(accessories.deletedAt), eq(accessories.isActive, true)));
  ...
  async getAccessories(
    ...
    logger.debug(`[AccessoryRepo] getAccessories(${limit}, ${offset}) returned ${result.length} items`);
  ...
  async getAccessoriesWithCount(
    ...
    const cached = await unifiedCache.get<{ accessories: Accessory[], total: number }>(cacheKey);
    if (cached) {
      perfTracker.setCacheHit(true).complete();
  ...
  async createAccessory(accessory: InsertAccessory): Promise<Accessory> {
    const [created] = await db.insert(accessories)
      .values(accessory)
      .returning();
    
    // Invalidate all paginated caches
    this.invalidateAccessoryCacheSelectively('create', created!.id)
      .catch(error => logger.debug('Cache invalidation failed (non-critical):', error));
  ...
  async updateAccessory(id: number, accessory: Partial<InsertAccessory>): Promise<Accessory | undefined> {
    ...
    if (updated) {
      this.invalidateAccessoryCacheSelectively('update', updated.id)
        .catch(error => logger.debug('Cache invalidation failed (non-critical):', error));
    }
  ...
  async deleteAccessory(id: number): Promise<boolean> {
    ...
    try {
      ...
      logger.error(`[AccessoryRepository] ❌ Cache invalidation failed for accessory ${id}:`, cacheError);
  ...
  private async invalidateAccessoryCacheSelectively(
    ...
    logger.info(`[AccessoryRepository] Cache invalidated for ${operation} operation on accessory ${accessoryId}`);
  }
}
