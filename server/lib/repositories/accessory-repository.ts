...
export class AccessoryRepository {
	...
	async getAccessories(
		...
		if (cached) {
			...
			logger.debug(
				`[AccessoryRepo] Cache HIT for getAccessories(${limit}, ${offset})`,
			);
			return cached;
		}
	...
	async createAccessory(accessory: InsertAccessory): Promise<Accessory> {
		...
		this.invalidateAccessoryCacheSelectively("create", created!.id).catch(
			(error) =>
	...
	async updateAccessory(
		...
		if (updated) {
			this.invalidateAccessoryCacheSelectively("update", updated.id).catch(
				(error) =>
					logger.debug("Cache invalidation failed (non-critical):", error),
			);
		}
	...
	async deleteAccessory(id: number): Promise<boolean> {
		...
		try {
			...
			logger.info(
				`[AccessoryRepository] ✅ Cache invalidated for accessory ${id}`,
			);
		} catch (cacheError) {
			logger.error(
		...
		if (!result.length) {
			logger.warn(
				`[AccessoryRepository] ⚠️ Accessory ${id} not found for deletion`,
			);
			return false;
		}

		logger.info(
			`[AccessoryRepository] ✅ Cache-first delete succeeded for accessory ${id}`,
		);
		return true;
	}
