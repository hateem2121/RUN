...
export class ProductRepository {
	...
	async getProductsSummary(
		...
		const perfTracker =
			queryPerformanceMonitor.startQuery("getProductsSummary");
	...
	async getHomepageFeaturedProducts(
		...
		const perfTracker = queryPerformanceMonitor.startQuery(
	...
	async getProductByPath(urlPath: string): Promise<any> {
		...
		if (cached) {
			...
			if ((cached as any).__notFound === true) {
				logger.info(
					`[ProductRepo] ✅ Cache HIT (404) for product path: ${urlPath}`,
				);
				perfTracker.setCacheHit(true).complete();
				return null;
			}
		...
		logger.info(
			`[ProductRepo] ❌ Cache MISS for product path: ${urlPath} - querying database`,
		);
		...
		const result = await dbCircuitBreaker.execute(async () => {
			...
			const productResult = await db
				...
				.leftJoin(
					fabrics,
					and(eq(products.fabricId, fabrics.id), isNull(fabrics.deletedAt)),
				)
				.leftJoin(
					sizeCharts,
					and(
						eq(products.sizeChartId, sizeCharts.id),
						isNull(sizeCharts.deletedAt),
					),
				)
				.where(
					and(
			...
			queryTimings["1_main_product"] = Math.round(
				performance.now() - mainQueryStart,
			);

			if (!product) {
				logger.info(
					`[ProductRepo] [CHUNK 2] Query timings (404): ${JSON.stringify(queryTimings)}`,
				);
				return null;
			}
			...
			const [
				...
				(async () => {
					...
					batchTimings["category_with_parent"] = Math.round(
				...
				(async () => {
					...
					const result = product.categoryId
						? await db
								...
								.limit(12)
						: [];
					batchTimings["categoryProducts"] = Math.round(
						performance.now() - start,
					);
			...
			queryTimings["2_parallel_batch"] = Math.round(
				performance.now() - batchQueryStart,
			);
			queryTimings["total_db_time"] = Math.round(
			...
			logger.info(
				`[ProductRepo] [CHUNK 2] Batch breakdown: ${JSON.stringify(batchTimings)}`,
			);
			...
			const currentIndex = allCategoryProductsResult.findIndex(
				(p) => p.id === product.id,
			);
			// Guard: Only set navigation if current product found in batch
			const previousProduct =
				currentIndex > 0 ? allCategoryProductsResult[currentIndex - 1] : null;
			const nextProduct =
		...
		if (result !== null) {
			try {
				...
				await unifiedCache.set(cacheKey, result, PRODUCT_CACHE_TTL);
				logger.info(
					`[ProductRepo] ✅ Cache SET successful for product path: ${urlPath}`,
				);
			} catch (cacheError) {
				logger.warn(
					`[ProductRepository] Failed to cache product ${urlPath}:`,
					cacheError,
				);
			}
		} else {
			...
			try {
				...
				logger.info(
					`[ProductRepo] ✅ Negative cache SET successful for 404 path: ${urlPath}`,
				);
			} catch (cacheError) {
				logger.warn(
					`[ProductRepository] Failed to set negative cache for ${urlPath}:`,
					cacheError,
				);
			}
		}
	...
	async getCategories(limit?: number, offset?: number): Promise<Category[]> {
		const cacheKey =
			limit && offset !== undefined
				? `categories:active:${limit}:${offset}`
				: "categories:active";
	...
	async getDeletedCategories(): Promise<Category[]> {
		...
		try {
			...
			logger.debug(
				"[Cache] Failed to get deleted categories from cache:",
				error,
			);
		}
	...
	async get3DModelMetadata(productId: number): Promise<any | null> {
		...
		const perfTracker =
			queryPerformanceMonitor.startQuery("get3DModelMetadata");
	...
	private async invalidateProductCache(): Promise<void> {
		try {
			...
			logger.debug(
				"[ProductRepository] Cache invalidation failed (non-critical):",
				error,
			);
		}
	}
