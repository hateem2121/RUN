...
export class MiscRepository {
	...
	async getFibers(): Promise<Fiber[]> {
		...
		logger.info(
			`[MiscRepo] Fibers loaded into memory cache (${result.length} items)`,
		);
		return result;
	}
	...
	async updateFabric(
		...
		const transformedFabric = this.transformFabricForDatabase(
			fabric,
			existing.properties,
		);
	...
	private transformFabricForDatabase(
		...
		if (enhancedMoistureManagement !== undefined)
			technicalProperties.enhancedMoistureManagement =
				enhancedMoistureManagement;
		...
		if (shrinkageTolerancePercentage !== undefined)
			technicalProperties.shrinkageTolerancePercentage =
				shrinkageTolerancePercentage;
	...
	async getCertificates(): Promise<Certificate[]> {
		try {
			const cached = await unifiedCache.get<Certificate[]>(
				this.CERTIFICATES_CACHE_KEY,
				"data",
			);
			if (cached) return cached;
		} catch (error) {
		...
		const result = await db
			...
			.where(
				and(eq(certificates.isActive, true), isNull(certificates.deletedAt)),
			)
	...
	> {
		const [settings] = await db
			.select()
			.from(navigationGlassmorphismSettings)
			.limit(1);
		return settings;
	}
	...
	async getFooterConfiguration(): Promise<FooterConfiguration | undefined> {
		...
		try {
			const cached = await unifiedCache.get<FooterConfiguration>(
				cacheKey,
				"data",
			);
			if (cached) return cached;
