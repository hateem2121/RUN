...
export class CacheOperations {
	...
	static async getHomepageData(
		key: keyof typeof CacheKeys.homepage,
		loader: () => Promise<any>,
	) {
	...
	static async getMediaBatch(ids: number[], loader: () => Promise<any>) {
		...
		if (data === null) {
			logger.debug(
				`[Cache] Loading media batch [${ids.join(",")}] from source`,
			);
			data = await loader();
			const options = CacheStrategies.MEDIA();
			await getCache().set(cacheKey, data, options.ttl, options.category);
	...
	static async invalidateCategories(id?: number) {
		if (id) {
			...
			logger.info(
				"[Cache] Invalidated all categories and related product list cache entries",
			);
		}
	}
