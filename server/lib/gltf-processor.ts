...
export class GLTFProcessor {
  ...
  private isGLBFormat(buffer: Buffer): boolean {
    ...
    return buffer.length >= 4 && 
           buffer[0] === 0x67 && buffer[1] === 0x6C && 
           buffer[2] === 0x54 && buffer[3] === 0x46;
  }
    ...
    try {
      ...
      if (this.isGLBFormat(buffer)) {
        ...
        try {
          ...
          if (!jsonData || typeof jsonData !== 'object' || !jsonData.asset || !jsonData.asset.version) {
          ...
          try {
            ...
            throw new Error(`Failed to parse GLTF file as both JSON and binary: JSON=${jsonError instanceof Error ? jsonError.message : String(jsonError)}, Binary=${binaryError instanceof Error ? binaryError.message : String(binaryError)}`);
          }
        }
      }
      ...
      logger.debug(`[GLTF] Validation complete: ${textures.length} textures, ${externalReferences.length} external refs`);
      ...
      if (errorMessage.includes('JSON')) {
        ...
        remediationHint = 'File may be corrupted or not a valid GLTF/GLB format. Verify the file is a valid GLTF JSON or binary GLB file.';
      } else if (errorMessage.includes('asset.version') || errorMessage.includes('missing required')) {
        errorCategory = 'STRUCTURE_ERROR';
        remediationHint = 'GLTF file is missing required fields (e.g., asset.version). Ensure the file follows GLTF 2.0 specification.';
      } else if (errorMessage.includes('accessor') || errorMessage.includes('buffer')) {
        errorCategory = 'DATA_ERROR';
        remediationHint = 'GLTF contains invalid accessor bounds or buffer references. Check mesh data integrity.';
      } else if (errorMessage.includes('texture') || errorMessage.includes('image')) {
        errorCategory = 'TEXTURE_ERROR';
        remediationHint = 'Issues with texture or image references. Ensure all textures are properly embedded or referenced.';
      } else {
        remediationHint = 'Unknown validation error. Use the Khronos GLTF Validator for detailed analysis.';
      }
  ...
  async embedTextures(buffer: Buffer, baseUrl?: string): Promise<GLTFProcessingResult> {
    ...
    try {
        ...
        if (uri && !uri.startsWith('data:')) {
          ...
          try {
            ...
            if (baseUrl) {
              ...
              logger.debug(`[GLTF] No baseUrl provided, leaving external reference unchanged: ${uri}`);
              // Do NOT call texture.setURI('') - this would strip the texture!
            }
          } catch (fetchError) {
        ...
        if (uri && !uri.startsWith('data:')) {
          ...
          try {
            if (baseUrl) {
              ...
              logger.debug(`[GLTF] No baseUrl provided, leaving external buffer reference unchanged: ${uri}`);
              // Do NOT call buffer.setURI('') - this would strip the buffer!
            }
          } catch (fetchError) {
      ...
      if (finalValidation.hasExternalReferences) {
        logger.warn(`[GLTF] Warning: ${finalValidation.externalReferences.length} external references still remain after processing`);
      ...
      await document.transform(
        ...
        draco({ quantizePosition: 14, quantizeNormal: 10, quantizeTexcoord: 12 }) // Draco compression for smaller files
      );
      ...
      logger.info(`[GLTF] Texture embedding complete: ${texturesEmbedded} textures embedded, ${externalReferencesRemoved} external refs removed`);
      logger.info(`[GLTF] Size change: ${originalSize} → ${processedSize} bytes (${((processedSize / originalSize - 1) * 100).toFixed(1)}%)`);
   ...
   */
  async validateForProductionUpload(buffer: Buffer): Promise<{ valid: boolean; reason?: string; details?: GLTFValidationResult }> {
    const validation = await this.validateGLTF(buffer);
    ...
    logger.debug(`[GLTF] Production validation passed: ${validation.textureCount} embedded textures, ${validation.bufferCount} buffers`);
...
export function getGLTFProcessor(): GLTFProcessor {
  if (!gltfProcessor) {
    try {
      ...
      throw new Error(`GLTF processor initialization failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
