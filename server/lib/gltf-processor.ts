...
export class GLTFProcessor {
	...
	async validateGLTF(buffer: Buffer): Promise<GLTFValidationResult> {
		try {
			logger.debug(
				"[GLTF] Validating GLTF file structure and texture embedding...",
			);
			...
			if (this.isGLBFormat(buffer)) {
				...
				try {
					...
					if (
						...
						throw new Error(
							"Invalid GLTF JSON structure: missing required asset.version field",
						);
					}
					...
					logger.debug(
						`[GLTF] JSON parsing failed (${jsonError}), attempting binary fallback...`,
					);
					try {
						document = await this.io.readBinary(new Uint8Array(buffer));
					} catch (binaryError) {
			...
			if (errorMessage.includes("JSON")) {
				...
				errorMessage.includes("accessor") ||
					...
					try {
						...
						if (baseUrl) {
							...
							if (response.ok) {
								...
								texturesEmbedded++;
								logger.debug(
									`[GLTF] Embedded texture: ${uri} (${textureBuffer.byteLength} bytes)`,
								);
							} else {
								// Leave texture unchanged if can't fetch - don't strip it
								logger.warn(
									`[GLTF] Could not fetch texture ${uri}, leaving unchanged`,
								);
						...
						logger.warn(
							`[GLTF] Failed to fetch texture ${uri}, leaving unchanged:`,
							fetchError,
						);
						// Do NOT call texture.setURI('') - this would strip the texture!
					}
					...
					try {
						if (baseUrl) {
							...
							if (response.ok) {
								...
								logger.debug(
									`[GLTF] Embedded buffer: ${uri} (${bufferData.byteLength} bytes)`,
								);
							} else {
								logger.warn(
									`[GLTF] Could not fetch buffer ${uri}, leaving unchanged`,
								);
						...
						logger.warn(
							`[GLTF] Failed to fetch buffer ${uri}, leaving unchanged:`,
							fetchError,
						);
						// Do NOT call buffer.setURI('') - this would strip the buffer!
					}
	 ...
	 */
	private async validateProcessedDocument(
		document: Document,
	...
	async processForUpload(
		...
		if (validation.hasEmbeddedTextures && !validation.hasExternalReferences) {
			logger.debug(
				"[GLTF] File already has embedded textures, no processing needed",
			);
			return {
				success: true,
				processedBuffer: buffer,
	 ...
	 */
	async validateForProductionUpload(
		buffer: Buffer,
	): Promise<{
		valid: boolean;
		reason?: string;
...
export function isGLTFFile(mimeType: string, filename: string): boolean {
	...
	const hasMimeType =
		validMimeTypes.includes(mimeType) || mimeType.includes("gltf");

	// Extension detection
	const lowerFilename = filename.toLowerCase();
	const hasValidExtension =
		lowerFilename.endsWith(".gltf") || lowerFilename.endsWith(".glb");
