...
export class AppStorageService {
	...
	constructor() {
		...
		if (!this.bucketName) {
			...
			logger.info(
				`[Storage] Initialized Google Cloud Storage with bucket: ${this.bucketName}`,
			);
		}
	}
	...
	private async withTimeoutAndRetry<T>(
		...
		throw new Error(
			`GCS ${operationName} failed after ${this.MAX_RETRIES + 1} attempts`,
		);
	}
	...
	async listAssets(prefix?: string): Promise<string[]> {
		try {
			...
			logger.info(
				`✅ Listed ${keys.length} assets with prefix: ${prefix || "none"}`,
			);
			return keys;
		} catch (error) {
			logger.error(
				`❌ List failed for prefix ${prefix}:`,
	...
	async batchUpload(
		...
		try {
			...
			throw new Error(
				`Failed to batch upload assets: ${(error as Error).message}`,
			);
		}
	}
			...
			async () => {
				...
				try {
					const [url] = await file.getSignedUrl({
						version: "v4",
						action:
							method === "GET" ? "read" : method === "PUT" ? "write" : "delete",
					...
					if (
						...
						logger.warn(
							`⚠️ GCS Signing failed, falling back to public URL for ${key}`,
						);
						return `https://storage.googleapis.com/${this.bucketName}/${key}`;
					}
					throw error;
