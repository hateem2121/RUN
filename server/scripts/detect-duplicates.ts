...
async function detectDuplicates() {
	...
	for (const file of files) {
		try {
			...
			process.stdout.write(
				`\rProcessing ${processed}/${files.length}: ${file.name || file.key}`,
			);
	...
	const thumbnails = Array.from(fileInfoMap.values()).filter((f) =>
		f.path.includes("thumb-"),
	);
	...
	const totalStorage = Array.from(fileInfoMap.values()).reduce(
		(sum, f) => sum + f.size,
		0,
	);
	...
	const efficiency =
		totalWastedSpace === 0
			? 100
			: ((uniqueStorage / totalStorage) * 100).toFixed(2);
}
