...
async function analyzePerformanceMetrics() {
	...
	const totalDuration =
		uploads.totalDuration + downloads.totalDuration + deletes.totalDuration;
	...
	const formatOp = (name: string, data: any) => {
		const successRate =
			data.count > 0
				...
				: "N/A";
		const retryRate =
			data.count > 0 ? ((data.retries / data.count) * 100).toFixed(2) : "0";
		const failureRate =
			data.count > 0 ? ((data.failures / data.count) * 100).toFixed(2) : "0";
	};
	...
	const avgDuration =
		totalOps > 0 ? (totalDuration / totalOps).toFixed(2) : "0";
	...
	const retryRate =
		totalOps > 0 ? ((totalRetries / totalOps) * 100).toFixed(2) : "0";
	...
	if (totalRetries > 0) {
		if (uploads.retries > 0) {
			const uploadRetryRate =
				uploads.count > 0
					? ((uploads.retries / uploads.count) * 100).toFixed(2)
					: "0";
		}
		if (downloads.retries > 0) {
			const downloadRetryRate =
				downloads.count > 0
					? ((downloads.retries / downloads.count) * 100).toFixed(2)
					: "0";
		}
		if (deletes.retries > 0) {
			const deleteRetryRate =
				deletes.count > 0
					? ((deletes.retries / deletes.count) * 100).toFixed(2)
					: "0";
		}
	}

	const failureRate =
		totalOps > 0 ? ((totalFailures / totalOps) * 100).toFixed(2) : "0";
	...
	if (totalFailures > 0) {
		if (uploads.failures > 0) {
			const uploadFailureRate =
				uploads.count > 0
					? ((uploads.failures / uploads.count) * 100).toFixed(2)
					: "0";
		}
		if (downloads.failures > 0) {
			const downloadFailureRate =
				downloads.count > 0
					? ((downloads.failures / downloads.count) * 100).toFixed(2)
					: "0";
		}
		if (deletes.failures > 0) {
			const deleteFailureRate =
				deletes.count > 0
					? ((deletes.failures / deletes.count) * 100).toFixed(2)
					: "0";
		}
	}
	...
	if (uploads.avgDuration > benchmarks.good && uploads.count > 10) {
		opportunities.push(
			"Upload Optimization: Implement client-side compression before upload",
		);
	}
	if (downloads.avgDuration > benchmarks.good && downloads.count > 10) {
		opportunities.push(
			"Download Optimization: Add CDN caching for frequently accessed files",
		);
	}
	...
	if (downloads.count > uploads.count * 3) {
		opportunities.push(
			"Caching Strategy: High download-to-upload ratio suggests caching benefits",
		);
	}
	...
	if (deletes.count > 20) {
		opportunities.push(
			"Batch Operations: Consider batch delete API for multiple files",
		);
	}
	...
	if (uploads.avgDuration > benchmarks.acceptable) {
		opportunities.push(
			"Compression: Pre-compress files before upload to reduce transfer time",
		);
	}
