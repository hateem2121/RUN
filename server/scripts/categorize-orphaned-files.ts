...
async function categorizeOrphanedFiles() {
  console.log('📊 Categorizing orphaned files with manual verification...\n');
  ...
  for (const file of verifiedData.needs_review) {
    if (knownActiveRoutes.includes(file.path)) {
      result.false_positives.push({
        ...
        reason: 'Registered via dynamic import in routes.ts or imported in index.ts (verified manually)'
      });
    } else {
  ...
  // Calculate stats
  result.stats.total = verifiedData.false_positives.length +
  ...
  // Export results
  fs.writeFileSync(
    'server/docs/orphaned-files-categorized.json',
  ...
  console.log('================================================================================');
  console.log(`Total files analyzed:              ${result.stats.total}`);
  console.log(`False positives (ARE used):        ${result.stats.false_positives}`);
  console.log(`True orphans (HIGH confidence):    ${result.stats.true_orphans_high}`);
  console.log(`True orphans (MEDIUM confidence):  ${result.stats.true_orphans_medium}`);
  console.log(`Needs manual review:               ${result.stats.needs_review}`);
  console.log('================================================================================\n');

  // Calculate total size of safe-to-remove files
  const highConfidenceSize = result.true_orphans_high_confidence.reduce((sum, f) => sum + (f.size || 0), 0);
  const mediumConfidenceSize = result.true_orphans_medium_confidence.reduce((sum, f) => sum + (f.size || 0), 0);

  console.log('💾 STORAGE IMPACT:');
  console.log(`HIGH confidence files: ${(highConfidenceSize / 1024).toFixed(2)} KB`);
  console.log(`MEDIUM confidence files: ${(mediumConfidenceSize / 1024).toFixed(2)} KB`);
  console.log(`Total potential savings: ${((highConfidenceSize + mediumConfidenceSize) / 1024).toFixed(2)} KB\n`);

  console.log('✅ Results exported to: server/docs/orphaned-files-categorized.json\n');
