...
class OrphanedFilesAnalyzer {
  ...
  async analyze(): Promise<void> {
    console.log('🔍 Starting comprehensive import dependency analysis...\n');

    // Step 1: Scan all files in the codebase
    console.log('📂 Step 1: Scanning all TypeScript/JavaScript files...');
    await this.scanAllFiles();
    console.log(`   Found ${this.allFiles.size} files\n`);

    // Step 2: Parse all imports
    console.log('🔗 Step 2: Parsing import statements...');
    await this.parseAllImports();
    console.log(`   Found ${this.allImports.size} unique imported files\n`);

    // Step 3: Load CSV data
    console.log('📋 Step 3: Loading orphaned files from CSV...');
    const csvFiles = await this.loadCSV();
    console.log(`   Loaded ${csvFiles.length} files from CSV\n`);

    // Step 4: Cross-reference and categorize
    console.log('🎯 Step 4: Cross-referencing and categorizing...');
    ...
    for (const file of this.allFiles) {
      ...
      try {
        ...
        for (const imp of imports) {
          const normalizedPath = this.normalizeImportPath(imp.importedPath, file);
  ...
  private extractImports(content: string, sourceFile: string): ImportInfo[] {
    ...
    const standardImportRegex = /import\s+(?:(?:\{[^}]*\}|\*\s+as\s+\w+|\w+)(?:\s*,\s*(?:\{[^}]*\}|\*\s+as\s+\w+|\w+))*\s+from\s+)?['"]([^'"]+)['"]/g;
    let match;
   ...
   */
  private normalizeImportPath(importPath: string, sourceFile: string): string | null {
  ...
  private async loadCSV(): Promise<OrphanedFile[]> {
    const csvPath = path.join(this.projectRoot, 'server/docs/orphaned-files.csv');
   ...
   */
  private async categorizeFiles(csvFiles: OrphanedFile[]): Promise<AnalysisResult> {
    ...
    for (const csvFile of csvFiles) {
      ...
      if (importers && importers.length > 0) {
        ...
        falsePositives.push({
          path: csvFile.path,
          imported_by: importers.map(imp => `${imp.importingFile} (${imp.importType})`)
        });
      } else {
        ...
        if (!fs.existsSync(fullPath)) {
          ...
          const isTestFile = normalizedPath.includes('.test.') || normalizedPath.includes('/test/');
          ...
          if (isTypeDeclaration) {
            needsReview.push({
              path: csvFile.path,
              concern: 'Type declaration file - may be used without explicit imports'
            });
          } else if (isTestFile) {
            needsReview.push({
              path: csvFile.path,
            ...
            needsReview.push({
              path: csvFile.path,
              concern: 'Script file - may be executed directly via npm scripts or CLI'
            });
          } else if (isMiddleware || isRoute) {
            needsReview.push({
              path: csvFile.path,
              concern: 'Middleware/Route file - may be registered dynamically or in a way not detected by static analysis'
            });
          } else {
  ...
  private normalizeCsvPath(csvPath: string): string {
    ...
    if (csvPath.startsWith('components/') ||
  ...
  private async exportResults(results: AnalysisResult): Promise<void> {
    const outputPath = path.join(this.projectRoot, 'server/docs/orphaned-files-verified.json');
  ...
  private printSummary(results: AnalysisResult): void {
    ...
    console.log(`Total files in CSV:           ${results.stats.total_csv}`);
    console.log(`Total files scanned:          ${results.stats.total_files_scanned}`);
    console.log(`False positives (imported):   ${results.stats.false_positives}`);
    console.log(`True orphans (not imported):  ${results.stats.true_orphans}`);
    console.log(`Needs manual review:          ${results.stats.needs_review}`);
    console.log('='.repeat(80));

    if (results.false_positives.length > 0) {
      console.log('\n🟢 FALSE POSITIVES (Files that ARE actually imported):');
      console.log('-'.repeat(80));
      results.false_positives.slice(0, 10).forEach(fp => {
        console.log(`\n📄 ${fp.path}`);
        console.log(`   Imported by: ${fp.imported_by.slice(0, 3).join(', ')}`);
        if (fp.imported_by.length > 3) {
          console.log(`   ... and ${fp.imported_by.length - 3} more`);
        }
      });
      if (results.false_positives.length > 10) {
        console.log(`\n... and ${results.false_positives.length - 10} more false positives`);
      }
    }

    if (results.true_orphans.length > 0) {
      console.log('\n🔴 TRUE ORPHANS (Files genuinely not imported):');
      console.log('-'.repeat(80));
      results.true_orphans.slice(0, 15).forEach(orphan => {
        console.log(`📄 ${orphan.path} (${orphan.size} bytes)`);
      });
      if (results.true_orphans.length > 15) {
        console.log(`\n... and ${results.true_orphans.length - 15} more true orphans`);
      }
    }
    ...
    if (results.needs_review.length > 0) {
      ...
      results.needs_review.slice(0, 10).forEach(review => {
        ...
        console.log(`   Concern: ${review.concern}`);
      });
      if (results.needs_review.length > 10) {
        console.log(`\n... and ${results.needs_review.length - 10} more files needing review`);
      }
    }

    console.log('\n' + '='.repeat(80));
    console.log('✨ Analysis complete! Check server/docs/orphaned-files-verified.json for full results.');
    console.log('='.repeat(80) + '\n');
  }
}
