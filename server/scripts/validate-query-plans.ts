...
export class QueryPlanValidator {
	...
	async validateGetProductByPath(
      ...
      `.replace("$1", `'${urlPath.replace(/'/g, "''")}'`);
			...
			const { planningTime, executionTime, plan } =
				this.parseExplainOutput(rawOutput);
			const { indexesUsed, sequentialScans, indexScans } =
				this.extractIndexUsage(plan);
	...
	private generateComparison(metrics: QueryPlanMetrics): ComparisonResults {
		...
		> = {
			...
			getProductsSummary: {
				...
				description:
					"Original: Single query with COUNT(*) OVER() window function",
			},
		...
		const executionChange =
			((metrics.executionTime - baseline.avgExecutionTime) /
	...
	async validateAll(): Promise<{
		...
		return {
			...
			summary: {
				...
				totalSequentialScans: successful.reduce(
					(sum, r) => sum + r.sequentialScans,
					0,
				),
			},
		};
