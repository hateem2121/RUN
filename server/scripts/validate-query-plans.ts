...
export class QueryPlanValidator {
   ...
   */
  private parseExplainOutput(rawOutput: unknown[]): {
    planningTime: number;
    executionTime: number;
    plan: unknown;
  } {
    ...
    return {
      ...
      executionTime: (explainData["Execution Time"] as number) || 0,
   ...
   */
  private extractIndexUsage(plan: unknown): {
    indexesUsed: string[];
    sequentialScans: number;
    indexScans: number;
  } {
    ...
    const traversePlan = (node: Record<string, unknown> | null | undefined) => {
      if (!node) {
        return;
      }
      ...
      if (nodeType === "Seq Scan") {
        ...
        indexScans++;
        if (node["Index Name"]) {
          indexesUsed.add(node["Index Name"] as string);
        }
      }

      if (node.Plans && Array.isArray(node.Plans)) {
        for (const childPlan of node.Plans as Record<string, unknown>[]) {
          traversePlan(childPlan);
        }
      }
    };

    traversePlan(plan as Record<string, unknown>);
   ...
   */
  private extractRowsScanned(plan: unknown): number {
    let totalRows = 0;
    ...
    const traversePlan = (node: Record<string, unknown> | null | undefined) => {
      ...
      if (node["Actual Rows"]) {
        totalRows += node["Actual Rows"] as number;
      }
   ...
   */
  async validateGetProductByPath(
    urlPath: string = "/men/t-shirts/performance-tee",
  ): Promise<QueryPlanMetrics> {
   ...
   */
  async validateGetProductsSummary(
    limit: number = 100,
    offset: number = 0,
  ): Promise<QueryPlanMetrics> {
   ...
   */
  async validateGetMediaAssetsSearch(
   ...
   */
  async validateGetProductsByTag(
    tag: string = "moisture-wicking",
    limit: number = 100,
  ): Promise<QueryPlanMetrics> {
  ...
  private generateComparison(metrics: QueryPlanMetrics): ComparisonResults {
    const baselines: Record<
      string,
      { queryCount: number; avgExecutionTime: number; description: string }
    > = {
      getProductByPath: {
        queryCount: 9,
        avgExecutionTime: 799,
        description:
          "Original: 9 sequential queries (product + fabric + sizeChart + media + certificates + accessories + category products)",
      },
      getProductsSummary: {
        queryCount: 1,
    ...
    const executionChange =
      ((metrics.executionTime - baseline.avgExecutionTime) / baseline.avgExecutionTime) * 100;
    const queryReduction =
      baseline.queryCount > 1
    ...
    return {
      ...
      improvement: {
        ...
        indexEfficiency:
          metrics.indexScans > 0 && metrics.sequentialScans === 0
  ...
  async printValidationReport(): Promise<void> {
    const queries = [
      { name: "getProductByPath", fn: () => this.validateGetProductByPath() },
      {
        ...
        fn: () => this.validateGetProductsSummary(),
      },
      {
        name: "getMediaAssets (search)",
        fn: () => this.validateGetMediaAssetsSearch(),
      },
