...
router.post("/contact", async (req, res) => {
	...
	if (validatedData.honeypot && validatedData.honeypot.trim().length > 0) {
		logger.warn(
			`[Contact] Honeypot triggered - potential spam from ${req.ip}`,
			{
	...
	if (process.env.NODE_ENV === "production" && recaptchaSecret) {
		if (!validatedData.recaptchaToken) {
			...
			return res
				.status(400)
				.json({ success: false, error: "Bot detected (missing token)" });
		}
		...
		try {
			...
			if (!recaptchaData.success || recaptchaData.score < 0.5) {
				logger.warn(
					`[Contact] reCAPTCHA failed for ${req.ip}: score ${recaptchaData.score}`,
				);
				return res
					.status(400)
					.json({ success: false, error: "Bot detected (low score)" });
			}
		} catch (error) {
			...
			return res
				.status(500)
				.json({ success: false, error: "Security check failed" });
		}
	}
	...
	logger.info(
		`[Contact] New inquiry #${inquiry.id} from ${validatedData.email}`,
	);
...
router.get("/contact-info", async (req, res) => {
	...
	if (shouldBypassCache(req)) {
		logger.debug(
			"[Contact] Admin/debug request - bypassing cache for contact info",
		);
	}
...
router.get("/locations", async (req, res) => {
	...
	if (shouldBypassCache(req)) {
		logger.debug(
			"[Contact] Admin/debug request - bypassing cache for locations",
		);
	}
