...
router.get("/", async (_req, res) => {
	try {
		...
		logger.info(
			`[TechnologyRoadmap] Retrieved ${roadmap.length} roadmap items`,
		);
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get roadmap items",
		});
	}
});
...
router.get("/:id", async (req, res) => {
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get roadmap item",
		});
	}
});
...
router.post("/", async (req, res) => {
	try {
		...
		try {
			...
			logger.error(
				"[TechnologyRoadmap] ❌ Cache invalidation failed:",
				cacheError,
			);
		}
...
router.patch("/:id", async (req, res) => {
	try {
		const { id } = idParamSchema.parse(req.params);
		const validation = insertTechnologyRoadmapSchema
			.partial()
			.safeParse(req.body);
...
router.patch("/reorder", async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[TechnologyRoadmap] Reorder validation failed:",
				validation.error,
			);
			return res.status(400).json({
				error: "Validation failed",
		...
		logger.info(
			`[TechnologyRoadmap] Reordered ${updates.length} roadmap items`,
		);
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to reorder roadmap items",
		});
	}
});
