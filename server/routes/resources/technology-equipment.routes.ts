...
router.get("/", async (_req, res) => {
	try {
		...
		logger.info(
			`[TechnologyEquipment] Retrieved ${equipment.length} equipment items`,
		);
...
router.post("/", async (req, res) => {
	try {
		...
		try {
			...
			logger.error(
				"[TechnologyEquipment] ❌ Cache invalidation failed:",
				cacheError,
			);
		}
...
router.patch("/:id", async (req, res) => {
	try {
		const { id } = idParamSchema.parse(req.params);
		const validation = insertTechnologyEquipmentSchema
			.partial()
			.safeParse(req.body);
...
router.patch("/reorder", async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[TechnologyEquipment] Reorder validation failed:",
				validation.error,
			);
			return res.status(400).json({
				error: "Validation failed",
		...
		logger.info(
			`[TechnologyEquipment] Reordered ${updates.length} equipment items`,
		);
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to reorder equipment",
		});
	}
});
