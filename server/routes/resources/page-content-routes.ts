...
router.get("/about-batch", async (_req, res) => {
	...
	try {
		...
		if (cached) {
			...
			res.setHeader(
				"X-Media-Assets-Loaded",
				(cached.mediaAssets?.length || 0).toString(),
			);
		...
		const mediaAssets =
			mediaIds.size > 0
				? await withTimeout(
						getStorage().getMediaAssetsByIds(
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to fetch about batch data",
	...
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to get sustainability hero",
		});
	}
});
...
router.patch("/sustainability-hero", requireAdmin, async (req, res) => {
	try {
		const validation = insertSustainabilityHeroSchema
			.partial()
			.safeParse(req.body);
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to update sustainability hero",
		});
	}
});
	...
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to fetch technology batch data",
			_meta: { responseTime: performance.now() - startTime },
		});
	}
});
	...
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to update technology hero",
		});
	}
});
