...
router.get("/", async (_req, res) => {
	try {
		...
		logger.info(
			`[ManufacturingCapabilities] Retrieved ${capabilities.length} capabilities`,
		);
		return res.json(capabilities);
	} catch (error) {
		logger.error(
			"[ManufacturingCapabilities] Error getting capabilities:",
			error,
		);
...
router.get("/:id", async (req, res) => {
	try {
		...
		logger.error(
			"[ManufacturingCapabilities] Error getting capability:",
			error,
		);
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get capability",
		});
	}
});
...
router.post("/", requireAdmin, async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[ManufacturingCapabilities] Validation failed:",
				validation.error,
			);
		...
		try {
			...
			logger.info(
				"[ManufacturingCapabilities] ✅ Cache invalidated after creation",
			);
		} catch (cacheError) {
			logger.error(
				"[ManufacturingCapabilities] ❌ Cache invalidation failed:",
				cacheError,
			);
		}

		logger.info(
			`[ManufacturingCapabilities] Created capability ${newCapability.id}`,
		);
		return res.status(201).json(newCapability);
	} catch (error) {
		logger.error(
			"[ManufacturingCapabilities] Error creating capability:",
			error,
		);
		return res.status(500).json({
			error:
...
router.patch("/:id", requireAdmin, async (req, res) => {
	try {
		...
		try {
			await CacheOperations.invalidateManufacturing();
			logger.info(
				"[ManufacturingCapabilities] ✅ Cache invalidated after update",
			);
		} catch (cacheError) {
			logger.error(
				"[ManufacturingCapabilities] ❌ Cache invalidation failed:",
		...
		logger.error(
			"[ManufacturingCapabilities] Error updating capability:",
			error,
		);
		return res.status(500).json({
			error:
...
router.delete("/:id", requireAdmin, async (req, res) => {
	try {
		...
		try {
			await CacheOperations.invalidateManufacturing();
			logger.info(
				"[ManufacturingCapabilities] ✅ Cache invalidated after deletion",
			);
		} catch (cacheError) {
			logger.error(
				"[ManufacturingCapabilities] ❌ Cache invalidation failed:",
		...
		logger.error(
			"[ManufacturingCapabilities] Error deleting capability:",
			error,
		);
		return res.status(500).json({
			error:
...
router.patch("/reorder", requireAdmin, async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[ManufacturingCapabilities] Reorder validation failed:",
				validation.error,
			);
			return res.status(400).json(validation.error);
		}
		...
		const updates = await Promise.all(
			validation.data.capabilities.map(
		...
		try {
			await CacheOperations.invalidateManufacturing();
			logger.info(
				"[ManufacturingCapabilities] ✅ Cache invalidated after reorder",
			);
		} catch (cacheError) {
			logger.error(
				"[ManufacturingCapabilities] ❌ Cache invalidation failed:",
		...
		logger.info(
			`[ManufacturingCapabilities] Reordered ${updates.length} capabilities`,
		);
		return res.json({ success: true, updated: updates.length });
	} catch (error) {
		logger.error(
			"[ManufacturingCapabilities] Error reordering capabilities:",
			error,
		);
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to reorder capabilities",
		});
	}
});
