...
router.get("/", async (_req, res) => {
	try {
		...
		logger.info(
			`[SustainabilityInitiatives] Retrieved ${initiatives.length} initiatives`,
		);
		return res.json(initiatives);
	} catch (error) {
		logger.error(
			"[SustainabilityInitiatives] Error getting initiatives:",
			error,
		);
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get initiatives",
		});
	}
});
...
router.get("/:id", async (req, res) => {
	try {
		...
		logger.error(
			"[SustainabilityInitiatives] Error getting initiative:",
			error,
		);
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get initiative",
		});
	}
});
...
router.post("/", requireAdmin, async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[SustainabilityInitiatives] Validation failed:",
				validation.error,
			);
		...
		try {
			...
			logger.info(
				"[SustainabilityInitiatives] ✅ Cache invalidated after creation",
			);
		} catch (cacheError) {
			logger.error(
				"[SustainabilityInitiatives] ❌ Cache invalidation failed:",
				cacheError,
			);
		}

		logger.info(
			`[SustainabilityInitiatives] Created initiative ${newInitiative.id}`,
		);
		return res.status(201).json(newInitiative);
	} catch (error) {
		logger.error(
			"[SustainabilityInitiatives] Error creating initiative:",
			error,
		);
		return res.status(500).json({
			error:
...
router.patch("/:id", requireAdmin, async (req, res) => {
	try {
		const { id } = idParamSchema.parse(req.params);
		const validation = insertSustainabilityInitiativeSchema
			.partial()
			.safeParse(req.body);
		...
		try {
			await CacheOperations.invalidateSustainability();
			logger.info(
				"[SustainabilityInitiatives] ✅ Cache invalidated after update",
			);
		} catch (cacheError) {
			logger.error(
				"[SustainabilityInitiatives] ❌ Cache invalidation failed:",
		...
		logger.error(
			"[SustainabilityInitiatives] Error updating initiative:",
			error,
		);
		return res.status(500).json({
			error:
...
router.delete("/:id", requireAdmin, async (req, res) => {
	try {
		...
		try {
			await CacheOperations.invalidateSustainability();
			logger.info(
				"[SustainabilityInitiatives] ✅ Cache invalidated after deletion",
			);
		} catch (cacheError) {
			logger.error(
				"[SustainabilityInitiatives] ❌ Cache invalidation failed:",
		...
		logger.error(
			"[SustainabilityInitiatives] Error deleting initiative:",
			error,
		);
		return res.status(500).json({
			error:
...
router.patch("/reorder", requireAdmin, async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[SustainabilityInitiatives] Reorder validation failed:",
				validation.error,
			);
			return res.status(400).json({
				error: "Validation failed",
		...
		try {
			await CacheOperations.invalidateSustainability();
			logger.info(
				"[SustainabilityInitiatives] ✅ Cache invalidated after reorder",
			);
		} catch (cacheError) {
			logger.error(
				"[SustainabilityInitiatives] ❌ Cache invalidation failed:",
		...
		logger.info(
			`[SustainabilityInitiatives] Reordered ${updates.length} initiatives`,
		);
		return res.json({ success: true, updated: updates.length });
	} catch (error) {
		logger.error(
			"[SustainabilityInitiatives] Error reordering initiatives:",
			error,
		);
		return res.status(500).json({
			error:
