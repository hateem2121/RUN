...
router.get("/", async (_req, res) => {
	try {
		...
		logger.info(
			`[ManufacturingQualities] Retrieved ${qualities.length} quality items`,
		);
		return res.json(qualities);
	} catch (error) {
		logger.error(
			"[ManufacturingQualities] Error getting quality items:",
			error,
		);
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get quality items",
		});
	}
});
...
router.get("/:id", async (req, res) => {
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get quality item",
		});
	}
});
...
router.post("/", requireAdmin, async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[ManufacturingQualities] Validation failed:",
				validation.error,
			);
		...
		try {
			...
			logger.info(
				"[ManufacturingQualities] ✅ Cache invalidated after creation",
			);
		} catch (cacheError) {
			logger.error(
				"[ManufacturingQualities] ❌ Cache invalidation failed:",
				cacheError,
			);
		}

		logger.info(
			`[ManufacturingQualities] Created quality item ${newQuality.id}`,
		);
		return res.status(201).json(newQuality);
	} catch (error) {
		logger.error(
			"[ManufacturingQualities] Error creating quality item:",
			error,
		);
		return res.status(500).json({
			error:
...
router.patch("/:id", requireAdmin, async (req, res) => {
	try {
		...
		logger.error(
			"[ManufacturingQualities] Error updating quality item:",
			error,
		);
		return res.status(500).json({
			error:
...
router.delete("/:id", requireAdmin, async (req, res) => {
	try {
		...
		try {
			await CacheOperations.invalidateManufacturing();
			logger.info(
				"[ManufacturingQualities] ✅ Cache invalidated after deletion",
			);
		} catch (cacheError) {
			logger.error(
				"[ManufacturingQualities] ❌ Cache invalidation failed:",
		...
		logger.error(
			"[ManufacturingQualities] Error deleting quality item:",
			error,
		);
		return res.status(500).json({
			error:
...
router.patch("/reorder", requireAdmin, async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[ManufacturingQualities] Reorder validation failed:",
				validation.error,
			);
			return res.status(400).json(validation.error);
		}
		...
		try {
			await CacheOperations.invalidateManufacturing();
			logger.info(
				"[ManufacturingQualities] ✅ Cache invalidated after reorder",
			);
		} catch (cacheError) {
			logger.error(
				"[ManufacturingQualities] ❌ Cache invalidation failed:",
		...
		logger.info(
			`[ManufacturingQualities] Reordered ${updates.length} quality items`,
		);
		return res.json({ success: true, updated: updates.length });
	} catch (error) {
		logger.error(
			"[ManufacturingQualities] Error reordering quality items:",
			error,
		);
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to reorder quality items",
		});
	}
});
