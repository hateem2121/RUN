...
const invalidateHomepageCache = async () => {
	try {
		...
		logger.info(
			"[Homepage Management] ✅ Cache invalidated via CacheOperations",
		);
	} catch (err) {
...
router.get(
	...
	asyncHandler(async (req, res) => {
		...
		if (shouldBypassCache(req)) {
			logger.debug(
				"[Homepage] Admin/debug request - bypassing cache for slogans",
			);
		}
...
router.get(
	...
	asyncHandler(async (req, res) => {
		if (shouldBypassCache(req)) {
			logger.debug(
				"[Homepage] Admin request - bypassing cache for process cards",
			);
		}
...
router.patch(
	...
	asyncHandler(async (req, res) => {
		const id = parseInt(req.params.id!);
		const validation = insertHomepageProcessCardSchema
			.partial()
			.safeParse(req.body);
		if (!validation.success) {
			return res.status(400).json({ error: validation.error.message });
		}
		const card = await getStorage().updateHomepageProcessCard(
			id,
			validation.data,
		);
		if (!card) {
...
router.patch(
	...
	asyncHandler(async (req, res) => {
		...
		const reorderedCards =
			await getStorage().reorderHomepageProcessCards(cards);
		await invalidateHomepageCache();
...
router.get(
	...
	asyncHandler(async (req, res) => {
		...
		if (shouldBypassCache(req)) {
			logger.debug(
				"[Homepage] Admin/debug request - bypassing cache for sections",
			);
		}
...
router.patch(
	...
	asyncHandler(async (req, res) => {
		const id = parseInt(req.params.id!);
		const validation = insertHomepageSectionSchema
			.partial()
			.safeParse(req.body);
		if (!validation.success) {
			return res.status(400).json({ error: validation.error.message });
...
router.patch(
	"/homepage-featured-products-settings",
	asyncHandler(async (req, res) => {
		const validation = insertHomepageFeaturedProductsSettingsSchema.safeParse(
			req.body,
		);
		if (!validation.success) {
			return res.status(400).json({ error: validation.error.message });
...
logger.debug(
	"[Homepage Management] ✅ Homepage management routes loaded (resources/, 21 routes)",
);
