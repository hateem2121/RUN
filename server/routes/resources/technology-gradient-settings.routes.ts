...
router.get("/", async (_req, res) => {
  ...
  try {
    ...
    if (cached) {
      logger.info("[TechnologyGradientSettings] ✅ Returning cached gradient settings");
      res.setHeader("X-Cache-Hit", "true");
      res.setHeader("X-Response-Time", (performance.now() - startTime).toString());
      return res.json(cached);
    }

    logger.info("[TechnologyGradientSettings] Cache miss - fetching from database");
    ...
    if (settings) {
      ...
      logger.info("[TechnologyGradientSettings] Settings cached for 120 minutes / 2 hours");
    }

    res.setHeader("X-Cache-Hit", "false");
    res.setHeader("X-Response-Time", (performance.now() - startTime).toString());
    logger.info("[TechnologyGradientSettings] Retrieved gradient settings from database");
    return res.json(settings || null);
  } catch (error) {
    logger.error("[TechnologyGradientSettings] Error getting settings:", error);
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to get gradient settings",
    });
  }
});
  ...
  try {
    ...
    if (frontendValidation.success) {
      ...
      if (!dbValidation.success) {
        logger.warn("[TechnologyGradientSettings] Validation failed for both formats:", {
    ...
    const finalValidation = insertTechnologyGradientSettingsSchema.partial().safeParse(storageData);
    ...
    try {
      ...
      logger.info("[TechnologyGradientSettings] ✅ Cache invalidated after update");
    } catch (cacheError) {
      logger.error("[TechnologyGradientSettings] ❌ Cache invalidation failed:", cacheError);
    }

    logger.info("[TechnologyGradientSettings] Settings updated successfully");
    return res.json(updated);
  } catch (error) {
    logger.error("[TechnologyGradientSettings] Error updating settings:", error);
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to update gradient settings",
    });
  }
});
