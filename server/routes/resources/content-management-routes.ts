		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to get contact page configuration",
		});
	}
});
...
router.post("/contact-page-configuration", async (req, res) => {
	try {
		...
		try {
			...
			logger.info(
				"[Contact] ✅ Cache invalidated after contact page configuration creation",
			);
		} catch (err) {
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to create contact page configuration",
		});
	}
});
...
router.patch("/contact-page-configuration", async (req, res) => {
	try {
		...
		try {
			await CacheOperations.invalidateContact();
			logger.info(
				"[Contact] ✅ Cache invalidated after contact page configuration update",
			);
		} catch (err) {
			logger.error("[Contact] ❌ Cache invalidation failed:", err);
			// Don't throw - cache failure should not block DB mutation
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to update contact page configuration",
		});
	}
});
...
router.get("/navigation-items", async (req, res) => {
	...
	try {
		...
		if (shouldBypassCache(req)) {
			logger.info(
				"[Navigation] Admin request detected - bypassing cache for real-time data",
			);
		...
		if (cached) {
			...
			const normalized = normalizeNavigationItems(
		...
		await unifiedCache.set(
			cacheKey,
			normalizedItems,
			CACHE_TTL_NAVIGATION * 1000,
		);
		logger.info(
			"[Navigation] Navigation items cached for 120 minutes / 2 hours (public)",
		);
...
router.post("/navigation-items", async (req, res) => {
	try {
		...
		return res
			.status(500)
			.json({ message: "Failed to create navigation item" });
	}
});
...
router.patch("/navigation-items/:id", async (req, res) => {
	try {
		...
		return res
			.status(500)
			.json({ message: "Failed to update navigation item" });
	}
});
...
router.delete("/navigation-items/:id", async (req, res) => {
	try {
		...
		return res
			.status(500)
			.json({ message: "Failed to delete navigation item" });
	}
});
	...
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to get navigation glassmorphism settings",
		});
	}
});
...
router.patch("/navigation-glassmorphism-settings", async (req, res) => {
	try {
		const validation = insertNavigationGlassmorphismSettingsSchema.safeParse(
			req.body,
		);
		if (!validation.success) {
			return res.status(400).json({ error: validation.error.message });
		...
		const settingsData = {
			...validation.data,
			opacity:
				validation.data.opacity !== undefined
					? String(validation.data.opacity)
					: undefined,
		};
		const settings = await withTimeout(
	...
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to get logo animation settings",
		});
	}
});
	...
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to update logo animation settings",
		});
	}
});
