...
router.get("/navigation-test", async (_req, res) => {
  return res.json({ message: "Navigation route system working", timestamp: new Date().toISOString() });
});
...
router.get("/navigation-items", async (req, res) => {
  ...
  try {
    ...
    const normalizeNavigationItems = (items: any[]) => items.map(item => ({
      ...item,
    ...
    if (shouldBypassCache(req)) {
      ...
      logger.debug('[Navigation DEBUG] Raw from DB:', JSON.stringify(navigationItems.slice(0, 2), null, 2));
      const normalizedItems = normalizeNavigationItems(navigationItems);
      logger.debug('[Navigation DEBUG] After normalization:', JSON.stringify(normalizedItems.slice(0, 2), null, 2));
...
router.patch("/navigation-items/reorder", async (req, res) => {
  try {
    ...
    logger.debug(`[Transaction] Bulk navigation reorder completed in ${duration}ms (${validatedData.items.length} items)`);
...
router.patch("/navigation-items/:id", async (req, res) => {
  try {
    ...
    const updateData = {
      ...validatedData,
      ...(validatedData.isActive !== undefined && { isActive: validatedData.isActive ?? true }),
      ...(validatedData.showOnDesktop !== undefined && { showOnDesktop: validatedData.showOnDesktop ?? true }),
      ...(validatedData.showOnMobile !== undefined && { showOnMobile: validatedData.showOnMobile ?? true }),
    };
...
router.get("/navigation-glassmorphism-settings", async (_req, res) => {
  try {
    ...
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to get navigation glassmorphism settings"
    });
  }
});
...
router.patch("/navigation-glassmorphism-settings", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to update navigation glassmorphism settings"
    });
  }
});
