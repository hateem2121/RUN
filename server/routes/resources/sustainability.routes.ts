			...
			error: {
				message:
					error instanceof Error
						? error.message
						: "Failed to get sustainability config",
			},
		});
	}
});
...
router.patch("/", async (req, res) => {
	try {
		...
		if (!updated) {
			logger.warn(
				`[Sustainability] Update failed - no record updated [${reqId}]`,
			);
			return res.status(404).json({
				success: false,
				error: {
					message:
						"Sustainability configuration not found or could not be updated",
				},
			});
		}
		...
		logger.info(
			`[Sustainability] ✅ Cache invalidated after update [${reqId}]`,
		);
		...
		return res.status(500).json({
			...
			error: {
				message:
					error instanceof Error
						? error.message
						: "Failed to update sustainability config",
			},
		});
	}
});
