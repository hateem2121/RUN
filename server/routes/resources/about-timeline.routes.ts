		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to get timeline entries",
		});
	}
});
...
router.get("/:id", async (req, res) => {
	try {
		...
		const entry = await withTimeout(
			aboutService.getTimelineEntry(id),
			10000,
			"Get timeline entry",
		);
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get timeline entry",
		});
	}
});
...
router.patch("/:id", requireAdmin, async (req, res) => {
	try {
		const { id } = idParamSchema.parse(req.params);
		const validation = insertAboutTimelineEntrySchema
			.partial()
			.safeParse(req.body);
...
router.patch("/reorder", requireAdmin, async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[AboutTimeline] Reorder validation failed:",
				validation.error,
			);
			return res.status(400).json({
				error: "Validation failed",
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to reorder timeline entries",
		});
	}
});
