...
router.get("/", async (_req, res) => {
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get team message",
		});
	}
});
...
router.patch("/", requireAdmin, async (req, res) => {
	try {
		const validation = insertAboutTeamMessageSchema
			.partial()
			.safeParse(req.body);
		...
		try {
			...
			logger.error(
				"[AboutTeamMessage] ❌ Cache invalidation failed:",
				cacheError,
			);
		}
