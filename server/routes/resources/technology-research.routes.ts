...
router.get("/", async (_req, res) => {
	try {
		...
		logger.info(
			`[TechnologyResearch] Retrieved ${research.length} research items`,
		);
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get research items",
		});
	}
});
...
router.get("/:id", async (req, res) => {
	try {
		...
		return res.status(500).json({
			error:
				error instanceof Error ? error.message : "Failed to get research item",
		});
	}
});
...
router.post("/", async (req, res) => {
	try {
		...
		const newResearch = await withTimeout(
			getStorage().createTechnologyResearch(validation.data),
			10000,
		...
		try {
			...
			logger.error(
				"[TechnologyResearch] ❌ Cache invalidation failed:",
				cacheError,
			);
		}
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to create research item",
		});
	}
});
...
router.patch("/:id", async (req, res) => {
	try {
		const { id } = idParamSchema.parse(req.params);
		const validation = insertTechnologyResearchSchema
			.partial()
			.safeParse(req.body);
		...
		const updated = await withTimeout(
			getStorage().updateTechnologyResearch(id, validation.data),
			10000,
		...
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to update research item",
		});
	}
});
...
router.patch("/reorder", async (req, res) => {
	try {
		...
		if (!validation.success) {
			logger.warn(
				"[TechnologyResearch] Reorder validation failed:",
				validation.error,
			);
			return res.status(400).json({
				error: "Validation failed",
		...
		const updates = await Promise.all(
			validation.data.research.map(({ id, position }) =>
		...
		logger.info(
			`[TechnologyResearch] Reordered ${updates.length} research items`,
		);
		return res.json({ success: true, updated: updates.length });
	} catch (error) {
		logger.error(
			"[TechnologyResearch] Error reordering research items:",
			error,
		);
		return res.status(500).json({
			error:
				error instanceof Error
					? error.message
					: "Failed to reorder research items",
		});
	}
});
