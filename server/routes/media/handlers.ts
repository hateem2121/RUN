...
export async function finalizeUpload(req: Request, res: Response) {
  try {
    ...
    // LOG: Upload session info
    console.log(
      `[Finalize Upload] Starting finalization for: ${session.filename}`,
    );
    console.log(`[Finalize Upload] → MIME Type: ${session.mimeType}`);
    console.log(`[Finalize Upload] → Total Size: ${session.totalSize} bytes`);
    console.log(
      `[Finalize Upload] → Chunks: ${session.receivedChunks.size}/${session.totalChunks}`,
    );
    ...
    try {
      ...
      console.log(
        `[Finalize Upload] ✅ Assembled ${chunks.length} chunks into ${assembledFile.length} bytes (parallel download: ${assemblyTime}ms)`,
      );
      ...
      try {
        ...
        console.log(`[Finalize Upload] → Detected media type: ${mediaType}`);

        const storageKey = generateOrganizedStoragePath(
          mediaType,
          session.filename,
        );
        console.log(`[Finalize Upload] → Generated storage key: ${storageKey}`);

        finalStorageKey = storageKey;
        await appStorageService.uploadAsset(storageKey, assembledFile);
        console.log(`[Finalize Upload] ✅ Uploaded to storage: ${storageKey}`);
        ...
        console.log(`[Finalize Upload] → File type: ${fileType}`);
  ...
  try {
    ...
    if (asset.type === "image" && asset.imageVariants?.original) {
      ...
      if (variantExists) {
        ...
        logger.warn(`Compressed variant missing for asset ${id}, falling back to original storage path`, {
    ...
    try {
      ...
      fs.appendFileSync('debug_media_error.log', `[${new Date().toISOString()}] Error serving media ${req.params.id}: ${JSON.stringify(serializeError(error), null, 2)}\n`);
    } catch (err) {
...
export async function batchGetContent(req: Request, res: Response) {
  try {
    ...
    const contents = await Promise.all(
      idList.map(async (id) => {
        ...
        try {
          ...
          logger.error(`[Batch Content] Failed to generate signed URL for asset ${id}:`, serializeError(error));
    ...
    const urlStrategy = process.env.NODE_ENV === 'production' 
      ? `all signed (${validContents.length})` 
      : `models signed (${modelCount}), others proxy (${otherCount})`;
    
    logger.info(`[Batch Content] ✅ Returned ${validContents.length}/${idList.length} assets [${urlStrategy}] (cache: ${cacheDuration}s)`);
...
export async function getCacheStats(_req: Request, res: Response) {
  try {
    ...
    if (healthStatus.stats.calculatedSize / (100 * 1024 * 1024) > 0.8) {
      recommendations.push("Cache approaching size limit - consider implementing cache eviction strategy");
    }
