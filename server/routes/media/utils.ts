...
export function generateOrganizedStoragePath(
  ...
  if (cdn.partitioning.enabled) {
    const partition = visibility === 'public' 
...
export function detectMediaType(mimeType: string): 'images' | 'videos' | 'models' | 'documents' {
  ...
  if (mimeType.includes('gltf') || mimeType.includes('glb') || mimeType.startsWith('model/')) return 'models';
...
export function toBuffer(x: unknown): Buffer | null {
  ...
  if (typeof x === 'object' && x !== null && 'type' in x && x.type === 'Buffer' && 'data' in x && Array.isArray(x.data)) 
...
export async function processUploadedFile(file: Express.Multer.File, options: UploadOptions = {}): Promise<MediaAsset> {
  ...
  try {
    ...
    // LOG: Original MIME type before correction
    console.log(`[Upload Processing] File: ${file.originalname}`);
    console.log(`[Upload Processing] → Original MIME: ${file.mimetype}`);
    console.log(`[Upload Processing] → Extension: .${extension}`);
    
    // Correct MIME type
    const correctedMime = correctMimeType(file.mimetype, file.originalname);
    
    // LOG: Corrected MIME type
    console.log(`[Upload Processing] → Corrected MIME: ${correctedMime}`);
    ...
    // LOG: Determined file type
    console.log(`[Upload Processing] → Detected Type: ${fileType}`);
    ...
    if (fileType === 'image') {
      try {
        ...
        try {
          ...
          logger.warn('Responsive variant generation failed (single upload):', serializeError(variantError));
        }
    ...
    // Compensating transaction: Delete uploaded file if DB operations fail
    // This prevents orphaned files in Object Storage when database operations fail
    if (storageKey) {
      logger.warn('Upload failed, cleaning up storage', {
        filename: file.originalname,
        storageKey,
        error: serializeError(error)
      });
      
      await appStorageService.deleteAsset(storageKey).catch(cleanupError =>
        logger.error('Failed to cleanup orphaned file:', { storageKey, error: serializeError(cleanupError) })
      );
    }
    throw error; // Re-throw original error for proper client notification
  }
}
