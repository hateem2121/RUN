...
export const enhancedValidation = {
	...
	sanitizeAsset: (asset: MediaAsset) => {
		return {
			...asset,
			filename:
				asset.filename?.replace(/[^a-zA-Z0-9._-]/g, "_") || asset.filename,
			originalName:
				asset.originalName?.replace(/[^a-zA-Z0-9._\-\s]/g, "_") ||
				asset.originalName,
	...
	validateAsset: (asset: MediaAsset): ValidationResult => {
		...
		if (
			asset.fileSize &&
			!enhancedValidation.validateFileSize(asset.fileSize, asset.type)
		) {
		...
		if (
			asset.mimeType &&
			!enhancedValidation.validateMimeType(asset.mimeType, asset.filename)
		) {
...
export function buildInsertMediaAsset(
	...
	try {
		...
		if (fileType === "image") {
			try {
				...
				let imageVariants;
				try {
					const { generateResponsiveVariants } = await import(
						"../../image-processor.js"
					);
