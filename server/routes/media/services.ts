...
export const enhancedUploadService = {
  ...
  cleanupFileHandles: () => {
    logger.info(`[File Handle Cleanup] Cleaning up ${enhancedUploadService.activeFileOperations.size} active operations`);
  ...
  uploadChunk: async (
    ...
    await withTimeout(appStorageService.uploadAsset(chunkKey, buffer, { isPublic: CHUNK_STORAGE_IS_PUBLIC }), 30000, 'Upload chunk to object storage');
  ...
  uploadRawChunk: async (uploadId: string, chunkNumber: number, chunkData: Buffer, metadata: {
    chunkSize: number;
    chunkHash: string;
    totalChunks: number;
  }) => {
    ...
    if (chunkData.length !== metadata.chunkSize) {
      throw new Error(`Chunk size mismatch: expected ${metadata.chunkSize}, got ${chunkData.length}`);
    }
    ...
    if (computedHash !== metadata.chunkHash) {
      logger.error(`[Raw Chunk] Hash mismatch for chunk ${chunkNumber}: expected ${metadata.chunkHash}, got ${computedHash}`);
    ...
    // Use shared CHUNK_STORAGE_BASE constant to ensure upload/finalize path consistency
    const chunkKey = `${CHUNK_STORAGE_BASE}/${uploadId}/chunk-${chunkNumber}`;
    await withTimeout(appStorageService.uploadAsset(chunkKey, chunkData, { isPublic: CHUNK_STORAGE_IS_PUBLIC }), 30000, 'Upload raw chunk to object storage');
    session.receivedChunks.set(chunkNumber, true); // Track receipt, not data

    const progress = Math.round((session.receivedChunks.size / session.totalChunks) * 100);
    const isComplete = session.receivedChunks.size === session.totalChunks;

    logger.debug(`[Raw Chunk] ✅ Chunk ${chunkNumber}/${session.totalChunks} uploaded with integrity verified (${progress}%)`);
  ...
  finalizeUpload: async (uploadId: string) => {
    ...
    try {
      ...
      logger.info(`[ChunkAssembly] 🔧 Starting parallel assembly for ${session.totalChunks} chunks, expected total: ${session.totalSize} bytes`);
      ...
      for (let batchStart = 0; batchStart < session.totalChunks; batchStart += PARALLEL_BATCH_SIZE) {
        ...
        const batchResults = await withTimeout(Promise.all(batchPromises), 60000, 'Download chunk batch from storage');
      ...
      if (computedTotal !== session.totalSize) {
        throw new Error(`Size mismatch: computed ${computedTotal} bytes, expected ${session.totalSize} bytes`);
      }

      logger.info(`[ChunkAssembly] 🔧 Performing single Buffer.concat with ${orderedChunks.length} chunks, exact total: ${computedTotal}`);
      ...
      if (assembledFile.length !== session.totalSize) {
        logger.error(`[ChunkAssembly] Size mismatch detected! Expected: ${session.totalSize}, Got: ${assembledFile.length}`);
        throw new Error(
      ...
      if (isGLTFFile(session.mimeType, session.filename)) {
        ...
        logger.debug(`[GLTF] Assembled file size: ${assembledFile.length} bytes, expected: ${session.totalSize} bytes`);
        ...
        if (assembledFile.length >= 12) {
          ...
          const magicMatch = magic[0] === 0x67 && magic[1] === 0x6C && magic[2] === 0x54 && magic[3] === 0x46; // 'glTF'
        ...
        try {
          ...
          if (jsonText.includes('"asset"') && jsonText.includes('"version"')) {
            ...
            if (largerText.includes('"asset"')) {
              ...
              logger.info(`[GLTF] Large GLTF file - accepting based on JSON structure (${assembledFile.length} bytes)`);
              hasAssetProperty = true;
            }
          }
        ...
        if (!isValidGLTFFormat) {
          logger.error(`[GLTF] Assembled file failed format validation - may be corrupted during chunk assembly`);
        ...
        try {
          ...
          const embedResult = await withTimeout(gltfProcessor.processForUpload(assembledFile), 60000, 'Process GLTF file for upload');
          ...
          const validation = await withTimeout(gltfProcessor.validateForProductionUpload(assembledFile), 20000, 'Validate GLTF for production upload');
          ...
          logger.info(`[GLTF] Successfully processed and validated ${session.filename} - ${embedResult.texturesEmbedded} textures embedded, ${embedResult.externalReferencesRemoved} external refs removed`);
        } catch (gltfError) {
          logger.error(`[GLTF] Processing/validation failed for ${session.filename}:`, serializeError(gltfError));
          uploadSessions.delete(uploadId);
          throw gltfError;
        }
      }
      ...
      await withTimeout(appStorageService.uploadAsset(storagePath, assembledFile, {
        contentType: session.mimeType,
        isPublic: true,
      }), 30000, 'Upload final assembled file to object storage');
      ...
      try {
        createdAsset = await withTimeout(storage.createMediaAsset(metadata), 10000, 'Create media asset database record');
        if (!createdAsset) {
          throw new Error("Database creation returned null");
        }

        // FIX: Update URL to use API proxy pattern like other media files
        const correctUrl = `/api/media/${createdAsset.id}/content`;
        const updatedAsset = await withTimeout(storage.updateMediaAsset(createdAsset.id, {
          url: correctUrl,
        }), 10000, 'Update media asset URL in database');
        ...
        try {
          await withTimeout(appStorageService.deleteAsset(storagePath), 10000, 'Delete uploaded file from storage (cleanup)');
        } catch (cleanupError) {
          logger.error(
      ...
      for (let i = 0; i < session.totalChunks; i++) {
        const chunkKey = `${CHUNK_STORAGE_BASE}/${uploadId}/chunk-${i}`;
        try {
          await withTimeout(appStorageService.deleteAsset(chunkKey), 10000, 'Delete chunk from storage (cleanup)');
