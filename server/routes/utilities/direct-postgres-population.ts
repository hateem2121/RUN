...
export function registerDirectPostgresPopulationRoutes(app: Express): void {
	...
	app.post("/api/direct-postgres/populate-all", async (_req, res) => {
		try {
			logger.debug(
				"[Direct PostgreSQL] 🚀 Populating all 47 business items directly...",
			);
			...
			const results = await db.transaction(async (tx) => {
				...
				for (const fabric of fabricData) {
					...
					logger.debug(
						`[Direct PostgreSQL] ✅ Created fabric: ${created.name} ID: ${created.id}`,
					);
				}
				...
				for (const cert of certificateData) {
					...
					logger.debug(
						`[Direct PostgreSQL] ✅ Created certificate: ${created.name}`,
					);
				}
				...
				const accessoryData = [
					...
					},
					{
						...
						description:
							'Muscle recovery tool - High-density foam, textured surface, 18" length',
						isActive: true,
					},
					{
						name: "Running Belt",
				...
				for (const accessory of accessoryData) {
					...
					logger.debug(
						`[Direct PostgreSQL] ✅ Created accessory: ${created.name}`,
					);
				}
			...
			logger.error(
				"[Direct PostgreSQL] ❌ Failed to populate PostgreSQL:",
				error,
			);
			res.status(500).json({
				success: false,
	...
	logger.debug(
		"[Direct PostgreSQL] ✅ Direct PostgreSQL population routes registered",
	);
}
