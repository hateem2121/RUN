  ...
  app.post('/api/direct-postgres/populate-all', async (_req, res) => {
    try {
      ...
      const results = await db.transaction(async (tx) => {
        ...
        // Create 3 Categories
        const categoryData = [
          { name: "EVERYDAY RUN", slug: "everyday-run", description: "Comfortable apparel for daily running activities", isActive: true, sortOrder: 10 },
          { name: "RUN AS ONE", slug: "run-as-one", description: "Team-focused running gear and apparel", isActive: true, sortOrder: 20 },
          { name: "ACTIVE RUN", slug: "active-run", description: "High-performance gear for intensive running", isActive: true, sortOrder: 30 }
        ];
        ...
        for (const cat of categoryData) {
          ...
          logger.debug(`[Direct PostgreSQL] ✅ Created category: ${created.name} ID: ${created.id}`);
        }
        ...
        for (const fiber of fiberData) {
          ...
          logger.debug(`[Direct PostgreSQL] ✅ Created fiber: ${created.name} Score: ${created.sustainabilityScore}`);
        }
      ...
      logger.debug(`[Transaction] Direct PostgreSQL population completed in ${duration}ms (${totalCreated} items across 5 tables)`);
      logger.debug(`[Direct PostgreSQL] 🎉 Successfully populated PostgreSQL with ${totalCreated} business items`);
