	...
	if (baseResponse.certificateIds && baseResponse.certificateIds.length > 0) {
		try {
			...
			const certificateMap = new Map(
				fetchedCertificates.map((cert) => [cert.id, cert]),
			);
...
router.patch(
    ...
    */
		...
		if (validatedData.navigationColumns) {
			normalizedData.navigationColumns = validatedData.navigationColumns.map(
				(col: any) => ({
					title: col.title,
					links:
		...
		if (validatedData.socialLinks) {
			normalizedData.socialLinks = validatedData.socialLinks.map(
				(social: any) => ({
...
logger.debug(
	"[Footer Config Routes] ✅ Footer configuration routes loaded (Drizzle Optimized)",
);
