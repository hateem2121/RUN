...
export async function optimizeMemory(req: Request, res: Response) {
  try {
    ...
    let result;
    if (emergency) {
      result = await withTimeout(memoryOptimizer.emergencyCleanup(), 15000, 'Emergency memory cleanup');
    } else {
...
export async function getBusinessMetrics(_req: Request, res: Response) {
  try {
    const metrics = await withTimeout(businessIntelligence.collectBusinessMetrics(), 10000, 'Collect business metrics');
...
export async function generateBusinessReport(_req: Request, res: Response) {
  try {
    const report = await withTimeout(businessIntelligence.generateBusinessReport(), 15000, 'Generate business report');
    ...
    logger.error('[Operational] Failed to generate business report:', { error });
    res.status(500).json({
...
export async function executeWorkflow(req: Request, res: Response) {
  try {
    ...
    const result = await withTimeout(workflowAutomation.executeWorkflowManually(workflowId!), 20000, 'Execute workflow manually');
...
export async function getOperationalDashboard(_req: Request, res: Response) {
  try {
    ...
    logger.error('[Operational] Failed to get operational dashboard:', { error });
    res.status(500).json({
