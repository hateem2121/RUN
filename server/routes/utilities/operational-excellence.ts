...
export async function getOperationalDashboard(_req: Request, res: Response) {
	try {
		...
		const [memoryStats, businessMetrics, workflows, rateLimitStats] =
			await withTimeout(
				Promise.all([
		...
		const failedWorkflows = workflows.filter(
			(w) => w.failureCount > w.successCount,
		);
		...
		const dashboard = {
			...
			api: {
				...
				activeClients: activeClients.length,
				clientTiers: activeClients.reduce(
	...
	try {
		...
		const status = {
			services: {
				...
				workflows: workflows.every((w) => w.enabled)
					? "operational"
					: "partial",
