...
export function registerMigrationExecutionRoutes(app: Express): void {
  ...
  app.get('/api/migrate/verify-completion', async (_req, res) => {
    try {
      ...
      const totalMigrated = Object.values(postgresqlCounts).reduce((sum, count) => sum + count, 0);
      ...
      res.json({
        ...
        data: {
          ...
          verification: {
            ...
            homepageContentPresent: (postgresqlCounts.homepageHero + postgresqlCounts.homepageSections) > 0
          }
        },
        timestamp: new Date().toISOString()
      });
