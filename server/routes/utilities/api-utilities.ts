...
export function setupResourceRoutes(app: Application, storage: IStorage) {
	...
	app.get("/api/resources/search", async (req: Request, res: Response) => {
		try {
			...
			logger.debug(
				`[Resource Search] Found ${results.length} results in ${responseTime}ms`,
			);
		...
		(_req: Request, res: Response) => {
			try {
				...
				res.json({
					success: true,
					message:
						"Emergency rollback activated - all experimental features disabled",
					flags: result.flags,
					timestamp: new Date().toISOString(),
				});
	...
	switch (type) {
		...
		case "fabrics":
			...
			if (item.compositions) {
				(item.compositions as Array<Record<string, unknown>>)?.forEach(
					(comp) => {
						if (comp.name) parts.push(String(comp.name));
						(comp.fibers as Array<Record<string, unknown>>)?.forEach(
							(fiber) => {
