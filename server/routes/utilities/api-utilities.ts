...
const featureFlags = {
  ...
  emergencyRollback: function() { this._flags = {}; },
  setRuntimeOverride: function(flag: string, enabled: boolean) { this._flags[flag] = enabled; }
};

// PHASE 2B: Simplified search schema without pagination
const searchSchema = z.object({
  q: z.string().optional(),
  type: z.enum(['all', 'certificate', 'accessory', 'sizechart', 'fabric', 'fiber']).optional().default('all'),
...
function createSearchableText(item: Record<string, unknown>, type: string): string {
  ...
  switch (type) {
    ...
    case 'fibers':
      ...
      if (item.sustainabilityScore) parts.push(`sustainability-${String(item.sustainabilityScore)}`);
      break;
  }
...
function calculateRelevanceScore(item: Record<string, unknown>, searchWords: string[], _type: string): number {
  let score = 0;
