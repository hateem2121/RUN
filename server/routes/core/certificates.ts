	...
	try {
		...
		const validatedData = {
			...
			issueDate: validation.issueDate
				? new Date(validation.issueDate)
				: undefined,
			expiryDate: validation.expiryDate
				? new Date(validation.expiryDate)
				: undefined,
		};
		...
		try {
			...
			logger.info(
				"[Certificates] ✅ Cache invalidated after certificate creation",
			);
		} catch (err) {
...
router.put("/certificates/:id", async (req, res) => {
	try {
		...
		try {
			...
			await CacheOperations.invalidateSustainability();
			logger.info(
				"[Certificates] ✅ Cache invalidated after certificate update",
			);
		} catch (err) {
			logger.error("[Certificates] ❌ Cache invalidation failed:", err);
		}
...
router.delete("/certificates/:id", async (req, res) => {
	try {
		...
		try {
			...
			await CacheOperations.invalidateSustainability();
			logger.info(
				"[Certificates] ✅ Cache invalidated after certificate deletion",
			);
		} catch (err) {
			logger.error("[Certificates] ❌ Cache invalidation failed:", err);
		}
