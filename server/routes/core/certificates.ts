...
router.get('/certificates', async (_req, res) => {
  try {
    const certificates = await withTimeout(getStorage().getCertificates(), 10000, 'Get all certificates');
...
router.post('/certificates', async (req, res) => {
  try {
    ...
    const certificate = await withTimeout(getStorage().createCertificate(validatedData), 10000, 'Create certificate');
...
router.put('/certificates/:id', async (req, res) => {
  try {
    ...
    const certificate = await withTimeout(getStorage().updateCertificate(id, validatedData), 10000, 'Update certificate');
...
router.delete('/certificates/:id', async (req, res) => {
  try {
    ...
    const success = await withTimeout(getStorage().deleteCertificate(id), 10000, 'Delete certificate');
...
router.get('/sustainability-certificates', async (_req, res) => {
  try {
    const certificates = await withTimeout(getStorage().getCertificates(), 10000, 'Get sustainability certificates');
    const sustainabilityCertificates = certificates.filter(
      (cert) =>
        cert.showOnSustainabilityPage === true && cert.isActive !== false,
    );
