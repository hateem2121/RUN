...
export function registerTaxonomyRoutes(app: Express): void {
	...
	app.post("/api/categories", async (req, res) => {
		try {
			...
			try {
				...
				logger.info(
					"[Categories] ✅ Cache invalidated after category creation",
				);
			} catch (err) {
	...
	app.delete("/api/categories/:id", async (req, res) => {
		try {
			...
			try {
				...
				await CacheOperations.invalidateHomepage();
				logger.info(
					"[Categories] ✅ Cache invalidated after category deletion",
				);
			} catch (err) {
				logger.error("[Categories] ❌ Cache invalidation failed:", err);
			}
	...
	app.patch("/api/categories/reorder", async (req, res) => {
		try {
			...
			if (!Array.isArray(categoryOrders)) {
				return res
					.status(400)
					.json({ error: "categoryOrders must be an array" });
			}
	...
	app.post("/api/fibers", async (req, res) => {
		try {
			...
			return res.status(500).json({
				error:
					error instanceof Error ? error.message : "Failed to create fiber",
			});
		}
	});
	...
	app.patch("/api/fibers/:id", async (req, res) => {
		try {
			...
			return res.status(500).json({
				error:
					error instanceof Error ? error.message : "Failed to update fiber",
			});
		}
	});
	...
	app.delete("/api/fibers/:id", async (req, res) => {
		try {
			...
			return res.status(500).json({
				error:
					error instanceof Error ? error.message : "Failed to delete fiber",
			});
		}
	});
