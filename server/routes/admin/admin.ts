...
router.get('/admin/products/initial-data', async (_req, res) => {
  try {
    ...
    const validMediaAssets = mediaAssets.filter(
      (asset): asset is MediaAsset => typeof asset.filename === 'string' && asset.filename !== 'undefined',
    );
    ...
    const enhancedProducts = products.map((product: Product) => {
      ...
      return {
        ...
        canonicalUrl: (product.urlPath ? `/categories/${product.urlPath}` : `/products/${product.slug}`),
        primaryModelId: product.modelFileId || null,
      };
    });

    const relevantMediaAssets = validMediaAssets.filter((asset: MediaAsset) => referencedMediaIds.has(asset.id));
    const additionalMedia = validMediaAssets.filter((asset: MediaAsset) => !referencedMediaIds.has(asset.id)).slice(0, 50);
      ...
      if (category.featuredContent) {
        ...
        for (const cardKey of ['card1', 'card2', 'card3', 'card4'] as const) {
          ...
          if (card && 'mediaUrl' in card && card.mediaUrl) {
            ...
            if (mediaUrl.includes('undefined') || mediaUrl === '/api/media/undefined/content') {
              ...
              const cardToUpdate = updatedFeaturedContent[cardKey as keyof typeof updatedFeaturedContent];
        ...
        if (needsUpdate) {
          const updateResult = await withTimeout(
            getStorage().updateCategory(category.id, { featuredContent: updatedFeaturedContent }),
            5000,
...
router.get('/enterprise/audit-config', async (_req, res) => {
  try {
    return res.json({
      enabled: true,
      trackedTables: ['categories', 'products', 'mediaAssets', 'fabrics', 'fibers', 'certificates', 'sizeCharts', 'accessories', 'navigationItems', 'footerLinks'],
