...
const BentoCard: React.FC<BentoCardProps> = ({ title, subtitle, icon: Icon, colSpan = "col-span-1", withRipple = false, isMobile, setCursor, image }) => (
    <div 
    ...
    >
        ...
        <div className="w-full flex justify-end relative z-10">
            <Icon className={`w-12 h-12 stroke-[1] transition-colors duration-300 ${withRipple ? 'text-blue-400' : 'text-gray-400 group-hover:text-blue-400'}`} />
        </div>
        ...
        <div className="max-w-[1600px] mx-auto">
            ...
            <div className="w-full border-y border-black mt-24 py-6 overflow-hidden" aria-hidden="true">
                <div className="flex whitespace-nowrap animate-marquee">
                    {Array(10).fill("GOTS CERTIFIED • OEKO-TEX STANDARD 100 • FAIR TRADE • ISO 9001 • ").map((text, i) => (
                        <span key={i} className="text-xl font-mono mx-4">{text}</span>
                    ))}
                </div>
            </div>
        </div>
