...
const OptimizedClothMaterial = () => {
  ...
  const uniforms = useMemo(() => ({
    ...
    uColorStart: { value: new Color('#050505') }, // Deep Black
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      if (!isMobile) {
          ...
          // Gentle rotation
          mesh.current.rotation.x = MathUtils.lerp(mesh.current.rotation.x, Math.sin(t * 0.2) * 0.1 + targetRotX, 0.1);
          mesh.current.rotation.y = MathUtils.lerp(mesh.current.rotation.y, Math.sin(t * 0.3) * 0.1 + targetRotY, 0.1);
      } else {
  ...
  return (
    <section ref={containerRef} className="relative w-full h-screen bg-[#FAFAFA] overflow-hidden">
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
        <div ref={textContainerRef} className="flex flex-col items-center justify-center perspective-[1000px] px-4 text-center">
