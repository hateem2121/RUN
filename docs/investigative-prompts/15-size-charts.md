# 🔍 INVESTIGATE: Size Charts Page
**Route**: `/size-charts`
**Agent Host**: Antigravity 2.0 Desktop · Agent Teams Panel
**Crawl Model**: `@gemini-3.5-flash`
**Synthesis Model**: `@claude-opus-4-6`
**Environment**: `http://localhost:5002/size-charts`
**Scope**: Read-only · Do NOT modify source files
**Output**: `findings/size/`
**Issue ID Prefix**: `SIZE-`

---
## Goal
Investigate `/size-charts` for all visual, CMS, API, performance, SEO, accessibility, animation, and TypeScript issues. Produce a severity-scored findings report without modifying any source files.

---

## Context
**Source File**: `client/app/routes/size-charts.tsx`
**Description**: Measurement reference tables for apparel styles. React Router loader.

### CMS API Endpoints
| Endpoint | Purpose |
|----------|---------|
| `GET /api/resources/batch` | Batch endpoint — size charts data subset |

### Known Issues
- SSR cache mismatch: manifest expects `/resources/size-charts`
- Tables on mobile: must scroll horizontally or reflow to card layout — not clip
- Unit conversion (cm/inches): if toggle present, verify mathematical correctness

---
## Pre-flight: Protocol 0

Run once before beginning. Log all output to `findings/size/protocol-0.txt`.
...
- [ ] Admin modules accessible and CRUD functional:
  - [ ] `http://localhost:5002/admin/size-charts`
...
### SIZE-CHARTS-SPECIFIC: Table Responsiveness Audit
- [ ] Tables at 1440px: all columns visible, no overflow
- [ ] Tables at 375px: horizontal scroll with visible indicator OR card reflow
- [ ] cm ↔ inches toggle (if present): verify 1 inch = 2.54cm conversion is correct
- [ ] Spot-check 3 size values against standard industry sizing
...
- [ ] Admin module verified: `http://localhost:5002/admin/size-charts`
