            ...
            case 'specs':
                return (
                    <div className="animate-fade-in">
                        <SectionHeader icon={<SpecsIcon className="w-5 h-5 text-gray-500" />} title="Product Specs" />
                        <ul className="space-y-2 text-gray-600 pl-1">
                            {product.productSpecs.map((spec, i) => <li key={i} className="flex items-start"><span className="mr-2 mt-1">-</span>{spec}</li>)}
                        </ul>
                    </div>
                );
            case 'info':
                return (
                    <div className="animate-fade-in">
                        ...
                        <ul className="space-y-2 text-gray-600 pl-1">
                            <li className="flex items-start"><span className="mr-2 mt-1">-</span>MOQ: {product.minOrderQty > 0 ? `${product.minOrderQty} units` : 'N/A'}</li>
                            <li className="flex items-start"><span className="mr-2 mt-1">-</span>Lead Time: {product.leadTime}</li>
                            <li className="flex items-start"><span className="mr-2 mt-1">-</span>Fit: {product.customFit}</li>
                            <li className="flex items-start"><span className="mr-2 mt-1">-</span>Weight: {product.customWeight}</li>
                        </ul>
                    </div>
                );
            case 'certs':
                return (
                    <div className="animate-fade-in">
                        <SectionHeader icon={<ShieldCheckIcon className="w-5 h-5 text-gray-500" />} title="Certifications" />
