...
const ProductGallery = forwardRef<ProductGalleryHandle, ProductGalleryProps>(({ media, hotspots }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    switchTo3DView: () => {
      ...
      if (model3DIndex !== -1) {
        ...
        console.warn("3D model not found in media assets.");
      }
    },
  }));
      ...
      <div className="relative aspect-w-1 aspect-h-1 h-[65vh] sm:h-[75vh] lg:h-[85vh] subtle-noise-bg">
        ...
        {!isLoading && activeAnnotation && (
          <div className="absolute top-4 right-4 bg-white p-4 rounded-lg shadow-lg z-20 max-w-xs transition-all duration-300 ease-in-out transform animate-fade-in">
              ...
              <button onClick={() => setActiveAnnotation(null)} className="absolute top-1 right-1 text-gray-500 hover:text-gray-800">
        ...
        >
            {media.map((item, index) => (
                <button 
                    ...
                    className={`w-16 h-16 flex-shrink-0 transition-all duration-300 ease-in-out transform ${activeIndex === index ? 'ring-2 ring-black ring-offset-2' : 'opacity-60 hover:opacity-100 hover:scale-105'}`}
                >
