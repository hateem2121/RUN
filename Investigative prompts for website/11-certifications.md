# 🔍 INVESTIGATE: Certifications Page
**Route**: `/certifications`
**Agent Host**: Antigravity 2.0 Desktop · Agent Teams Panel
**Crawl Model**: `@gemini-3.5-flash`
**Synthesis Model**: `@claude-opus-4-6`
**Environment**: `http://localhost:5002/certifications`
**Scope**: Read-only · Do NOT modify source files
**Output**: `findings/cert/`
**Issue ID Prefix**: `CERT-`

---
## Goal
Investigate `/certifications` for all visual, CMS, API, performance, SEO, accessibility, animation, and TypeScript issues. Produce a severity-scored findings report without modifying any source files.

---

## Context
**Source File**: `client/app/routes/certifications.tsx`
**Description**: Quality standards, ISO certifications, and digital certificates. React Router loader.

### CMS API Endpoints
| Endpoint | Purpose |
|----------|---------|
| `GET /api/resources/batch` | Batch endpoint — certifications data subset |

### Known Issues
- SSR cache mismatch: manifest expects `/resources/certifications` — confirm visiting that URL gives 404
- Cert display: must NOT overstate direct certifications — verify supplier-level disclaimer present
- Certificate file links (PDF/image): verify they resolve to actual files
- Expired certifications: verify none are shown with past expiry dates

---
## Pre-flight: Protocol 0

Run once before beginning. Log all output to `findings/cert/protocol-0.txt`.
...
- [ ] Admin modules accessible and CRUD functional:
  - [ ] `http://localhost:5002/admin/certifications`
...
### CERT-SPECIFIC: Compliance Language Audit
- [ ] Every certification displayed: is there a visible disclaimer (supplier-level, partner-supported)?
- [ ] Cert document links: `curl -I [cert-pdf-url]` → 200 (not 404)
- [ ] Verify `/resources/certifications` returns HTTP 404: `curl -sw "\nHTTP: %{http_code}\n" http://localhost:5002/resources/certifications`
...
- [ ] Admin module verified: `http://localhost:5002/admin/certifications`
