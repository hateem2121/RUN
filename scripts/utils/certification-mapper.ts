...
export class CertificationMapper {
  ...
  async initialize(): Promise<void> {
    ...
    console.log('🏆 Loading existing certifications for mapping...');
    ...
    console.log(`✅ Mapped ${certificates.length} certifications with ${this.certificationMap.size} total name variations`);
  ...
  private isPartialMatch(searchName: string, mappedName: string): boolean {
    ...
    const matchingWords = searchWords.filter(word => 
      mappedWords.some(mappedWord => 
        mappedWord.includes(word) || word.includes(mappedWord)
      )
    );
  ...
  async createMissingCertifications(missingCertifications: string[]): Promise<void> {
    console.log(`🏆 Creating ${missingCertifications.length} missing certifications...`);
    ...
    for (const certName of uniqueCertifications) {
      ...
      try {
        ...
        console.log(`✅ Created certification: ${createdCert.name} (ID: ${createdCert.id})`);
      } catch (error) {
        console.error(`❌ Failed to create certification: ${certName}`, error);
      }
    }
  }
