...
export class CertificationMapper {
	...
	async mapCertifications(
		certificationNames: string[],
	): Promise<CertificationMappingResult> {
		if (!this.initialized) {
	...
	private isPartialMatch(searchName: string, mappedName: string): boolean {
		...
		const matchingWords = searchWords.filter((word) =>
			mappedWords.some(
				(mappedWord) => mappedWord.includes(word) || word.includes(mappedWord),
			),
		);
	...
	async createMissingCertifications(
		missingCertifications: string[],
	): Promise<void> {
	...
	private createCertificationData(certificationName: string) {
		...
		const certificationData: { [key: string]: any } = {
			"oeko-tex standard 100": {
				...
				description:
					"International safety standard for textiles tested for harmful substances",
				issuingBody: "International OEKO-TEX Association",
			},
			gots: {
				type: "Organic Certification",
				description:
					"Global Organic Textile Standard for organic fiber processing",
			...
			bci: {
				type: "Sustainability",
				description:
					"Better Cotton Initiative for sustainable cotton production",
			...
			ocs: {
				...
				description:
					"Organic Content Standard for organic raw material content",
				issuingBody: "Textile Exchange",
			},
			"organic content standard": {
				type: "Organic Content",
		...
		if (normalizedName.includes("organic")) {
			...
			normalizedName.includes("oeko")
		) {
			certProps.type = "Chemical Safety";
		} else if (
			normalizedName.includes("social") ||
			normalizedName.includes("responsible")
		) {
			certProps.type = "Social Compliance";
		} else if (
			normalizedName.includes("sustainable") ||
