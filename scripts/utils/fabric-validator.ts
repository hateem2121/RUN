...
export class FabricValidator {
  ...
  validateFabric(fabric: any): ValidationResult {
    ...
    if (fabric.airPermeability) {
      ...
      if (!airValid) {
        errors.push(`Invalid air permeability: ${fabric.airPermeability} (expected: 0-1000 L/m²/s)`);
      }
    }
    ...
    if (fabric.enhancedMoistureManagement) {
      ...
      if (!moistureValid) {
        errors.push(`Invalid moisture management rating: ${fabric.enhancedMoistureManagement} (expected: 1-5/5)`);
      }
    }
    ...
    for (const composition of compositions) {
      ...
      for (const fiber of composition.fibers) {
        ...
        if (isNaN(percentage) || percentage < 0 || percentage > 100) {
          errors.push(`Invalid fiber percentage in composition "${composition.name}": ${fiber.percentage}`);
        } else {
      ...
      if (totalPercentage !== 100) {
        errors.push(`Composition "${composition.name}" percentages don't add up to 100% (total: ${totalPercentage}%)`);
      }
    }
