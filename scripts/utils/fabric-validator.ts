...
export class FabricValidator {
	validateFabric(fabric: any): ValidationResult {
		...
		if (fabric.weight) {
			...
			if (!weightValid) {
				errors.push(
					`Invalid weight range: ${fabric.weight} (expected: 50-500 GSM)`,
				);
			}
		}
		...
		if (fabric.sustainabilityScore) {
			const scoreValid = this.validateSustainabilityScore(
				fabric.sustainabilityScore,
			);
			if (!scoreValid) {
				errors.push(
					`Invalid sustainability score: ${fabric.sustainabilityScore} (expected: 1-5)`,
				);
			}
		}
		...
		if (fabric.stretchPercentage) {
			const stretchValid = this.validateStretchPercentage(
				fabric.stretchPercentage,
			);
			if (!stretchValid) {
				errors.push(
					`Invalid stretch percentage: ${fabric.stretchPercentage} (expected: 0-100%)`,
				);
			}
		}
		...
		if (fabric.waterColumn) {
			...
			if (!waterValid) {
				errors.push(
					`Invalid water column: ${fabric.waterColumn} (expected: 0-30000 mm)`,
				);
			}
		}
		...
		if (fabric.enhancedMoistureManagement) {
			const moistureValid = this.validateMoistureManagement(
				fabric.enhancedMoistureManagement,
			);
			if (!moistureValid) {
				errors.push(
		...
		if (fabric.wickingRate) {
			...
			if (!wickingValid) {
				errors.push(
					`Invalid wicking rate: ${fabric.wickingRate} (expected: 0-500 mm/hr)`,
				);
			}
		}
		...
		if (fabric.dryingTime) {
			...
			if (!dryingValid) {
				errors.push(
					`Invalid drying time: ${fabric.dryingTime} (expected: 0-300 minutes)`,
				);
			}
		}
		...
		if (fabric.abrasionResistance) {
			const abrasionValid = this.validateAbrasionResistance(
				fabric.abrasionResistance,
			);
			if (!abrasionValid) {
				errors.push(
					`Invalid abrasion resistance: ${fabric.abrasionResistance} (expected: 1-5/5)`,
				);
			}
		}
		...
		if (fabric.pillingGrade) {
			...
			if (!pillingValid) {
				errors.push(
					`Invalid pilling grade: ${fabric.pillingGrade} (expected: 1-5)`,
				);
			}
		}
		...
		if (fabric.shrinkageTolerance) {
			const shrinkageValid = this.validateShrinkageTolerance(
				fabric.shrinkageTolerance,
			);
			if (!shrinkageValid) {
				errors.push(
					`Invalid shrinkage tolerance: ${fabric.shrinkageTolerance} (expected: 0-20%)`,
				);
			}
		}
		...
		if (fabric.washTemperature) {
			...
			if (!washValid) {
				errors.push(
					`Invalid wash temperature: ${fabric.washTemperature} (expected: 0-100°C)`,
				);
			}
		}
	...
	private validateMoistureManagement(moisture: string): boolean {
		...
		const maxMoisture = moistureMatch[2]
			? parseInt(moistureMatch[2])
			: minMoisture;
	...
	private validateAbrasionResistance(abrasion: string): boolean {
		...
		const maxAbrasion = abrasionMatch[2]
			? parseInt(abrasionMatch[2])
			: minAbrasion;
	...
	private validateShrinkageTolerance(shrinkage: string): boolean {
		...
		const maxShrinkage = shrinkageMatch[2]
			? parseInt(shrinkageMatch[2])
			: minShrinkage;

		return (
			minShrinkage >= 0 && maxShrinkage <= 20 && minShrinkage <= maxShrinkage
		);
	}
