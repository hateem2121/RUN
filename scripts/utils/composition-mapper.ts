		...
		for (const composition of compositions) {
			...
			for (const fiber of composition.fibers) {
				...
				if (fiberId) {
					...
					result.errors.push(
						`Fiber not found: ${fiber.fiberName} in composition "${composition.name}"`,
					);
	...
	private createFiberData(fiberName: string) {
		...
		const fiberProperties: { [key: string]: any } = {
			polyester: {
				...
				description:
					"Durable synthetic fiber with excellent moisture-wicking properties",
				sustainabilityScore: 2,
				environmentalImpactNotes:
					"Petroleum-based, long-lasting but not biodegradable",
				properties: {
					strength: "High",
			...
			"recycled polyester": {
				...
				description:
					"Recycled polyester from plastic bottles, offering sustainability benefits",
				sustainabilityScore: 4,
				environmentalImpactNotes:
					"Made from recycled plastic bottles, reduces waste",
				properties: {
					strength: "High",
					moisture: "Wicks moisture away from skin",
					durability: "Excellent",
			...
			cotton: {
				...
				environmentalImpactNotes:
					"Natural and biodegradable, but water-intensive cultivation",
				properties: {
					strength: "Moderate",
			...
			spandex: {
				...
				environmentalImpactNotes:
					"Synthetic and non-biodegradable, difficult to recycle",
				properties: {
					strength: "Moderate",
			...
			nylon: {
				...
				sustainabilityScore: 2,
				environmentalImpactNotes:
					"Petroleum-based, durable but not biodegradable",
				properties: {
					strength: "Very high",
		...
		let fiberProps =
			fiberProperties[normalizedName] || fiberProperties["polyester"]; // Default fallback
