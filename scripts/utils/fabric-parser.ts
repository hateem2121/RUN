...
export class FabricParser {
  ...
  private parseSection(section: string): ParsedFabric | null {
    try {
      ...
      const stretchPercentage = this.extractField(section, 'Stretch Percentage \\(%\\):', '([^\\n]+)');
      const airPermeability = this.extractField(section, 'Air Permeability:', '([^\\n]+)');
      const waterColumn = this.extractField(section, 'Water Column:', '([^\\n]+)');
      const enhancedMoistureManagement = this.extractField(section, 'Enhanced Moisture Management Rating:', '([^\\n]+)');
      const wickingRate = this.extractField(section, 'Wicking Rate \\(mm/hr\\):', '([^\\n]+)');
      const dryingTime = this.extractField(section, 'Drying Time \\(min\\):', '([^\\n]+)');
      const performanceFeatures = this.extractMultilineList(section, 'Performance Features:', 'Yarn Count/Construction:');
      const yarnCountConstruction = this.extractField(section, 'Yarn Count/Construction:', '([^\\n]+)');
      ...
      const shrinkageTolerance = this.extractField(section, 'Shrinkage Tolerance \\(%\\):', '([^\\n]+)');
      ...
      const careInstructions = this.extractField(section, 'Care Instructions & Restrictions:', '([^\\n]+)');
      const detailedInstructions = this.extractField(section, 'Detailed Instructions:', '([^\\n]+)');
      ...
      console.error(`Error parsing fabric section: ${error}`);
      return null;
    }
  }
  ...
  private parseCompositions(section: string): Array<{
    ...
    const primarySection = this.extractCompositionSection(section, 'Fiber Compositions:', 'Multiple Alternative Compositions:');
    const primaryCompositions = this.parseCompositionLines(primarySection);

    // Extract alternative compositions
    const alternativeSection = this.extractCompositionSection(section, 'Multiple Alternative Compositions:', 'Fabric Type:');
  ...
  private extractCompositionSection(section: string, startMarker: string, endMarker: string): string {
    const startIndex = section.indexOf(startMarker);
    const endIndex = section.indexOf(endMarker);
  ...
  private parseCompositionLines(compositionSection: string): string[] {
    return compositionSection.split('\n')
      .map(line => line.trim())
      .filter(line => line.length > 0 && line.includes('%'))
      .map(line => line.replace(/^[-•*]\s*/, '').trim());
  }
