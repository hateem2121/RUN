...
export class FabricParser {
	...
	private parseSection(section: string): ParsedFabric | null {
		try {
			...
			// Parse each field
			const weight = this.extractField(
				section,
				"Weight \\(GSM\\):",
				"([0-9]+-[0-9]+\\s+GSM)",
			);
			...
			const fabricType = this.extractField(
				section,
				"Fabric Type:",
				"([^\\n]+)",
			);
			const keyApplications = this.extractListField(
				section,
				"Key Applications:",
				"([^\\n]+)",
			);
			...
			const airPermeability = this.extractField(
				section,
				"Air Permeability:",
				"([^\\n]+)",
			);
			const waterColumn = this.extractField(
				section,
				"Water Column:",
				"([^\\n]+)",
			);
			...
			const wickingRate = this.extractField(
				section,
				"Wicking Rate \\(mm/hr\\):",
				"([^\\n]+)",
			);
			const dryingTime = this.extractField(
				section,
				"Drying Time \\(min\\):",
				"([^\\n]+)",
			);
			...
			const abrasionResistance = this.extractField(
				section,
				"Abrasion Resistance:",
				"([^\\n]+)",
			);
			const pillingGrade = this.extractField(
				section,
			...
			const washTemperature = this.extractField(
				section,
				"Wash Temperature \\(°C\\):",
				"([^\\n]+)",
			);
			const sustainabilityScore = this.extractSustainabilityScore(section);
			const certificationTags = this.extractListField(
				section,
				"Certification Tags:",
				"([^\\n]+)",
			);
			const endOfLifeOptions = this.extractListField(
				section,
				"End-of-Life Options:",
				"([^\\n]+)",
			);
	...
	private extractMultilineList(
		...
		const listSection = section.substring(
	...
	private extractSustainabilityScore(section: string): string {
		const scoreMatch = section.match(
			/Sustainability Score:\s*⭐*\s*\((\d+)\/5\s*stars?\)/i,
		);
	...
	private parseCompositions(section: string): Array<{
		...
		const alternativeCompositions =
			this.parseCompositionLines(alternativeSection);
	...
	private parseCompositionString(composition: string): Array<{
		...
		for (const part of parts) {
			...
			if (match) {
				...
				fiberName = fiberName.replace(
					/\s*(Organic|Conventional|Recycled|rPET)\s*/g,
					"",
				); // Remove modifiers
