...
import {
	TECHNOLOGY_DEFAULTS,
	TECHNOLOGY_THEME,
} from "../client/src/lib/technology-constants.js";
...
assert(
	hexRegex.test(TECHNOLOGY_THEME.colors.gradientStart),
	"Gradient Start color is valid hex",
);
assert(
	hexRegex.test(TECHNOLOGY_THEME.colors.gradientEnd),
	"Gradient End color is valid hex",
);
...
try {
	...
	if (hexMatches) {
		...
		const suspiciousHex = uniqueMatches.filter(
			(hex) =>
				hex.toUpperCase() !== "#1E40AF" && hex.toUpperCase() !== "#3B82F6", // Allow Tailwind defaults if they persisted in comments
		);
