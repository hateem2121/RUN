...
function findFiberId(allFibers: any[], name: string): number | null {
  ...
  if (lowerName.includes("polyester") || lowerName.includes("poly")) {
    if (lowerName.includes("recycled") || lowerName.includes("rpet") || lowerName.includes("eco")) {
      return allFibers.find((f) => f.name === "Recycled Polyester (rPET)")?.id || null;
    }
    if (lowerName.includes("graphene")) {
      return allFibers.find((f) => f.name === "Graphene-Infused Polyester")?.id || null;
    }
    return allFibers.find((f) => f.name === "Virgin Polyester (Standard Performance)")?.id || null;
  }

  if (lowerName.includes("cotton")) {
    if (lowerName.includes("organic")) {
      return allFibers.find((f) => f.name === "Organic Cotton (GOTS)")?.id || null;
    }
    return allFibers.find((f) => f.name === "Conventional Cotton (Carded/Combed)")?.id || null;
  }
  ...
  if (
    ...
    if (lowerName.includes("eco") || lowerName.includes("recycled")) {
      return allFibers.find((f) => f.name === "Eco-Smart Elastane (Recycled Content)")?.id || null;
    }
    return allFibers.find((f) => f.name === "Standard Elastane (Spandex/Lycra)")?.id || null;
  }

  if (lowerName.includes("nylon") || lowerName.includes("polyamide")) {
    if (lowerName.includes("recycled") || lowerName.includes("bio")) {
      return allFibers.find((f) => f.name === "Recycled Nylon (pre-consumer)")?.id || null;
    }
    return allFibers.find((f) => f.name === "Virgin Nylon 6 (Polyamide)")?.id || null;
  }

  if (lowerName.includes("fleece")) {
    if (lowerName.includes("recycled")) {
      return allFibers.find((f) => f.name === "Recycled Performance Fleece")?.id || null;
    }
    return allFibers.find((f) => f.name === "Standard Micro-Fleece")?.id || null;
  }

  if (lowerName.includes("neoprene") || lowerName.includes("rubber")) {
    if (lowerName.includes("limestone") || lowerName.includes("bio")) {
      return allFibers.find((f) => f.name === "Eco-Limestone Neoprene")?.id || null;
    }
    return allFibers.find((f) => f.name === "Standard Neoprene (Petroleum)")?.id || null;
  }
  ...
  if (
    ...
    return allFibers.find((f) => f.name === "MicroModal (Beechwood)")?.id || null;
  }

  if (lowerName.includes("polypropylene") || lowerName.includes("olefin")) {
    return allFibers.find((f) => f.name === "Polypropylene (Olefin)")?.id || null;
  }
...
async function seedFabrics() {
  try {
    ...
    console.log(`📚 Loaded ${allFibers.length} fibers for mapping.`);
    ...
    console.log(`✅ Successfully added ${inserted.length} fabrics:`);
    inserted.forEach((f) => {
