...
async function triggerFabricsCacheClear() {
	try {
		...
		if (!createResponse.ok) {
			throw new Error(
				`Failed to create dummy fabric: ${createResponse.statusText}`,
			);
		}
		...
		const deleteResponse = await fetch(
			`http://localhost:5001/api/fabrics/${fabricId}`,
			{
				method: "DELETE",
			},
		);
		...
		if (!deleteResponse.ok) {
			throw new Error(
				`Failed to delete dummy fabric: ${deleteResponse.statusText}`,
			);
		}
		process.exit(0);
	} catch (error) {
