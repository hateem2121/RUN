...
async function comprehensiveCMSSeeding() {
	try {
		...
		const insertedHomepageHero = await db
			.insert(homepageHero)
			.values(homepageHeroData)
			.returning();
		...
		const insertedAboutHero = await db
			.insert(aboutHero)
			.values(aboutHeroData)
			.returning();
		...
		const aboutLocationData = [
			...
			},
			{
				...
				longitude: "-74.0060",
				description:
					"Sales and customer service office for North American clients.",
		...
		const aboutStatisticsData = [
			...
			},
			{
				...
				unit: "Brands",
				description:
					"Trusted manufacturing partner for leading athletic brands worldwide",
				iconName: "global",
				isActive: true,
				sortOrder: 2,
			},
			{
				...
				unit: "Units",
				description:
					"Large-scale production capabilities with consistent quality",
				iconName: "factory",
				isActive: true,
				sortOrder: 3,
			},
			{
				...
				unit: "Countries",
				description:
					"Global reach with reliable international shipping networks",
				iconName: "shipping",
				isActive: true,
				sortOrder: 4,
			},
