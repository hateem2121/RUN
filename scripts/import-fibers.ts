...
const fiberData = [
  ...
  },
  {
    ...
    properties: {
      "Performance": "Identical performance characteristics to virgin polyester including moisture management",
      "Energy efficiency": "Reduces energy consumption by 30-50% and carbon emissions by 60% compared to virgin polyester",
      "Durability": "Maintains excellent durability and color retention properties",
      "Waste reduction": "Diverts plastic waste from landfills and oceans into functional sportswear",
      "Manufacturing compatibility": "Compatible with existing textile manufacturing processes and equipment",
      "Quick-drying": "Offers the same quick-drying and wrinkle-resistant benefits as traditional polyester"
    },
    sustainabilityScore: 4,
    environmentalImpactNotes: "Recycled polyester significantly reduces environmental impact with 30-50% lower energy consumption and 60% fewer carbon emissions compared to virgin polyester production. This circular approach diverts millions of plastic bottles from landfills and oceans annually, though the recycling process still requires energy and chemical processing. The main challenge lies in microplastic shedding during washing, but ongoing research focuses on developing filtration systems and fiber modifications to minimize this impact.",
    isActive: true
  },
  {
    name: "Nylon",
  ...
  },
  {
    ...
    properties: {
      ...
      "Recovery": "Superior recovery properties that return fabric to original shape after stretching",
      "Versatility": "Versatile application across multiple sportswear categories from compression wear to swimwear",
      "Lightweight": "Lightweight construction that doesn't add bulk to garments",
      "Chemical resistance": "Chemical resistance that maintains performance through multiple wash cycles",
      "Texture": "Smooth texture that reduces friction and prevents skin irritation"
    },
    sustainabilityScore: 1,
    environmentalImpactNotes: "Spandex production relies heavily on petroleum-based chemicals and energy-intensive manufacturing processes, resulting in significant carbon emissions and environmental pollution. The fiber's synthetic nature means it doesn't biodegrade naturally, contributing to long-term textile waste accumulation. Additionally, spandex garments are difficult to recycle due to their complex chemical composition and are often blended with other fibers, making separation and processing challenging for circular economy initiatives.",
    isActive: true
  },
  {
    name: "Cotton",
    ...
    properties: {
      ...
      "Moisture absorption": "Absorbs moisture but retains it within the fabric rather than wicking it away",
      "Comfort": "Soft, comfortable texture that feels natural against the skin",
      "Hypoallergenic": "Hypoallergenic properties that reduce risk of skin irritation and allergic reactions",
  ...
  },
  {
    ...
    properties: {
      "Temperature regulation": "Natural temperature regulation that keeps body comfortable in various climates",
      "Moisture management": "Enhanced moisture management when blended with synthetic fibers for improved drying time",
  ...
  },
  {
    ...
    properties: {
      ...
      "Moisture-wicking": "Superior moisture-wicking capabilities that keep skin dry during intense activities",
      ...
      "Biodegradability": "Biodegradable composition that breaks down naturally without environmental harm"
    },
    sustainabilityScore: 4,
    environmentalImpactNotes: "Bamboo is a plant that doesn't require many pesticides and grows rapidly without depleting soil nutrients, making it an environmentally superior raw material choice. Bamboo cultivation actually improves soil health and sequesters more carbon than traditional crops, contributing to climate change mitigation. However, the chemical processing required to transform bamboo into textile fiber can involve harsh chemicals, though closed-loop production systems are increasingly being adopted to minimize environmental impact and chemical waste.",
    isActive: true
  },
  {
    name: "TENCEL™ Lyocell",
  ...
  },
  {
    ...
    properties: {
      ...
      "Color retention": "Exceptional color retention that maintains vibrancy after multiple wash cycles",
  ...
  },
  {
    ...
    properties: {
      "Flexibility": "Excellent flexibility and moisture-wicking capabilities suitable for athletic applications",
  ...
  },
  {
    ...
    properties: {
      ...
      "Moisture-wicking": "Excellent moisture-wicking capabilities enhanced through modern processing techniques",
...
async function importFibers() {
  try {
    ...
    for (const fiber of fiberData) {
      ...
      const existingFiber = existingFibers.find(f => f.name.toLowerCase() === fiber.name.toLowerCase());
      
      if (existingFiber) {
        console.log(`Skipping ${fiber.name} - already exists`);
        skippedCount++;
        continue;
      }
      ...
      console.log(`Created fiber: ${newFiber.name} (ID: ${newFiber.id})`);
      importedCount++;
    }
    
    console.log(`\nImport complete!`);
    console.log(`- Imported: ${importedCount} fibers`);
    console.log(`- Skipped: ${skippedCount} fibers (already existed)`);
    console.log(`- Total fibers in database: ${existingFibers.length + importedCount}`);
    
  } catch (error) {
    console.error('Error importing fibers:', error);
  }
}
