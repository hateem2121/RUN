...
const fibersData = [
  {
    ...
    description: "The industrial standard for athletic apparel. Engineered from purified terephthalic acid (PTA), this fiber offers the legendary durability and wrinkle resistance that defined the modern sportswear era. It provides a sharper price point than rPET while maintaining identical moisture management and tensile strength profiles. Ideally suited for high-volume promotional runs or budget-tier team kits.",
    environmentalImpact: "High energy consumption in production and non-biodegradable. However, it remains highly durable, preventing early garment disposal.",
    properties: {
  ...
  },
  {
    ...
    environmentalImpact: "A natural, biodegradable fiber, but conventionally grown with standard water usage and crop protection methods.",
    properties: {
      "Breathability": "High",
  ...
  },
  {
    ...
    type: "Natural (Cellulose)",
    sustainabilityScore: 5,
    description: "The soul of our premium \"Casual Wear\". Grown without toxic pesticides or synthetic fertilizers, this fiber protects the farmer and the soil. It offers a noticeably softer \"hand-feel\" than conventional cotton, appealing to partners seeking \"Authentic brand storytelling\".",
    environmentalImpact: "Strictly GOTS Certified. Rain-fed cultivation reduces water consumption significantly and eliminates chemical runoff.",
    properties: {
  ...
  },
  {
    ...
    type: "Synthetic (Petroleum-based)",
    sustainabilityScore: 2,
    description: "The essential ingredient for \"Active Wear\". This is the industry-standard power-stretch fiber. It provides the necessary \"snap-back\" recovery for leggings and compression gear. Without it, performance wear would simply be a plastic bag. We use high-modulus variants to ensure garments don't bag out after 10 washes.",
    environmentalImpact: "Standard production requires significant energy. It is difficult to recycle once blended, necessitating responsible end-of-life planning.",
    properties: {
      "Elongation": "500-600%",
      "Recovery": "98% (Memory)",
      "Compression": "High"
  ...
  },
  {
    ...
    sustainabilityScore: 1,
    description: "Known as the \"Tough Guy\" of synthetics. Used in our \"Outer Wear\" and \"Sports Accessories\" (like bags or reinforced panels). Virgin Nylon offers superior abrasion resistance compared to polyester. It has a cooler, silkier touch, making it a favorite for seamless yoga wear, though at a higher carbon cost than poly.",
    environmentalImpact: "High energy intensity in production. Not biodegradable.",
    properties: {
  ...
  },
  {
    ...
    environmentalImpact: "Cleans up ocean waste and industrial landfills. Reduces the global warming impact of nylon by up to 90%.",
    properties: {
  ...
  },
  {
    ...
    sustainabilityScore: 1,
    description: "The cost-effective solution for warmth. A brushed polyester knit that traps air for insulation. It is lightweight, hydrophobic (doesn't hold water like wool), and dries rapidly. Ideal for sideline bench jackets and mass-market winter training gear where budget is the primary driver.",
    environmentalImpact: "Sheds microfibers during washing; we recommend washing bags. Sourced from standard petrochemicals.",
    properties: {
      "Insulation": "High",
      "Weight": "Lightweight",
  ...
  },
  {
    ...
    sustainabilityScore: 5,
    description: "Warmth without waste. This fleece takes the rPET chip and engineers it into a high-loft thermal fabric. By using longer staple fibers in the yarn spinning process, we significantly reduce shedding (micro-plastic release) compared to standard fleece, while delivering the same thermal efficiency for our \"Outer Wear\".",
    environmentalImpact: "100% Recycled Content. Supports the circular economy by creating demand for waste plastic.",
    properties: {
      "Insulation": "High",
  ...
  },
  {
    ...
    sustainabilityScore: 1,
    description: "The historic standard for wetsuits. Derived from oil, this rubber offers the classic durability and heavy-duty insulation required for cold-water diving. It is the accessible choice for dive schools and rental fleets needing rugged, affordable gear.",
    environmentalImpact: "High carbon footprint extraction and processing. Non-biodegradable.",
    properties: {
      "Insulation": "Good",
  ...
  },
  {
    ...
    description: "The \"Premium\" upgrade for our Wetsuit Edition. We replace the oil base with limestone (calcium carbonate). This creates a cell structure that is 98% water impermeable (warmer) and significantly lighter/stretchier than petroleum neoprene. It is the choice for pro-surfers and eco-conscious dive brands.",
    environmentalImpact: "Lower carbon footprint than oil. Limestone reserves are vast (est. 3 billion years supply), though it is still an extractive resource.",
    properties: {
  ...
  },
  {
    ...
    environmentalImpact: "Low energy impact during production and fully recyclable (Grade 5 plastic), though rarely recycled in municipal systems.",
    properties: {
  ...
  },
  {
    ...
    sustainabilityScore: 3,
    description: "The frontier of material science. We coat or infuse standard polyester yarns with Graphene—a material consisting of a single layer of carbon atoms. This transforms a standard fabric into a \"Smart Fabric\" that naturally conducts heat (cooling the body in summer) and retains far-infrared radiation (warming the body in winter). It is also bacteriostatic and anti-static.",
    environmentalImpact: "Extends the functionality of the garment, potentially reducing the need for multiple layers (\"buy less, buy better\"), though the graphene production process is energy-intensive.",
    properties: {
...
async function seedFibers() {
  try {
    ...
    inserted.forEach(fiber => {
      console.log(`- ${fiber.name} (ID: ${fiber.id})`);
    });

    console.log('\n✨ Done!');
    process.exit(0);
  } catch (error) {
    console.error('❌ Error seeding fibers:', error);
    process.exit(1);
  }
}
