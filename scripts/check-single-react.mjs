...
console.log("--- React Singularity Check ---");
...
try {
  ...
  console.log("Detected React versions:", [...versions]);

  if (versions.size > 1) {
    console.error("[FAIL] Multiple React versions detected! This causes Dual Context issues.");
    process.exit(1);
  }

  if (versions.size === 0) {
    console.warn("[WARN] No React version detected?");
  } else {
    console.log("[PASS] Single React version confirmed.");
  }
} catch (error) {
