...
const fabricData = [
  ...
  {
    name: "Pique Knit",
    description: "Pique Knit features a distinctive raised geometric pattern created through specialized knitting techniques that form a textured surface with enhanced breathability and visual appeal. This classic construction creates natural air channels that promote ventilation while maintaining the fabric's structural integrity, making it the preferred choice for traditional polo shirts and sophisticated athletic wear.",
  ...
  },
  {
    ...
    sustainabilityScore: "4",
    certificationTags: ["Better Cotton Initiative (BCI)", "Recycled Claim Standard (RCS)", "OEKO-TEX Standard 100"],
    isActive: true
  },
  {
    name: "Fleece",
    description: "Fleece represents advanced thermal insulation technology through specialized brushing and napping processes that create a plush, warm surface while maintaining lightweight properties. This high-performance fabric provides exceptional warmth-to-weight ratio and superior moisture management, making it essential for cold-weather athletic activities and outdoor sports applications.",
    ...
    sustainabilityScore: "4",
    certificationTags: ["Recycled Claim Standard (RCS)", "Bluesign", "OEKO-TEX Standard 100"],
    isActive: true
  }
];
...
async function populateFabricData() {
  ...
  console.log("=== PHASE 3: FABRIC DATA POPULATION ===\n");
  ...
  console.log(`Current fabrics counter: ${currentCounter}`);
  
  let newId = currentCounter + 1;
  
  for (const fabric of fabricData) {
    try {
      console.log(`\n🏭 Creating fabric ${newId}: ${fabric.name}`);
      ...
      console.log(`  ✅ Created: ${fabric.name} (ID: ${newId})`);
      console.log(`  📊 Weight: ${fabric.weight}g/m², Type: ${fabric.fabricType}`);
      console.log(`  🌱 Sustainability: ${fabric.sustainabilityScore}/5`);
      console.log(`  🏷️ Applications: ${fabric.keyApplications.slice(0, 3).join(', ')}`);
      
      newId++;
    } catch (error) {
      console.error(`❌ Error creating fabric ${fabric.name}:`, error);
    }
  }
  
  console.log(`\n=== FABRIC DATA POPULATION COMPLETE ===`);
  console.log(`Created ${fabricData.length} comprehensive fabric records`);
  console.log(`Updated fabrics counter: ${currentCounter} → ${newId - 1}`);
  console.log(`Your comprehensive sportswear fabric data is now available!`);
}
