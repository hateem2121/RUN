...
async function populateSampleData() {
  ...
  // Add size charts
  console.log('Adding size charts...');
  for (const chart of sizeCharts) {
    try {
      ...
      console.log(`✓ Added size chart: ${chart.name}`);
    } catch (error) {
      console.error(`✗ Failed to add size chart ${chart.name}:`, error.message);
    }
  }
  ...
  for (const accessory of accessories) {
    try {
      ...
      console.error(`✗ Failed to add accessory ${accessory.name}:`, error.message);
    }
  }
