...
console.log("🛡️  Scanning for forbidden router artifacts...");
const rootDir = path.resolve(__dirname, "../client/src");
const errors = scanDirectory(rootDir);

if (errors.length > 0) {
  console.error("❌ Router Guardrail Failed:");
  errors.forEach((err) => console.error(err));
  process.exit(1);
} else {
  console.log("✅ Router Guardrail Passed: No dual-router artifacts found.");
  process.exit(0);
}
