...
async function runHealthCheck(): Promise<MemoryHealthReport> {
  console.log("🔍 Starting Node.js Memory & Performance Health Check...\n");
...
async function generateReport(): Promise<void> {
  try {
    ...
    console.log("═══════════════════════════════════════════════════════════");
    console.log("        NODE.JS MEMORY & PERFORMANCE HEALTH CHECK          ");
    console.log(
      "═══════════════════════════════════════════════════════════\n",
    );
    ...
    console.log(
      `${scoreEmoji} Overall Health Score: ${report.healthScore}/100\n`,
    );
    ...
    console.log(
      `   Heap Utilization: ${((report.memory.heapUsed / report.memory.heapTotal) * 100).toFixed(1)}%`,
    );
    console.log(`   External:         ${formatBytes(report.memory.external)}`);
    console.log(`   RSS (Total):      ${formatBytes(report.memory.rss)}`);
    console.log(
      `   Array Buffers:    ${formatBytes(report.memory.arrayBuffers)}\n`,
    );
      ...
      report.eventLoop.lag < 10
        ...
        : report.eventLoop.lag < 20
          ...
          : report.eventLoop.lag < 50
            ...
            : "❌ POOR";
    console.log(
      `   Lag:              ${report.eventLoop.lag.toFixed(2)}ms ${lagStatus}`,
    );
    console.log(
      `   Lag Percent:      ${report.eventLoop.lagPercent.toFixed(1)}% (target: <100%)`,
    );
    console.log(`   Active Handles:   ${report.eventLoop.activeHandles}`);
    console.log(`   Active Requests:  ${report.eventLoop.activeRequests}\n`);

    // Data Structures Section
    console.log("📊 Major Data Structures:");
    console.log("─────────────────────────────────────────────────────────");
    ...
    if (highRisk.length > 0) {
      ...
      highRisk.forEach((s) => {
        ...
        console.log(
          `        Size: ${typeof s.size === "number" ? s.size.toLocaleString() : s.size}`,
        );
        console.log(`        Memory: ${s.estimatedMemory}`);
        console.log(
          `        Bounded: ${s.bounded ? "Yes" : "NO - LEAK RISK!"}`,
        );
        console.log(`        Cleanup: ${s.cleanupStrategy || "NONE"}\n`);
      });
    }

    if (mediumRisk.length > 0) {
      console.log("   ⚠️  MEDIUM RISK STRUCTURES:");
      mediumRisk.forEach((s) => {
        ...
        console.log(`        Memory: ${s.estimatedMemory}`);
        console.log(`        Bounded: ${s.bounded ? "Yes" : "NO"}`);
        console.log(`        Cleanup: ${s.cleanupStrategy || "NONE"}\n`);
      });
    }

    console.log(`   ✅ LOW RISK STRUCTURES (${lowRisk.length} total):`);
    lowRisk.slice(0, 3).forEach((s) => {
      console.log(
        `      • ${s.name}: ${s.estimatedMemory} (bounded: ${s.bounded})`,
      );
    });
    if (lowRisk.length > 3) {
    ...
    if (report.leakRisks.length > 0) {
      ...
      console.log("✅ No Memory Leak Risks Detected\n");
    }
    ...
    console.log("─────────────────────────────────────────────────────────");
    console.log("   GC is handled automatically by V8");
    console.log("   To enable GC metrics: node --expose-gc app.js");
    console.log("   To profile heap: node --heap-prof app.js");
    console.log("   To debug memory: node --inspect app.js\n");

    // Summary
    console.log("═══════════════════════════════════════════════════════════");
    console.log(`Report generated: ${report.timestamp}`);
    console.log("For detailed analysis, see: NODEJS_MEMORY_HEALTH_REPORT.md");
    console.log(
      "═══════════════════════════════════════════════════════════\n",
    );
    ...
    if (report.healthScore < 60) {
      console.error("❌ Memory health is POOR. Immediate action required.");
      process.exit(1);
    } else if (report.healthScore < 80) {
      console.warn("⚠️  Memory health is FAIR. Review recommendations.");
      process.exit(0);
    } else {
      console.log("✅ Memory health is EXCELLENT.");
      process.exit(0);
    }
  } catch (error) {
    console.error("❌ Error running memory health check:", error);
    process.exit(1);
  }
}
