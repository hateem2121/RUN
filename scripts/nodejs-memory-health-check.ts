...
async function measureEventLoopLag(): Promise<EventLoopMetrics> {
  ...
  const activeHandles = processAny._getActiveHandles ? processAny._getActiveHandles().length : 0;
  const activeRequests = processAny._getActiveRequests ? processAny._getActiveRequests().length : 0;
...
function identifyLeakRisks(structures: DataStructureInfo[]): string[] {
  ...
  if (unbounded.length > 0) {
    unbounded.forEach((s) => {
      risks.push(`⚠️  ${s.name}: UNBOUNDED growth - ${s.cleanupStrategy || "No cleanup strategy"}`);
    });
  }
  ...
  if (highRisk.length > 0) {
    highRisk.forEach((s) => {
      risks.push(`❌ ${s.name}: HIGH RISK - ${s.cleanupStrategy || "No cleanup strategy"}`);
    });
  }
  ...
  if (intervalCount > 10) {
    risks.push(`⚠️  Too many setInterval timers: ${intervalCount} (review cleanup on shutdown)`);
  }
...
function generateRecommendations(
  ...
  // Specific fixes
  const alertCooldown = structures.find((s) => s.name === "AlertManager.alertCooldown");
...
async function runHealthCheck(): Promise<MemoryHealthReport> {
  ...
  const recommendations = generateRecommendations(memory, structures, eventLoop);
...
async function generateReport(): Promise<void> {
  try {
    ...
    const mediumRisk = report.dataStructures.filter((s) => s.riskLevel === "medium");
