...
async function fetchMetrics(endpoint: string): Promise<any> {
  ...
  try {
    ...
    console.warn(`   ⚠️ Failed to fetch ${endpoint}:`, error instanceof Error ? error.message : String(error));
    return null;
  }
}

async function collectRealMetrics(): Promise<RealCacheMetrics> {
  console.log('\n📊 Collecting real-time metrics from running server...\n');
  ...
  return {
    ...
    http: {
      ...
      error_rate_percent: ((http.statusCategories?.['5xx'] || 0) / Math.max(http.totalRequests, 1)) * 100
    }
  };
}
...
function analyzeMetrics(metrics: RealCacheMetrics): AnalysisReport {
  ...
  const slowQueryPercentage = metrics.database.total_queries > 0
  ...
  const missComputeTime = metrics.unified_cache.total_misses * metrics.unified_cache.avg_response_time_ms;
  const slowQueryComputeTime = metrics.database.slow_queries * metrics.database.avg_response_time_ms;
  ...
  const cacheableQueryPotential = totalComputeTime > 0 
    ? (potentialSavingsTime / totalComputeTime) * 100
    : 0;
  ...
  if (metrics.unified_cache.hit_rate < 85) {
    bottlenecks.push(`Unified cache hit rate (${metrics.unified_cache.hit_rate.toFixed(1)}%) below 85% target`);
  }
  ...
  if (metrics.unified_cache.evicted_entries > 100) {
    bottlenecks.push(`High cache eviction rate (${metrics.unified_cache.evicted_entries} entries) indicates memory pressure`);
  }
  
  if (metrics.batch_cache.hit_rate < 80) {
    bottlenecks.push(`Batch cache hit rate (${metrics.batch_cache.hit_rate.toFixed(1)}%) below 80% target`);
  }

  if (metrics.database.avg_response_time_ms > 200) {
    bottlenecks.push(`Database avg response time (${metrics.database.avg_response_time_ms.toFixed(1)}ms) exceeds 200ms target`);
  }
  ...
  const totalSavingsPotential = recommendations.reduce((sum, rec) => sum + rec.estimated_savings_percent, 0);
  ...
  return {
    ...
    raw_metrics: metrics,
    analysis: {
      cache_efficiency: overallHitRate >= 85 ? 'EXCELLENT' : overallHitRate >= 70 ? 'GOOD' : 'NEEDS IMPROVEMENT',
...
function generateEvidenceBasedRecommendations(
  ...
  if (hitRateGap > 5) {
    ...
    recommendations.push({
      ...
      implementation: 'Increase SEMI_STATIC TTL from 2hr → 6hr in server/lib/unified-replit-cache.ts',
  ...
  if (metrics.unified_cache.swr_stale_serves < metrics.unified_cache.total_hits * 0.1) {
    ...
    recommendations.push({
      ...
      files_affected: ['server/lib/repositories/product-repository.ts', 'server/routes/core/products.ts'],
  ...
  if (metrics.database.slow_queries > 0) {
    ...
    const slowQueryComputeTime = metrics.database.slow_queries * metrics.database.avg_response_time_ms;
    const slowQuerySavings = totalComputeTime > 0 ? (slowQueryComputeTime / totalComputeTime) * 100 : 0;
  ...
  if (metrics.unified_cache.evicted_entries > 100) {
    ...
    const evictionSavings = totalComputeTime > 0 ? (timeSavedFromEvictions / totalComputeTime) * 100 : 0;
  ...
  if (metrics.http.total_requests > 100) {
    ...
    const httpCacheSavings = totalComputeTime > 0 ? (timeSavedFromHTTPCache / totalComputeTime) * 100 : 0;
...
${report.recommendations.filter(r => r.effort === 'LOW').map(r => 
  `- ${r.title} (${r.estimated_savings_percent}% savings)`
).join('\n') || 'None identified'}

**Phase 1 Total:** ${report.recommendations.filter(r => r.effort === 'LOW').reduce((s, r) => s + r.estimated_savings_percent, 0).toFixed(1)}%

### Phase 2: High-Impact Features (Medium Effort)
${report.recommendations.filter(r => r.effort === 'MEDIUM' && r.impact === 'HIGH').map(r => 
  `- ${r.title} (${r.estimated_savings_percent}% savings)`
).join('\n') || 'None identified'}

**Phase 2 Total:** ${report.recommendations.filter(r => r.effort === 'MEDIUM' && r.impact === 'HIGH').reduce((s, r) => s + r.estimated_savings_percent, 0).toFixed(1)}%
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('   CACHE PERFORMANCE ANALYSIS - REAL METRICS COLLECTION   ');
  console.log('═══════════════════════════════════════════════════════════');
  ...
  try {
    ...
    console.log('═══════════════════════════════════════════════════════════');
    console.log('\n📊 Key Findings (Based on Real Metrics):');
    console.log(`   • Overall Cache Hit Rate: ${report.executive_summary.overall_cache_hit_rate}%`);
    console.log(`   • Unified Cache: ${report.executive_summary.unified_cache_hit_rate.toFixed(1)}%`);
    console.log(`   • Batch Cache: ${report.executive_summary.batch_cache_hit_rate.toFixed(1)}%`);
    console.log(`   • NEON Savings Potential: ${report.executive_summary.neon_compute_savings_potential}%`);
    console.log(`   • Goal Achievement (≥20%): ${report.executive_summary.meets_20_percent_goal ? '✅ YES' : '❌ NO'}`);
    console.log(`   • Recommendations: ${report.recommendations.length}`);
    console.log(`   • Performance Bottlenecks: ${report.analysis.performance_bottlenecks.length}`);
    console.log('\n📝 Review the generated reports for detailed analysis.\n');
    
    process.exit(0);
  } catch (error) {
    console.error('\n❌ Analysis failed:', error instanceof Error ? error.message : String(error));
    console.error('\nℹ️  Make sure the server is running at', BASE_URL);
