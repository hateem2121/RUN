...
async function collectRealMetrics(): Promise<RealCacheMetrics> {
	...
	if (!metricsData) {
		throw new Error(
			"Failed to fetch primary metrics. Is the server running at " +
				BASE_URL +
				"?",
		);
	}
	...
	return {
		...
		unified_cache: {
			...
			l1_memory_usage_mb:
				(unifiedCache.estimatedMemoryUsage || 0) / (1024 * 1024),
		...
		database: {
			...
			avg_response_time_ms:
				database.averageResponseTime || dbPool.averageQueryTime || 0,
		...
		http: {
			...
			error_rate_percent:
				((http.statusCategories?.["5xx"] || 0) /
					Math.max(http.totalRequests, 1)) *
				100,
		},
	};
}
...
function analyzeMetrics(metrics: RealCacheMetrics): AnalysisReport {
	...
	const totalHits =
		metrics.unified_cache.total_hits + metrics.batch_cache.total_requests;
	...
	const missComputeTime =
		metrics.unified_cache.total_misses *
		metrics.unified_cache.avg_response_time_ms;
	const slowQueryComputeTime =
		metrics.database.slow_queries * metrics.database.avg_response_time_ms;
	const totalComputeTime =
		metrics.database.total_queries * metrics.database.avg_response_time_ms;
	...
	if (metrics.database.slow_queries > 0) {
		bottlenecks.push(
			`${metrics.database.slow_queries} slow database queries detected`,
		);
	}
	...
	return {
		...
		executive_summary: {
			...
			neon_compute_savings_potential:
				Math.round(totalSavingsPotential * 100) / 100,
		...
		analysis: {
			...
			neon_compute_analysis: {
				...
				cacheable_query_potential:
					Math.round(cacheableQueryPotential * 100) / 100,
			},
		},
		recommendations,
	};
...
function generateEvidenceBasedRecommendations(
	...
	if (hitRateGap > 5) {
		...
		const timeSavedOnMisses =
			missesPreventable * metrics.unified_cache.avg_response_time_ms;
		const ttlSavings =
			totalComputeTime > 0 ? (timeSavedOnMisses / totalComputeTime) * 100 : 0;
	...
	if (
		metrics.unified_cache.swr_stale_serves <
		metrics.unified_cache.total_hits * 0.1
	) {
		...
		const timeSavedBySWR =
			productMisses * metrics.unified_cache.avg_response_time_ms;
		const swrSavings =
			totalComputeTime > 0 ? (timeSavedBySWR / totalComputeTime) * 100 : 0;
	...
	if (metrics.database.slow_queries > 0) {
		...
		const slowQuerySavings =
			totalComputeTime > 0
				? (slowQueryComputeTime / totalComputeTime) * 100
				: 0;
	...
	if (metrics.unified_cache.evicted_entries > 100) {
		...
		const timeSavedFromEvictions =
			evictionsSaved * metrics.unified_cache.avg_response_time_ms;
		const evictionSavings =
			totalComputeTime > 0
				? (timeSavedFromEvictions / totalComputeTime) * 100
				: 0;
	...
	if (metrics.http.total_requests > 100) {
		...
		const timeSavedFromHTTPCache =
			cacheableRequests * metrics.http.avg_latency_ms;
		const httpCacheSavings =
			totalComputeTime > 0
				? (timeSavedFromHTTPCache / totalComputeTime) * 100
				: 0;
...
${
	report.analysis.performance_bottlenecks.length > 0
		? report.analysis.performance_bottlenecks
				.map((b, idx) => `${idx + 1}. ${b}`)
				.join("\n")
