...
async function completeWebsiteContent() {
  console.log("=== COMPLETING WEBSITE CONTENT ===\n");
  ...
  try {
    ...
    if (!hero || Object.keys(hero).length === 0) {
      console.log("Creating homepage hero...");
      const heroData = {
    ...
    if (slogans.length === 0) {
      console.log("Creating homepage slogans...");
      const sampleSlogans = [
        {
    ...
    if (processCards.length === 0) {
      console.log("Creating process cards...");
      const cards = [
        {
    ...
    console.error("Homepage creation failed:", error);
  }
