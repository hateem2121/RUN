  ...
  lines.forEach((line, idx) => {
    ...
    if (line.includes("Math.random()")) {
      ...
      if (
        ...
        errors.push(
          `Line ${lineNum}: Potential non-deterministic Math.random() usage.`
        );
      }
    }
    ...
    if (line.includes("new Date(") && !line.includes("useEffect")) {
      if (line.match(/new Date\(\s*\)/)) {
        errors.push(
          `Line ${lineNum}: Potential non-deterministic new Date() usage.`
        );
      }
    }
    ...
    if (
      ...
      if (!line.includes("window.location.href") && !line.includes("open(")) {
        errors.push(
          `Line ${lineNum}: Potential unsafe window/document access in render.`
        );
      }
    }
  });
...
console.log("🔍 Scanning for non-deterministic code patterns...");
...
if (foundIssues) {
  console.log(
    "\n❌ Potential non-deterministic code found. Please verify usage and suppress if safe with // IGNORE_DETERMINISM"
  );
  process.exit(1);
} else {
  console.log("\n✅ No obvious non-deterministic patterns found.");
  process.exit(0);
}
