...
async function seedMissingEntities() {
	try {
		...
		const insertedFeaturedProducts = await db
		...
		const insertedTeamMessages = await db
		...
		const sustainabilityInitiativesData = [
			...
			},
			{
				...
				description:
					"Converting all manufacturing facilities to 100% renewable energy sources.",
				impact:
					"Reduced carbon footprint by 60% and achieved carbon neutrality in 3 facilities.",
				status: "active",
		...
		const sustainabilityGoalsData = [
			...
			},
			{
				...
				description:
					"Implement closed-loop manufacturing with 90% material reuse and recycling.",
		...
		const insertedManufacturingHero = await db
		...
		const manufacturingCapabilitiesData = [
			{
				...
				description:
					"Large-scale manufacturing capabilities with consistent quality control.",
				capacity: "1,000,000",
				unit: "units/month",
				category: "production",
				specifications: {
			...
			},
			{
				...
				description:
					"Full-service design and development from concept to production.",
				capacity: "50+",
				unit: "designs/month",
				category: "design",
				specifications: {
		...
		const insertedCapabilities = await db
		...
		const insertedTechnologyHero = await db
		...
		const technologyEquipmentData = [
			{
				...
				model: "Vector iX9",
				description:
					"High-precision automated cutting system with AI-optimized fabric utilization.",
				specifications: {
					accuracy: "±0.1mm",
					speed: "120m/min",
			...
			},
			{
				...
				model: "TERRA 320",
				description:
					"Advanced digital textile printing system for custom designs and small batches.",
				specifications: {
					width: "3200mm",
					speed: "550 sqm/h",
					resolution: "1800 dpi",
		...
		const insertedResearch = await db
