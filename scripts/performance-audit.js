...
class PerformanceAuditor {
  ...
  async analyzeAssets() {
    ...
    const _totalSize = assets.reduce((sum, asset) => sum + asset.size, 0);

    // Find large assets
    const largeAssets = assets.filter((asset) => asset.size > 500 * 1024); // 500KB
    if (largeAssets.length > 0) {
      largeAssets.forEach((_asset) => {});
      this.results.recommendations.push(
    ...
    const images = assets.filter((asset) => asset.name.match(/\.(jpg|jpeg|png|gif|bmp)$/i));
...
Generated by RUN APPAREL Performance Auditor
  ...
  formatRecommendations() {
    ...
    return this.results.recommendations.map((rec, index) => `${index + 1}. ${rec}`).join("\n");
  }
