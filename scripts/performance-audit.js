...
class PerformanceAuditor {
  ...
  async runAudit() {
    console.log('🚀 Starting Performance Audit for RUN APPAREL Homepage...\n');
  ...
  async analyzeBundleSize() {
    ...
    console.log(`   📊 JavaScript: ${this.formatSize(jsSize)}`);
    console.log(`   🎨 CSS: ${this.formatSize(cssSize)}`);
    console.log(`   📈 Total: ${this.formatSize(totalSize)}`);
    
    // Add recommendations based on size
    if (totalSize > 2 * 1024 * 1024) {
      this.results.recommendations.push('Bundle size exceeds 2MB - implement code splitting');
    }
    if (jsSize > 1 * 1024 * 1024) {
      this.results.recommendations.push('JavaScript bundle is large - lazy load non-critical components');
    }
    if (cssSize > 500 * 1024) {
      this.results.recommendations.push('CSS bundle is large - consider purging unused styles');
    }
  }
  ...
  estimateGzippedSize(size, ext) {
    ...
    return Math.round(size * (compressionRatios[ext] || 0.60));
  }
  ...
  async analyzeDependencies() {
    ...
    if (foundHeavyDeps.includes('three')) {
      this.results.recommendations.push('Three.js found - ensure 3D components are lazy loaded');
    }
    if (foundHeavyDeps.includes('gsap')) {
      this.results.recommendations.push('GSAP found - lazy load for non-critical animations');
    }
    if (foundHeavyDeps.includes('locomotive-scroll')) {
      this.results.recommendations.push('Locomotive Scroll found - lazy load for non-critical pages');
    }
  }
  ...
  async checkLazyLoading() {
    ...
    const nonLazyComponents = componentFiles.filter(file => 
    ...
    if (nonLazyComponents.length > 10) {
      this.results.recommendations.push(`${nonLazyComponents.length} components could be lazy loaded`);
    }
  }
  ...
  async analyzeAssets() {
    ...
    console.log(`   📊 Static assets: ${assets.length} files`);
    console.log(`   📈 Total size: ${this.formatSize(totalSize)}`);
    
    // Find large assets
    const largeAssets = assets.filter(asset => asset.size > 500 * 1024); // 500KB
    if (largeAssets.length > 0) {
      console.log(`   ⚠️  Large assets (>500KB): ${largeAssets.length}`);
      largeAssets.forEach(asset => {
        console.log(`      • ${asset.name}: ${this.formatSize(asset.size)}`);
      });
      this.results.recommendations.push(`${largeAssets.length} large assets found - consider optimization`);
    }
    ...
    if (images.length > 0) {
      ...
      if (largeImages.length > 0) {
        this.results.recommendations.push(`${largeImages.length} large images found - consider WebP format and compression`);
      }
    }
  }
  ...
  calculateScore() {
    ...
    const jsSize = bundleAnalysis.js?.reduce((sum, file) => sum + file.size, 0) || 0;
    const cssSize = bundleAnalysis.css?.reduce((sum, file) => sum + file.size, 0) || 0;
    const totalSize = jsSize + cssSize;
    
    if (totalSize > 2 * 1024 * 1024) score -= 30; // 2MB
    else if (totalSize > 1 * 1024 * 1024) score -= 20; // 1MB
...
Generated by RUN APPAREL Performance Auditor
  ...
  formatBundleAnalysis() {
    ...
    if (bundleAnalysis.js) {
      const jsSize = bundleAnalysis.js.reduce((sum, file) => sum + file.size, 0);
      output += `JavaScript: ${this.formatSize(jsSize)} (${bundleAnalysis.js.length} files)\n`;
    }
    
    if (bundleAnalysis.css) {
      const cssSize = bundleAnalysis.css.reduce((sum, file) => sum + file.size, 0);
