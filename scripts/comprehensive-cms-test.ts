...
async function comprehensiveCMSTesting() {
	try {
		...
		try {
			...
			if (
				updatedHero.length === 1 &&
				updatedHero[0].title === "Updated Test Hero"
			) {
				passedTests++;
				testResults.homepage.passed++;
			} else {
		...
		try {
			...
			if (timeline.length >= 3) {
				...
				const sortedByYear = timeline.sort(
					(a, b) => parseInt(a.year) - parseInt(b.year),
				);
				if (
					sortedByYear[0].year &&
					parseInt(sortedByYear[0].year) <
						parseInt(sortedByYear[sortedByYear.length - 1].year)
				) {
					passedTests++;
		...
		try {
			...
			if (processes.length >= 2) {
				...
				const processesWithEquipment = processes.filter(
					(p) =>
						p.equipment && Array.isArray(p.equipment) && p.equipment.length > 0,
				);
		...
		try {
			...
			if (innovations.length >= 2) {
				...
				const techCategories = [
					...new Set(innovations.map((i) => i.category).filter(Boolean)),
				];
				...
				const innovationsWithBenefits = innovations.filter(
					(i) =>
						i.benefits && Array.isArray(i.benefits) && i.benefits.length > 0,
				);
		...
		Object.entries(testResults).forEach(([category, results]) => {
			const successRate =
				results.tests > 0
					? ((results.passed / results.tests) * 100).toFixed(1)
					: "0";
		});
