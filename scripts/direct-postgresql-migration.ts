...
export class DirectPostgreSQLMigration {
	...
	async executeHybridMigration(): Promise<MigrationResult> {
		...
		try {
			...
			result.migrated.fabricCompositions =
				await this.migrateFabricCompositions();
	...
	private logMigrationSummary(result: MigrationResult) {
		const totalMigrated = Object.values(result.migrated).reduce(
			(a, b) => a + b,
			0,
		);
		const totalPreserved = Object.values(result.preserved).reduce(
			(a, b) => a + b,
			0,
		);
