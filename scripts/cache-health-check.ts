...
async function analyzeCacheHealth(): Promise<CacheAnalysis> {
	...
	if (metrics.hitRate < 95) {
		...
		if (metrics.hitRate < 80) {
			recommendations.push(
				"CRITICAL: Optimize TTL settings for frequently accessed data",
			);
			recommendations.push(
				"Consider implementing cache warmup for common queries",
			);
		} else if (metrics.hitRate < 90) {
			recommendations.push(
				"Extend TTL for static content (size charts, categories)",
			);
	...
	if (memoryMB > 80) {
		issues.push(
			`Memory usage (${memoryMB.toFixed(2)}MB) approaching limit (100MB)`,
		);
	...
	if (metrics.memoryPressureDetected) {
		...
		recommendations.push(
			"Review entry sizes and consider compression for large objects",
		);
	}
	...
	if (metrics.evictedEntries > 100) {
		issues.push(
			`High eviction count (${metrics.evictedEntries}) - possible cache thrashing`,
		);
	...
	if (metrics.avgResponseTime > 500) {
		issues.push(
			`Average response time (${metrics.avgResponseTime}ms) exceeds 500ms target`,
		);
...
async function generateHealthReport(): Promise<void> {
	try {
		...
		const memoryStatus =
			memoryMB < 50 ? "✅ HEALTHY" : memoryMB < 80 ? "⚠️ MODERATE" : "❌ HIGH";
