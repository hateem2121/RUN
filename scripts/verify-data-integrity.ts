...
async function main() {
	...
	try {
		...
		const invalidMOQ = await db
			.select()
			.from(products)
			.where(lte(products.minimumOrderQuantity, 0));
		...
		if (orphanedProducts.length > 0) {
			logWarn(
				`Found ${orphanedProducts.length} products without a category (orphaned)`,
			);
		} else {
		...
		const fabricsWithComp = await db
			.select({
		...
		const fabricsWithoutComp = allFabrics.filter(
			(f) => !fabricIdsWithComp.has(f.id),
		);

		if (fabricsWithoutComp.length > 0) {
			logWarn(
				`${fabricsWithoutComp.length} fabrics have no fiber composition defined`,
			);
		} else {
		...
		const pastGoals = susGoals.filter(
			(g) =>
				g.targetYear !== null &&
		...
		if (pastGoals.length > 0) {
			logWarn(
				`${pastGoals.length} sustainability goals have past target years but are not 100% complete`,
			);
		} else {
		...
		if (navItems.length === 0)
			logFail("No Navigation Items defined (Menu is empty)");
		...
		const invalidEmails = await db
			.select()
			.from(inquiries)
			.where(notLike(inquiries.email, "%@%"));
		if (invalidEmails.length > 0) {
			logFail(
				`Found ${invalidEmails.length} inquiries with invalid email formats`,
			);
		} else {
		...
		const negativeMetrics = await db
			.select()
			.from(performanceMetrics)
			.where(lt(performanceMetrics.value, "0"));
		if (negativeMetrics.length > 0) {
			logFail(
				`Found ${negativeMetrics.length} performance metrics with negative values`,
			);
		} else {
		...
		const productsMissingMedia = await db
			.select({ id: products.id, name: products.name })
			.from(products)
			.leftJoin(mediaAssets, eq(products.primaryImageId, mediaAssets.id))
			.where(
				and(isNotNull(products.primaryImageId), sql`${mediaAssets.id} IS NULL`),
			);

		if (productsMissingMedia.length > 0) {
			logFail(
				`${productsMissingMedia.length} products reference non-existent media assets`,
			);
		} else {
		...
		const catsMissingMedia = await db
			.select({ id: categories.id, name: categories.name })
			.from(categories)
			.leftJoin(mediaAssets, eq(categories.primaryImageId, mediaAssets.id))
			.where(
				and(
					isNotNull(categories.primaryImageId),
					sql`${mediaAssets.id} IS NULL`,
				),
			);

		if (catsMissingMedia.length > 0) {
			logFail(
				`${catsMissingMedia.length} categories reference non-existent media assets`,
			);
		} else {
