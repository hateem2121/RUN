...
(async () => {
  ...
  if (counter === 0) {
    console.log("Counter is 0, scanning for actual assets...");
    ...
    console.log(`Found assets up to ID: ${actualCounter}`);
  }
  
  console.log(`Checking ${actualCounter} media assets`);
  ...
  for (let id = 1; id <= actualCounter; id++) {
    ...
    if (migrationCount < 3) {
      console.log(`Asset ${id} structure:`, {
    ...
    if (needsUpdate) {
      ...
      const updatedAsset = typeof asset === 'string' ? JSON.stringify(clone) : 
                          (parsedAsset.value ? { ...parsedAsset, value: clone } : clone);
      
      await db.set(key, updatedAsset);
      updateCount++;
      console.log(`✅ Updated asset ${id}: ${value.filename}`);
    }
  ...
  console.log(`📁 Backed up ${migrationCount} assets`);
  console.log(`🔄 Updated ${updateCount} assets with proxy URLs`);
})();