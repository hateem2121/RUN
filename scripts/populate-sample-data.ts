...
async function populateAllSampleData() {
  ...
  // 1. PRODUCTS - Core manufacturing products
  ...
  // 2. FABRICS - Manufacturing fabric specifications
  console.log("\n--- Creating Fabrics ---");
  try {
    ...
    if (fabrics.length === 0) {
      ...
      for (const fabric of sampleFabrics) {
        ...
        console.log(`✅ Created fabric: ${created.name} (${created.weight}g/m²)`);
        createdItems.push(`Fabrics: ${created.name}`);
      }
    }
  } catch (error) {
    console.error("❌ Fabrics creation failed:", error);
  }

  // 3. NAVIGATION ITEMS - Website navigation
  ...
  try {
    ...
    if (navItems.length === 0) {
      ...
      for (const item of sampleNavItems) {
        ...
        console.log(`✅ Created navigation: ${created.name} (${created.path})`);
  ...
  // 4. FOOTER SECTIONS & LINKS
  console.log("\n--- Creating Footer Sections & Links ---");
  try {
    ...
    if (footerSections.length === 0) {
      ...
      for (const section of sampleFooterSections) {
        ...
        console.log(`✅ Created footer section: ${created.name}`);
      ...
      const footerLinks = [
        ...
        { name: "About Us", path: "/about", sectionId: 2, order: 1, isActive: true },
        { name: "Manufacturing", path: "/manufacturing", sectionId: 2, order: 2, isActive: true },
        { name: "Sustainability", path: "/sustainability", sectionId: 2, order: 3, isActive: true },
        { name: "Careers", path: "/careers", sectionId: 2, order: 4, isActive: true },

        // Support section (sectionId: 3)
        { name: "Contact Us", path: "/contact", sectionId: 3, order: 1, isActive: true },
        { name: "Size Guide", path: "/size-guide", sectionId: 3, order: 2, isActive: true },
        { name: "Custom Orders", path: "/custom-orders", sectionId: 3, order: 3, isActive: true },
        { name: "FAQ", path: "/faq", sectionId: 3, order: 4, isActive: true },

        // Legal section (sectionId: 4)
        { name: "Privacy Policy", path: "/privacy", sectionId: 4, order: 1, isActive: true },
        { name: "Terms of Service", path: "/terms", sectionId: 4, order: 2, isActive: true },
        { name: "Quality Standards", path: "/quality", sectionId: 4, order: 3, isActive: true },
        { name: "Certifications", path: "/certifications", sectionId: 4, order: 4, isActive: true },
      ];

      for (const link of footerLinks) {
        const created = await storage.createFooterLink(link);
        console.log(`✅ Created footer link: ${created.name}`);
  ...
  try {
    ...
    if (!hero || Object.keys(hero).length === 0) {
      ...
      console.log(`✅ Created homepage hero: ${createdHero.title}`);
    ...
    if (sections.length === 0) {
      ...
      for (const section of sampleSections) {
        ...
        console.log(`✅ Created homepage section: ${created.title}`);
  ...
  try {
    const hero = await storage.getHomepageHero();
    console.log(`✅ Homepage Hero: ${hero && hero.title ? "configured" : "not configured"}`);
  } catch (error) {
    console.log(`❌ Homepage objects: ERROR`);
  }
}
