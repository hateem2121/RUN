...
async function verifyDatabaseRelationships() {
  ...
  Object.entries(entityCounts).forEach(([_entity, _count]) => {});
  allChecks++;
  passedChecks++;
  ...
  const productsWithValidPrices = allProducts.filter(
    (p) => !p.basePrice || !Number.isNaN(parseFloat(p.basePrice)),
  );
  ...
  Object.entries(categoryProducts).forEach(([_category, _count]) => {});
  const fabricUsage = {};
  for (const product of allProducts) {
    if (product.fabricId) {
      const fabric = await db.select().from(fabrics).where(eq(fabrics.id, product.fabricId));
  ...
  Object.entries(fabricUsage).forEach(([_fabric, _count]) => {});
