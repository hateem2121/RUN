...
async function verifyDatabaseRelationships() {
	try {
		...
		allChecks++;
		if (
			productCategoryLinks === allProducts.filter((p) => p.categoryId).length
		) {
			passedChecks++;
		} else {
		}
		...
		allChecks++;
		if (
			productMediaLinks === allProducts.filter((p) => p.primaryImageId).length
		) {
			passedChecks++;
		} else {
		}
		...
		const validFiberTypes = fibersWithType.filter(
			(f) => f.type && f.type.length > 0,
		);
			...
			if (product.categoryId) {
				...
				if (category.length > 0) {
					...
					if (!categoryProducts[categoryName])
						categoryProducts[categoryName] = 0;
