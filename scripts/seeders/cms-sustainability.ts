...
export async function seedSustainabilityInitiatives(): Promise<SeedResult> {
	return seedWithTransaction("sustainabilityInitiatives", async () => {
		...
		const sustainImages = allMedia.filter((m) =>
			m.filename?.startsWith("sustainability-"),
		);
		...
		const goalsData = [
			{
				...
				description:
					"Transition all facilities to 100% renewable energy by 2026",
				targetYear: 2026,
				category: "energy",
				progress: 80,
				status: "in_progress",
			...
			},
			{
				...
				description:
					"Achieve zero waste to landfill status across all manufacturing sites",
				targetYear: 2025,
				category: "waste",
				progress: 90,
				status: "in_progress",
			...
			},
			{
				...
				description:
					"Become water neutral through 100% recycling and rainwater harvesting",
				targetYear: 2027,
				category: "water",
				progress: 60,
				status: "in_progress",
			...
			},
			{
				...
				description:
					"Source 95% of materials from certified sustainable suppliers",
				targetYear: 2026,
				category: "materials",
				progress: 65,
				status: "in_progress",
			...
			},
			{
				...
				description:
					"Achieve full supply chain transparency and ethical sourcing certification",
				targetYear: 2025,
				category: "ethics",
				progress: 75,
				status: "in_progress",
	...
	return seedWithTransaction("sustainabilityFeatures", async () => {
		const featuresData = [
			...
			},
			{
				...
				description:
					"High-performance recycled polyester made from post-consumer plastic bottles",
				category: "materials",
				icon: "Recycle",
				isActive: true,
				sortOrder: 2,
			},
			{
				...
				description:
					"Water-based, non-toxic dyes with minimal environmental impact",
				category: "chemicals",
				icon: "Droplet",
				isActive: true,
				sortOrder: 3,
			},
			{
				...
				description:
					"Latest generation equipment reduces energy consumption by 40%",
				category: "energy",
				icon: "Zap",
				isActive: true,
				sortOrder: 4,
			},
			{
				...
				description:
					"Compostable packaging materials for minimal environmental footprint",
				category: "packaging",
				icon: "Package",
				isActive: true,
				sortOrder: 5,
...
export async function seedUnifiedSustainability(): Promise<SeedResult> {
	return seedWithTransaction("unifiedSustainability", async () => {
		const unifiedData = {
			...
			subheadline:
				"Integrating environmental, social, and economic responsibility",
			content:
				"We believe in a future where manufacturing is a force for good.",
