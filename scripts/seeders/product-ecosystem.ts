...
export async function seedFabricCompositions(): Promise<SeedResult> {
	return seedWithTransaction("fabricCompositions", async () => {
		...
		if (perfMesh && polyester) {
			compositions.push({
				fabricId: perfMesh.id,
				fiberId: polyester.id,
				percentage: "100",
			});
		}
...
export async function seedProducts(): Promise<SeedResult> {
	return seedWithTransaction("products", async () => {
		...
		const athleticCategory = allCategories.find(
			(c) => c.slug === "athletic-wear",
		);
		const uniformsCategory = allCategories.find(
			(c) => c.slug === "team-uniforms",
		);
		const corporateCategory = allCategories.find(
			(c) => c.slug === "corporate-apparel",
		);

		const performanceFabric = allFabrics.find(
			(f) => f.name === "Performance Mesh",
		);
		...
		const productData = [
			{
				...
				sku: "PPT-001",
				description:
					"Professional grade performance t-shirt with moisture-wicking technology",
			...
			},
			{
				...
				sku: "ETJ-001",
				description:
					"Sustainable team jersey made from organic cotton and recycled polyester",
		...
		const certData = [
			{
				...
				description:
					"Global organic textile standard certification for organic cotton processing",
