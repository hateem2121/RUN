            ...
            navigationColumns: [
                {
                    ...
                    links: [
                        ...
                        { label: 'Corporate Apparel', href: '/products?category=corporate' },
...
export async function seedContactPageConfiguration(): Promise<SeedResult> {
    return seedWithTransaction('contactPageConfigurations', async () => {
        const contactData = {
            ...
            workingHours: 'Monday - Friday: 9:00 AM - 6:00 PM PKT\\nSaturday: 9:00 AM - 2:00 PM PKT',
            ...
            platformOptions: ['Phone Call', 'WhatsApp', 'WeChat', 'Telegram', 'Email', 'Other'],
            formButtonText: 'Get a Response Within 24 Hours',
            formPrivacyText: 'We value your privacy and will never share your information.',
            successHeading: 'Thank you!',
            successMessage: 'Your message has been received. We\'ll get back to you within 24 hours.',
            isActive: true
        };

        return await db.insert(contactPageConfigurations).values(contactData).returning();
    });
}
...
export async function seedLogoAnimationSettings(): Promise<SeedResult> {
    return seedWithTransaction('logoAnimationSettings', async () => {
        ...
        return await db.insert(logoAnimationSettings).values([{ ...logoData, name: 'default' }]).returning();
    });
}
...
export async function seedNavigationGlassmorphismSettings(): Promise<SeedResult> {
    return seedWithTransaction('navigationGlassmorphismSettings', async () => {
        ...
        return await db.insert(navigationGlassmorphismSettings).values([glassData]).returning();
    });
}
