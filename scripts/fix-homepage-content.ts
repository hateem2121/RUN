...
async function fixHomepageContent() {
	...
	try {
		...
		const heroData = {
			...
			subtitle:
				"Quality athletic apparel for teams and organizations worldwide",
			description:
			...
			const cards = [
				{
					...
					description:
						"Custom sportswear design that meets your specific requirements",
					order: 1,
					isActive: true,
				},
				{
					...
					description:
						"Choose from extensive library of high-performance fabrics",
					order: 2,
					isActive: true,
				},
				{
		...
		const sectionData = [
			...
			},
			{
				...
				content:
					"Committed to environmental responsibility and eco-friendly materials",
				isActive: true,
			},
		];

		for (const section of sectionData) {
			const updated = await storage.updateHomepageSection(
				section.name,
				section,
			);
