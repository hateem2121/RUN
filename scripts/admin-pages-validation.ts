	...
	try {
		...
		try {
			...
			const sustainabilityData =
				await db.query.homepageSustainability.findFirst();
			if (sustainabilityData) {
				// Check if metrics field is properly formatted
				if (
					sustainabilityData.metrics &&
					typeof sustainabilityData.metrics === "object"
				) {
					passedChecks++;
				} else {
			...
			const featuredProductsData =
				await db.query.homepageFeaturedProductsSettings.findFirst();
			...
			try {
				...
				if (response.ok) {
					...
					if (html.includes("<!DOCTYPE html") && html.includes("<script")) {
						...
						issues.push(
							`${route}: Route serves content but may not be fully functional`,
						);
						failedChecks++;
					}
				} else {
