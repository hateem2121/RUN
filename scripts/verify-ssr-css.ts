	...
	try {
		...
		const criticalStyle = styleTags.find(
			(s: any) =>
				...
				s.textContent?.includes("background-color: #09090b"),
		);

		if (!criticalStyle) {
			errors.push(
				"❌ Missing critical inline CSS (body { background-color: #09090b })",
			);
		} else {
		}
		...
		if (!hasCssLink) {
			errors.push(
				"❌ Missing external stylesheet link (<link rel='stylesheet'>)",
			);
		} else {
		}
		...
		if (html.includes("<!--ssr-styles-->")) {
			errors.push(
				"❌ <!--ssr-styles--> placeholder still present (not replaced)",
			);
		} else {
		}
