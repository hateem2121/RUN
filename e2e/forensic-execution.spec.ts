...
const TARGET_ROUTES: RouteConfig[] = [
  ...
  { path: "/categories/women/tops/t-shirt-1", name: "product-detail", criticality: "High" },
  ...
  { path: "/resources/size-charts", name: "resource-size-charts", criticality: "Medium" },
  { path: "/resources/fabrics", name: "resource-fabrics", criticality: "Medium" },
  { path: "/admin", name: "admin-dashboard", criticality: "High", skipMobile: true },
  { path: "/admin/media", name: "admin-media", criticality: "High", skipMobile: true },
];
...
test.describe("Forensic UI Audit", () => {
      ...
      for (const scheme of COLOR_SCHEMES) {
        ...
        test(`Forensic Snapshot: ${route.name} [${viewport.name}] [${scheme}]`, async ({
          ...
          if (consoleErrors.length > 0) {
            console.warn(`[Forensic Warning] Console Errors on ${route.path}:`, consoleErrors);
          }
  ...
  test("Hotspot: Manufacturing Token Consistency", async ({ page }) => {
    ...
    console.log("[Forensic Info] Manufacturing Primary Light Token:", primaryColor);
