...
test.describe("Phase 5: Forensic UI Proof", () => {
  ...
  test("P0: Contact Page Content & No Loading State", async ({ page }) => {
    ...
    // Capture Proof
    await page.screenshot({ path: "e2e/artifacts/contact-proof.png", fullPage: true });
  });
  ...
  test("P2: Z-Index Hardening (Header Layering)", async ({ page }) => {
    ...
    if (isLoading) console.log("[Test] Warning: Page shows 'Loading...' state.");

    await page.waitForSelector('[data-testid="hydration-status"]', { state: "attached" });
    try {
      await expect(page.getByTestId("hydration-status")).toHaveText("HYDRATED", { timeout: 30000 });
    } catch (e) {
      console.log("[Test] Hydration Timeout. Dumping HTML:");
    ...
    console.log(`[Test] Dialog Content Z-Index: ${zIndex}`);
    ...
    console.log(`[Test] Is Mock Header Logo Clickable? ${isLogoClickable}`);

    if (isLogoClickable) {
      console.error("[Test] FAILURE: Mock Header Logo is clickable (Above Overlay/Backdrop).");
      // Fail manually
      expect(isLogoClickable).toBe(false);
    } else {
      console.log("[Test] SUCCESS: Mock Header Logo is NOT clickable (Covered).");
    }
