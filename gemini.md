...
| `z-(--z-index-*)` or `text-(--text-*)` | Direct `@theme` functional utilities (`z-dock`, `text-display-xl`) | Critical |
| Arbitrary variable brackets for `@theme` tokens | First-class functional utilities generated by Tailwind v4 | High |
...
- **Bulk Refactoring & CSS Invariant**: NEVER use generic regex or search-and-replace scripts to modify TypeScript syntax, CSS class names, or Radix UI data attributes (`data-[state=...]`). Doing so corrupts valid selectors into non-existent phantom classes (e.g. `custom-misc-*`). Rely exclusively on careful `ts-morph` AST manipulation or explicit design token definitions inside `client/app/styles/theme.css`.
...
- **Code Scanning Analysis Purging**: When removing or retiring security scanning workflows (e.g., OSSAR, Hadolint, Bandit), purge stale historical analyses via the GitHub REST API (`DELETE /repos/{owner}/{repo}/code-scanning/analyses/{id}?confirm_delete`) to clear orphaned "out of date / reporting problems" tool status warnings from the Security Overview.
- **Sub-Router Rate Limiting Attachment**: To satisfy CodeQL's static AST dataflow analysis (`js/missing-rate-limiting`), rate-limiting middleware (`criticalTier`, `apiTier`, `publicTier`, `uploadTier`) MUST be mounted directly at the individual sub-router file level (`router.use(...)`) or directly on route handlers containing authorization guards (`authService.requireAdmin`). CodeQL evaluates files in isolation and cannot trace parent router middleware cascading across separate modules.
- **Timer Delay Bounds & Resource Exhaustion**: When implementing timer or timeout wrappers (`setTimeout`, `setInterval`), always enforce static, bounded fallback limits (e.g., `delay >= 100 && delay <= 60000`) before scheduling the timer to prevent CodeQL `js/resource-exhaustion` (CWE-400 / CWE-770) flags.
- **Container Image SHA256 Digest Pinning**: Base images in Dockerfiles MUST be pinned to immutable SHA256 digest hashes alongside their tag (e.g., `FROM node:24-alpine@sha256:<digest>`) to satisfy OpenSSF Scorecard `Pinned-Dependencies` invariant.

### 5.1.6 Tailwind CSS v4 Theme Token Resolution & SWR Cache Invariant

- **Direct Token Utilities**: In Tailwind CSS v4, any custom property registered under `@theme` inside `client/app/styles/theme.css` (e.g., `--z-index-dock: 1100;`, `--text-display-xl: clamp(...);`, `--shadow-dock: ...;`) MUST be consumed using its direct functional utility class (`z-dock`, `text-display-xl`, `shadow-dock`).
- **Forbidden Bracket Syntax**: NEVER use arbitrary variable brackets such as `z-(--z-index-dock)`, `text-(--text-display-xl)`, or `shadow-(--shadow-dock)`. In Tailwind CSS v4, this syntax emits zero CSS rules, causing properties to silently drop and fall back to browser defaults.
- **In-Memory Two-Tier SWR Cache Invalidation**: When executing standalone database manipulation, sanitization, or migration scripts against Neon Postgres, the running development server's in-memory L1 cache (`twoTierBatchCache` in `server/lib/cache/two-tier-batch.ts`) retains batch data for up to 120 minutes. You MUST trigger cache bypass by requesting batch endpoints with `referer: /admin` or restarting the dev process to propagate new database rows to the client.

### 5.1.7 Visual Auditing, Fluid Typography & WCAG 2.2 Invariants

- **Fluid Display Typography Clamp Bounds**: Any custom fluid typography token registered under `@theme` (e.g., `--text-display-xl`, `--text-display-2xl`) consumed by uppercase brutalist display headings MUST clamp the mobile minimum bound to `≤ 2.125rem` (34px). Never use minimum bounds `>= 3rem` for mobile display titles, as words longer than 8 characters will clip on 375px viewports.
- **Admin Visual Capture Authentication**: Screenshot capture harnesses and visual regression tests targeting `/admin/*` routes MUST route through `/api/auth/mock-login?returnTo=${encodeURIComponent(route)}` and wait for DOM stabilization (`Checking access...` removed) before taking snapshots to prevent capturing unauthenticated loading shells.
- **WCAG 2.2 Focus Not Obscured (SC 2.4.11)**: All scroll containers in layouts featuring sticky or floating dock headers MUST declare `scroll-padding-top: 5rem` (or equivalent header height) to ensure keyboard-focused elements are never obscured by floating UI elements.
