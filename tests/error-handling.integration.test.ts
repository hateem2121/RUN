  ...
  describe("🛡️ Security & Validation", () => {
    test("POST /api/products - Should validate payload using Zod", async () => {
      ...
      if (response.status === 400) {
        ...
        // Check z.min(3) error
        // Note: details structure depends on ZodError mapping in validation.ts
  ...
  describe("🧠 Observability & Metrics", () => {
    test("GET /metrics - Should expose Prometheus metrics", async () => {
      ...
      if (response.status === 200) {
        ...
        console.warn("⚠️ /metrics endpoint might not be mounted");
      }
    });
    ...
    test("GET /health/deep - Should return system health status", async () => {
      ...
      if (response.status === 200 || response.status === 503) {
        ...
        console.warn(`⚠️ /health/deep endpoint returned ${response.status}`);
      }
    });
  });
  ...
  describe("🔌 Resilience", () => {
    test("Database failure simulation (using Mock if possible or check handling)", async () => {
      ...
      try {
        ...
        console.log("✅ DB Connection active during integration test");
      } catch (e) {
        console.warn("⚠️ DB Connection failed during test (expected if testing resilience)");
      }
    });
  });
});
