    ...
    try {
      ...
      if (response.ok) {
        ...
        if (health.overall === "healthy") {
          consecutiveHealthy++;
          console.log(`✓ Health check ${i + 1}: healthy (${consecutiveHealthy}/${CONFIG.healthyThreshold})`);
...
async function verifyLowErrorRate(): Promise<{ low: boolean; rate?: number }> {
  ...
  const requests = Array(10).fill(null).map(() =>
    fetch(`${CONFIG.targetUrl}${CONFIG.healthEndpoint}`)
  );
...
async function verifyRecovery(): Promise<RecoveryReport> {
  ...
  console.log(cbResult.closed ? `✓ Circuit breaker: ${cbResult.state}` : `✗ Circuit breaker: ${cbResult.state}`);
