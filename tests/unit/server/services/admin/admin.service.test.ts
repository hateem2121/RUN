...
vi.mock('../../../../../server/lib/resilience/circuit-breaker.js', () => ({
  DB_CIRCUIT_OPTIONS: {},
  withCircuit: vi.fn(async (name, fn) => fn())
}));
...
import { AppError, InternalError, NotFoundError } from '../../../../../server/lib/errors.js';
...
describe('AdminService', () => {
  ...
  describe('getInitialProductsData', () => {
    ...
    it('should handle repository failure and return error', async () => {
      vi.mocked(productRepository.getProductsIncludingDeleted).mockRejectedValueOnce(new Error('DB error'));
    ...
    it('should return AppError if thrown', async () => {
      vi.mocked(productRepository.getProductsIncludingDeleted).mockRejectedValueOnce(new NotFoundError('Missing'));
      const result = await adminService.getInitialProductsData();
      expect(result.isErr()).toBe(true);
      if (result.isErr()) {
  ...
  describe('getProductsList', () => {
    it('should filter and return products list on success', async () => {
      ...
      const result = await adminService.getProductsList({
        page: 1, limit: 50, search: 'shirt', categoryId: '1', status: 'active'
      });
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        expect(result.value.products.length).toBe(1);
      ...
      const resFeatured = await adminService.getProductsList({ page:1, limit: 50, status: 'featured' });
      expect(resFeatured.isOk()).toBe(true);

      vi.mocked(productRepository.getProductsIncludingDeleted).mockResolvedValueOnce([
        { id: 3, name: 'Deleted', sku: 'D-01', deletedAt: new Date() } as any,
      ]);
      const resDeleted = await adminService.getProductsList({ page:1, limit: 50, status: 'deleted' });
    ...
    it('should handle failures', async () => {
      vi.mocked(productRepository.getProductsIncludingDeleted).mockRejectedValueOnce(new Error('fail'));
  ...
  describe('fixCorruptedMedia', () => {
    it('should fix corrupted media and log audit', async () => {
      vi.mocked(productRepository.getCategories).mockResolvedValueOnce([
        ...
        {
          id: 2, name: 'C2', featuredContent: null
        } as any
      ]);
  ...
  describe('updateAuditConfig', () => {
    it('should update audit config and log audit', async () => {
      const result = await adminService.updateAuditConfig(mockAuditContext, { enabled: true, trackedTables: ['users'] });
      expect(result.isOk()).toBe(true);
    ...
    it('should handle failures', async () => {
      vi.mocked(systemRepository.setAuditTrailEnabled).mockImplementationOnce(() => { throw new Error('fail'); });
  ...
  describe('getDashboardStats', () => {
    ...
    it('should handle failures', async () => {
      vi.mocked(db.select).mockImplementationOnce(() => { throw new Error('fail'); });
      const result = await adminService.getDashboardStats();
      expect(result.isErr()).toBe(true);
    });
  });
  ...
  describe('AboutTimeline', () => {
    ...
    it('createAboutTimelineEntry', async () => {
      ...
      vi.mocked(aboutService.createTimelineEntry).mockResolvedValueOnce(err(new InternalError('fail')));
    ...
    it('updateAboutTimelineEntry', async () => {
      ...
      vi.mocked(aboutService.getTimelineEntry).mockResolvedValueOnce(err(new NotFoundError('fail')));
      const resErr1 = await adminService.updateAboutTimelineEntry(mockAuditContext, 1, {});
      expect(resErr1.isErr()).toBe(true);

      vi.mocked(aboutService.getTimelineEntry).mockResolvedValueOnce(ok({ id: 1 } as any));
      vi.mocked(aboutService.updateTimelineEntry).mockResolvedValueOnce(err(new InternalError('fail')));
    ...
    it('deleteAboutTimelineEntry', async () => {
      ...
      vi.mocked(aboutService.deleteTimelineEntry).mockResolvedValueOnce(err(new InternalError('fail')));
