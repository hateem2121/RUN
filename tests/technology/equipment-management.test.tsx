...
const MockTechnologyEquipmentManagement = ({ isLoading }: any) => {
	...
	return (
		<div data-testid="equipment-management">
			...
			<div data-testid="equipment-header">
			...
			<button
				...
				onClick={() =>
					setEquipmentForm({ ...equipmentForm, name: "Updated Equipment" })
				}
			>
				Update Name
			</button>
...
describe("TechnologyEquipmentManagement - Extracted Module", () => {
	...
	it("should render equipment management interface", () => {
		...
		expect(
			screen.getByText("Manage your advanced manufacturing equipment"),
		).toBeInTheDocument();
	...
	it("should display equipment details correctly", () => {
		...
		expect(screen.getByTestId("equipment-name")).toHaveTextContent(
			"Automated Cutting System",
		);
		expect(screen.getByTestId("equipment-category")).toHaveTextContent(
			"Cutting",
		);
	...
	it("should handle equipment capabilities correctly", () => {
		...
		expect(screen.getByTestId("capability-0")).toHaveTextContent(
			"High Precision",
		);
	...
	it("should display specifications properly", () => {
		...
		expect(screen.getByTestId("spec-Max Speed")).toHaveTextContent(
			"Max Speed: 1000 rpm",
		);
	});
