...
describe("Security Hardening (Integration Tier)", () => {
  ...
  it(
    "should RETURN_404 for /api/debug/crash in PRODUCTION mode",
    async () => {
  ...
  it(
    "should RETURN_404 for /api/debug/crash in TEST mode if ENABLE_DEBUG_ROUTES is missing",
    async () => {
      server = await startTestServer({
        NODE_ENV: "test",
  ...
  it(
    "should RETURN_404 if Token is missing (even if enabled)",
    async () => {
      server = await startTestServer({
        NODE_ENV: "test",
        ENABLE_DEBUG_ROUTES: "true",
  ...
  it(
    "should RETURN_404 if Token is incorrect",
    async () => {
      server = await startTestServer({
        NODE_ENV: "test",
        ENABLE_DEBUG_ROUTES: "true",
  ...
  it(
    "should BLOCK .map files with 404 in PRODUCTION",
    async () => {
      server = await startTestServer({
        NODE_ENV: "production",
      });
  ...
  it(
    "should REMOVE X-Powered-By header in PRODUCTION",
    async () => {
      server = await startTestServer({
        NODE_ENV: "production",
      });
  ...
  it(
    "should ENFORCE Content-Security-Policy header in PRODUCTION",
    async () => {
      server = await startTestServer({
        NODE_ENV: "production",
      });
