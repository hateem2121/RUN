...
const MediaAssetSchema = z.object({
  ...
  deletedAt: z.union([z.date(), z.string(), z.null()]).nullable().transform((val) => {
    if (!val) return null;
    if (val instanceof Date) return val;
 ...
 */
export const validateMediaAssetArray = (data: unknown[]): {
  valid: MediaAsset[];
...
export const filterValidMediaAssets = (assets: unknown[]): MediaAsset[] => {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[Schema Validator] Validation Summary:', result.summary);

    if (result.invalid.length > 0) {
      console.warn('[Schema Validator] Invalid assets found:');
      result.invalid.forEach(({ index, errors }) => {
        console.warn(`  Asset ${index}:`, errors);
      });
    }
  }
 ...
 */
export const monitorSchemaDrift = (apiData: unknown[], context = 'unknown'): void => {
  if (process.env.NODE_ENV !== 'development') {
  ...
  if (result.invalid.length > 0) {
    console.warn(`[Schema Monitor] ${context}: ${result.invalid.length}/${result.summary.total} assets failed validation`);

    // Sample first few errors for debugging
    result.invalid.slice(0, 3).forEach(({ index, errors }) => {
      console.warn(`[Schema Monitor] Sample error (asset ${index}):`, errors[0]);
    });
  }

  if (result.summary.warnings > 0) {
    console.warn(`[Schema Monitor] ${context}: ${result.summary.warnings} validation warnings detected`);
  }
};