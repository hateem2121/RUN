...
export const parseApiDate = (dateString: string | null | undefined): Date | null => {
  ...
  try {
    ...
    if (isNaN(date.getTime())) {
      console.warn('[DateHelpers] Invalid date string received:', dateString);
      return null;
    }
    return date;
  } catch (error) {
    console.error('[DateHelpers] Failed to parse date:', dateString, error);
    return null;
  }
};
...
export const formatApiDate = (date: Date | null | undefined): string | null => {
  ...
  try {
    ...
    console.error('[DateHelpers] Failed to format date:', date, error);
    return null;
  }
};
